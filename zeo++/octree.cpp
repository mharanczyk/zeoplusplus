...
bool Node::Threshold(vector<Point>& objects) {
  if ((object.size() < MAXOBJECTSPERNODE) || m_Depth > MAXOCTREEDEPTH ||
      m_Length > MINBOXSIZE) {
    return true;
  }
  return false;
...
Node* Node::InitializeChildren() {
  ...
  // Now I will add all node specific information Eg. Location, and pointers
  // which will significantly speed up the code
...
bool Node::Subset(Sphere object) {
  // If the center lies within the cube then it is a member (Of course this
  // needs to be Extended
  if (m_ABC[0] + m_Length > object.center[0] &&
      m_ABC[0] - m_Length < object.center[0]) {
    if (m_ABC[1] + m_Length > object.center[1] &&
        m_ABC[1] - m_Length < object.center[1]) {
      if (m_ABC[2] + m_Length > object.center[2] &&
          m_ABC[2] - m_Length < object.center[2]) {
        return true;
      }
    }
  }
