...
UNIT_CELL::UNIT_CELL(double vax, double vbx, double vby, double vcx, double vcy,
                     double vcz) {
  va_x = vax;
  vb_x = vbx;
  vb_y = vby;
  ...
  inv_va_x = 1 / va_x;
  inv_vb_x = invDet * -1 * vc_z * vb_x;
  inv_vb_y = 1 / vb_y;
  inv_vc_x = invDet * (vc_y * vb_x - vb_y * vc_x);
  inv_vc_y = invDet * -1 * vc_y * va_x;
  inv_vc_z = 1 / vc_z;
}
...
void UNIT_CELL::abc_to_xyz(double a, double b, double c, double &x, double &y,
                           double &z) {
...
/** Transform the coordinates relative to the xyz coordinate system vectors into
 * ones relative to the unit cell vectors and store them using the provided
 * pointers. */
void UNIT_CELL::xyz_to_abc(double x, double y, double z, double &a, double &b,
                           double &c) {
...
DISP_INFO::DISP_INFO() { isReal = false; }
...
/* Returns the energy of the point located in the grid referred to by the
 * TRIPLET of indices. Assumes the grid indices are within the appropriate
 * range.*/
 ...
 *  is indeed accessible. Assumes the grid indices are within the appropriate
 * range. */
 ...
 *  Stores a TRIPLET representing to which unit cell the indices originally
 * referred.*/
void adjustIndices(TRIPLET &gridIndices, TRIPLET &shift, UNIT_CELL &uc) {
  ...
  newA = translate_to_original_uc(a);
  newB = translate_to_original_uc(b);
  newC = translate_to_original_uc(c);
  double x, y, z;
  uc.abc_to_xyz(newA, newB, newC, x, y, z);
  shift =
      TRIPLET(nearestInt(a - newA), nearestInt(b - newB), nearestInt(c - newC));
  gridIndices = TRIPLET(nearestInt(x), nearestInt(y), nearestInt(z));
}

/* Returns true iff the grid point referred to by the provided triplet of grid
 * indices is indeed accessible. Assumes the grid indices are within the
 * appropriate range. */
...
bool findMinEnergyBarrier(int startX, int startY, int startZ, double &barrier,
                          int numX, int numY, int numZ, float *energyGrid,
                          float *accessGrid, UNIT_CELL unit_cell) {
  barrier = DBL_MAX;
  vector<DISP_INFO> visitedNodes =
      vector<DISP_INFO>(numX * numY * numZ, DISP_INFO());

  // Place starting node on heap with (0,0,0) displacement
  HEAP<pair<TRIPLET, DISP_INFO> > heap(
      hasHigherEnergy);  // Lowest energy paths are favored
  heap.insert(pair<TRIPLET, DISP_INFO>(
      TRIPLET(startX, startY, startZ),
      DISP_INFO(0, 0, 0,
                getEnergy(startX, startY, startZ, numX, numY, energyGrid))));

  while (heap.size() != 0) {
    // Remove best remaining path
    pair<TRIPLET, DISP_INFO> pathInfo = heap.pop();
    int arrayIndex = transformIndex(pathInfo.first[0], pathInfo.first[1],
                                    pathInfo.first[2], numX, numY);
      ...
      for (int i = 0; i < NUM_DIRECTIONS; i++) {
        ...
        if (isAccessible(newIndices, numX, numY, accessGrid)) {
          ...
          heap.insert(pair<TRIPLET, DISP_INFO>(
              newIndices,
              DISP_INFO(da, db, dc,
                        max(pathInfo.second.maxEnergy,
                            getEnergy(newIndices, numX, numY, energyGrid)))));
        }
      }
    }
  }
...
/* Calculates the minimum energy barrier of a path that must pass through the
 * point referred to by the provided grid indices. The resulting barrier is
 * stored using the provided pointer. If no path is possible,
 * the barrier is set to DBL_MAX. The function returns true iff a path is found.
 */
bool calculateMinEnergyBarrier(double &minEnergy, double &barrierEnergy,
                               float box_x, float box_y, float box_z, int numX,
                               int numY, int numZ, float *energyGrid,
                               float *accessGrid, double vax, double vbx,
                               double vby, double vcx, double vcy, double vcz) {
  ...
  if ((minX == -1) || (minY == -1) || (minZ == -1)) {
    cerr << "Error: Unable to identify global energy minimum when calculating "
            "energy barriers."
         << "\n"
  ...
  double grid_spacing_x = box_x / numX, grid_spacing_y = box_y / numY,
         grid_spacing_z = box_z / numZ;
  vax /= grid_spacing_x;
  vbx /= grid_spacing_x;
  vby /= grid_spacing_y;
  vcx /= grid_spacing_x;
  ...
  cout << "Calculating energy barrier for box of dimensions:" << box_x << " by "
       << box_y << " by " << box_z << "\n"
  ...
  bool result =
      findMinEnergyBarrier(minX, minY, minZ, barrierEnergy, numX, numY, numZ,
                           energyGrid, accessGrid, unit_cell);
