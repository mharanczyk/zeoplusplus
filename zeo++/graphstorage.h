...
class DIJKSTRA_NODE {
 public:
  ...
  std::vector<CONN> connections;  // List of connections that lead from the node
                                  // to other nodes
  ...
  /* Creates a node. Default is with an invalid id, no connections and a radius
   * of 0.*/
  DIJKSTRA_NODE(int myID = -1, double myX = 0, double myY = 0, double myZ = 0,
                double maxR = 0, bool active_flag = true);

  /** Output information about the node to the provided output stream. Default
   * is std::cout*/
  void print(std::ostream &out = std::cout) const;
};
...
class DIJKSTRA_NETWORK {
 public:
  ...
  /* Output information about the network to the provided output stream. Default
   * is std::cout*/
  void print(std::ostream &out = std::cout);

  static void buildDijkstraNetwork(const VORONOI_NETWORK *vornet,
                                   DIJKSTRA_NETWORK *dnet);
  static void filterDnetEdges(std::vector<int> nodeIDs, VORONOI_NETWORK *vornet,
                              DIJKSTRA_NETWORK *dnet);
};
