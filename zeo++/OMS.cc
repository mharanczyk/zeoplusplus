...
void Sort(double **MoleculeCoordinates, int Criterion, int Size) {
  int i, j;
  double *Swap;

  for (i = 0; i < Size - 1; i++)
    for (j = 0; j < Size - i - 1; j++)
      if (MoleculeCoordinates[j][Criterion] >
          MoleculeCoordinates[j + 1][Criterion]) {
...
/* For a plane given through two vectors (departing from the origin), and a
 * point, returns: 0 if the point is in the plane -1 if the point is in the
 * "bottom" of the plane (normal sign negative) +1 if the point is in the "top"
 * of the plane (normal sign positive)
 */
double PlaneSide(double *Vector1, double *Vector2, double *Point) {
  ...
  /**** Fill the matrix ****/
  // Each column contains one vector, so the system solved returns the
  // coordinates of the point in the system v1,v2,n
  for (int i = 0; i < DIM; i++)
    for (int j = 0; j < DIM; j++)
      if (i == 0)
        A(j, i) = v1(j);
...
bool IsExposedMolecule(vector<vector<double> > MoleculeCoordinates) {
  ...
  // vector<double>(DIM) ); vector < vector <double> >
  // MoleculeSphericalCoordinates(Size);

  double **TranslatedCoordinates =
      (double **)malloc((NPoints - 1) * sizeof(double *));
  ...
  // Displace all the coordinates in order to set the metal molecule at the
  // center
  for (i = 1; i < NPoints; i++) {
    TranslatedCoordinates[i - 1] = (double *)malloc(sizeof(double) * DIM);
    // Assign values from input to the spherical coordinates vector and
  ...
  /* For every two non-metal atoms in the molecule, form a vector (M-O), then a
   * plane, then decide whether the rest of the poins are
  ...
  for (i = 0; i < NPoints - 1; i++)
    for (j = i + 1; j < NPoints - 1; j++) {
      ...
      if (!LinearlyDependent(TranslatedCoordinates[i],
                             TranslatedCoordinates[j])) {
        for (k = 0; k < NPoints - 1; k++)
          if (k != i && k != j) {
            Lambda =
                PlaneSide(TranslatedCoordinates[i], TranslatedCoordinates[j],
                          TranslatedCoordinates[k]);

            // cout << "k: " << k << "\nLambda: " << Lambda << "\n";

            S_lambda = Sign(Lambda);

            // Update S_lambda_old only the first time that the point is not in
            // the plane
            if (Lambda != 0 && S_lambda_old == 0) S_lambda_old = Sign(Lambda);

            // If the sign changes anytime, stop this loop (the plane has points
            // on both sides)
...
double PlaneAngle(double *Vector1, double *Vector2, double *Point) {
  ...
  PVMod = sqrt(	VectorToPoint(0)*VectorToPoint(0) +
VectorToPoint(1)*VectorToPoint(1) + VectorToPoint(2)*VectorToPoint(2) ); PNMod =
sqrt(	PlaneNormal(0)*PlaneNormal(0) + PlaneNormal(1)*PlaneNormal(1) +
...
/*bool IsExposedMoleculeThreshold (vector < vector <double> >
MoleculeCoordinates, double Threshold  )
{

        int NPoints = (int) MoleculeCoordinates.size();

        double **TranslatedCoordinates = (double **)
malloc((NPoints-1)*sizeof(double *)); double *OriginCoordinates = (double *)
malloc (DIM*sizeof(double));
        ...
        // Displace all the coordinates in order to set the metal molecule at
the center for(i = 1; i < NPoints; i++)
        {
                TranslatedCoordinates[i-1] = (double *)
malloc(sizeof(double)*DIM);
                // Assign values from input to the spherical coordinates vector
and translate it to origin for (j = 0; j < DIM; j++)
                        TranslatedCoordinates[i-1][j] =
MoleculeCoordinates.at(i).at(j) - OriginCoordinates[j];
        }
        ...
        for (i = 0; i < NPoints-1; i++)
                for (j = i+1; j < NPoints-1; j++)
                {
                        ...
                        Theta = 0.;
                        Exposure = 0.;
                        // For every point except the two composing the plane,
compute if (!LinearlyDependent(TranslatedCoordinates[i],
TranslatedCoordinates[j]))
                        {
                                for (k = 0; k < NPoints-1; k++)
                                        if (k != i && k != j )
                                        {
                                                ...
                                                Lambda = PlaneSide
(TranslatedCoordinates[i], TranslatedCoordinates[j], TranslatedCoordinates[k]);
...
" << Lambda << "\n";
                                                ...
                                                // Update S_lambda_old only the
first time that the point is not in the plane if (Lambda != 0 && S_lambda_old ==
0) S_lambda_old = Sign(Lambda);

                                                // If the sign changes anytime,
check the angle if (S_lambda != S_lambda_old)
                                                {
                                                        Theta =
PlaneAngle(TranslatedCoordinates[i], TranslatedCoordinates[j],
TranslatedCoordinates[k]);
...
exceeds the threshold, stop this iteration if (abs(Theta) > Threshold)
                                                        {
                                                                PlaneFound =
false;
                                                                //cout << "This
plane separates the points \n"; break;
                                                        }
                                                        else if (abs(Theta) >
Exposure)
                                                        {
...
abs(Theta);
                        ...
                        else 	// If the vectors are linearly dependent the
iteration is ignored
                        ...
                        // Check if a plane with all the points on one side was
found if (PlaneFound) return true;
                                //return Exposure;
                }
...
bool IsExposedMoleculeThreshold(vector<vector<double> > MoleculeCoordinates,
                                double Threshold) {
  int NPoints = (int)MoleculeCoordinates.size();
  ...
  for (i = 0; i < NPoints - 1; i++)
    for (j = i + 1; j < NPoints - 1; j++) {
      ...
      if (!LinearlyDependent(TranslatedCoordinates[i],
                             TranslatedCoordinates[j])) {
        // POINTS ITERATION: Compare the points with the plane selected in
        // previous iterations
        for (k = 0; k < NPoints - 1; k++)
          if (k != i && k != j) {
            // Compute the side of the plane
            ...
            if (S_lambda > 0) {
              Theta =
                  PlaneAngle(TranslatedCoordinates[i], TranslatedCoordinates[j],
                             TranslatedCoordinates[k]);
              if (Theta > Exposure_pos) Exposure_pos = Theta;
            }
            if (S_lambda < 0) {
              Theta =
                  PlaneAngle(TranslatedCoordinates[i], TranslatedCoordinates[j],
                             TranslatedCoordinates[k]);
              if (Theta > Exposure_neg) Exposure_neg = Theta;
            }
          }

        // Compute exposures (only if the vector forms a plane)
        Exposure_plane = min(Exposure_pos, Exposure_neg);
        if (Exposure_plane < Threshold) return true;
      }
    }

  // Maximum angle formed by a point not contained within the same side of the
  // rest of the points
  return false;
}
...
double DegreeOfExposure(vector<vector<double> > MoleculeCoordinates) {
  ...
  // Angles
  double Theta = 0.;
  double Exposure_pos = 0., Exposure_neg = 0., Exposure = PI / 2.,
         Exposure_plane = 0.;
