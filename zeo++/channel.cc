...
bool CHANNEL::isUnicellular() { return unitCells.size() == 1; }
 ...
 */
PORE::PORE(vector<int> nodeIDs, DIJKSTRA_NETWORK *dnet, int dim,
           int basisVecs[3][3]) {
  ...
  for (unsigned int i = 0; i < nodeIDs.size(); i++) {
    ...
    DIJKSTRA_NODE newNode =
        DIJKSTRA_NODE(i, oldNode.x, oldNode.y, oldNode.z, oldNode.max_radius);
    ...
    for (unsigned int j = 0; j < oldNode.connections.size(); j++) {
      ...
      if (id2Iter != idMappings.end()) {
        CONN newConn = CONN(id1Iter->second, id2Iter->second, oldConn.length,
                            oldConn.max_radius, oldConn.deltaPos);
  ...
  basis[0][0] = basisVecs[0][0];
  basis[0][1] = basisVecs[0][1];
  basis[0][2] = basisVecs[0][2];
  basis[1][0] = basisVecs[1][0];
  basis[1][1] = basisVecs[1][1];
  basis[1][2] = basisVecs[1][2];
  basis[2][0] = basisVecs[2][0];
  basis[2][1] = basisVecs[2][1];
  basis[2][2] = basisVecs[2][2];
  dimensionality = dim;
 ...
 * CHANNEL and inaccessible POCKET. This function identifies the POREs that
 * exist for the provided particle diameter and stores them using the provided
 * pointer to a vector of POREs. CHANNEL and PORE can be distinguished by
 * dimentionality. In addition, the pointer to the vector of bools is used to a
 * store bool
 *  for each VORONOI_NODE, where infoStorage[i] is true if node #i is
 * accessible. */
/*  WARNING: this function assumes that at some point before VORONOI_NETWORK was
 * pruned with prune() function. This is to set "active" flag on nodes larger
 * than probe radius */
void PORE::findChannelsAndPockets(DIJKSTRA_NETWORK *dnet,
  ...
  int noAccDijkNodes =
      0;  // number of active Dijkstra nodes (ones with radius > 0)
  int noAccDijkNodesAssigned =
      0;  // number of active Dijkstra nodes already assigned to pores.
  ...
  cout << "\nFinding channels and pockets in Dijkstra network of "
       << dnet->nodes.size() << " node(s). " << noAccDijkNodes
       << " are expected to compose pores." << endl;
  ...
  while (nodeIndex != accessStatuses.size()) {
    ...
    int dim = 0;
    int basis[3][3] = {{0, 0, 0}, {0, 0, 0}, {0, 0, 0}};
    ...
    visitedNodeDisplacement.insert(
        pair<int, DELTA_POS>(nodeIndex, displacement));
    ...
    while (stack.size() != 0) {
      ...
      while (connIter != currentNode.connections.end()) {
        ...
        if (localAccessStatus == UNKNOWN) {
          map<int, DELTA_POS>::iterator visitedNode =
              visitedNodeDisplacement.find(to);
          if (visitedNode != visitedNodeDisplacement.end()) {
            if (visitedNode->second.equals(newDisplacement)) {
              // Circling back to previous node
              // Do nothing
            } else {
              // Nodes are ACCESSIBLE b/c same node visited in different unit
              // cell
              ...
              if (dim == 0) {
                ...
                if (basis[0][0] != 0)
                  v = 1.0 * direction.x / basis[0][0];
                else if (basis[1][0] != 0)
                  v = 1.0 * direction.y / basis[1][0];
                else if (basis[2][0] != 0)
                  v = 1.0 * direction.z / basis[2][0];
                else {
                ...
                if (!(basis[0][0] * v == direction.x &&
                      basis[1][0] * v == direction.y &&
            ...
            visitedNodeDisplacement.insert(
                pair<int, DELTA_POS>(to, newDisplacement));
          ...
          // were not explored.
          cerr << "Error: Illogical result  when attempting to identify "
                  "channels/pockets."
               << "\n";
          cerr << "Please contact the source code provider with your program "
                  "input. "
               << "\n";
          cerr << "Exiting ..."
               << "\n";
    ...
    if (accessStatus == ACCESSIBLE) {
      ...
      // output << dim << " ";
    } else {  // OTHERWISE create POCKET from INACCESSIBLE nodes (dim set to 0
              // indicates pocket)
      // additionally pockets have to be built from active nodes (this is to
      // ensure their size is larger than mix_radius)
    ...
    for (unsigned int i = 0; i < listOfIDs.size(); i++) {
      if (i == 0)
        flg = dnet->nodes[listOfIDs[0]].active;
      else {
        if (flg != dnet->nodes[listOfIDs[i]].active)
          cout << "pocket error" << endl;
      };
    };
    // end of debug
  }
  ...
  for (unsigned int i = 0; i < pores->size(); i++) {
    if (pores->at(i).dimensionality > 0)
      nchannel++;
    else
      npocket++;
  };
  cout << "Analyzed and assigned " << nodeIndex << " nodes.";
  cout << "\nIdentified " << nchannel << " channels and " << npocket
       << " pockets.\n";
 ...
 * either accessible or inaccessible based on a give probe diameter. This is
 * done by constructing DIJKSTRA_NETWORK and calling the other
 * findChannelsAndPockets function */
...
/** Calculates center of mass and internal void radii (distance between the
 * center of mass and its nearest atom) This function attempts pore
 * reconstruction in cases where pores cross the cell boundaries.
 */
pair<XYZ, double> PORE::getCenterOfMass() {
  /*
   Pore variables
      std::vector<DELTA_POS> unitCells;    // List of displacements of unitcells
   in single PORE unit std::vector< std::vector<int> > ucNodes;  // List of
   nodes contained in each unit cell XYZ v_a, v_b, v_c;              // Unit
   cell vectors
  ...
  */
  ...
  for (int unsigned i = 0; i < unitCells.size(); i++) {
    for (int unsigned j = 0; j < ucNodes[i].size(); j++) {
      ...
      XYZ deltapos(double(unitCells[i].x), double(unitCells[i].y),
                   double(unitCells[i].z));

      XYZ node(nodes[nd].x, nodes[nd].y, nodes[nd].z);

      node = node + v_a.scale(deltapos.x) + v_b.scale(deltapos.y) +
             v_c.scale(deltapos.z);
  ...
  for (int unsigned i = 0; i < reconstructedNodeCoords.size(); i++) {
    if (i == 0) {
      ...
      if (centerOfMass.euclid_dist(reconstructedNodeCoords[i]) < dist)
        dist = centerOfMass.euclid_dist(reconstructedNodeCoords[i]);
    };
  };

  if (dimensionality > 0)
    cout << "Center of Mass calculation: PORE dimensionality>0; results may be "
            "buggy due to PORE reconstruction handling\n";

  /* DEBUGGING: Printing out coordinates
   for(int unsigned i=0; i<reconstructedNodeCoords.size(); i++)
      {
      cout << "C    " << reconstructedNodeCoords[i].x << "  " <<
   reconstructedNodeCoords[i].y << "  " << reconstructedNodeCoords[i].z << "\n";
      };
  */
...
/**  This function attempts pore reconstruction in cases where pores cross the
 * cell boundaries. It returns a vector of vectors with pore XYZ coordinates
 *   */
vector<pair<int, XYZ> > PORE::getReconstructedPore() {
  /*
   *  Pore variables
   *      std::vector<DELTA_POS> unitCells;    // List of displacements of
   * unitcells in single PORE unit std::vector< std::vector<int> > ucNodes;  //
   * List of nodes contained in each unit cell XYZ v_a, v_b, v_c; // Unit cell
   * vectors
   *
   *              */
  ...
  /* DEBUGGING: Check if reconstructured nodes become duplicate atoms
   ...
   cout << "DEBUG: Pore/molecule size after reconstruction (XYZ coords) " <<
   reconstructedNodeCoords.size() << endl;
  */

  if (dimensionality > 0)
    cout << "Calling PORE::getReconstructedPore for a pore with dim>0, it was "
            "not intended. DO NOT TRUST\n";
...
/**  This function attempts pore reconstruction in cases where pores cross the
 * cell boundaries. In case of pores/molecules that cross boundy, multiple
 * copies are saved each corresponding to one instance in periodic box. It
 * returns a vector of vectors with pore XYZ coordinates and Ids
 *   */

vector<vector<pair<int, XYZ> > >
PORE::getReconstructredPoresWithCrossBoundryCopies() {
  /*
   *  Pore variables
   *      std::vector<DELTA_POS> unitCells;    // List of displacements of
   * unitcells in single PORE unit std::vector< std::vector<int> > ucNodes;  //
  ...
  vector<pair<int, XYZ> >
      reconstructedNodeCoords; /* holds the reconstructure pore/molecule */
  ...
  vector<vector<pair<int, XYZ> > >
      reconstructedCopies;  // This will store copies of all reconstructure
                            // pores/molecules

  if (dimensionality > 0)
    cout << "Calling PORE::getReconstructredPoresWithCrossBoundryCopies() for "
            "a pore with dim>0, it was not intended. DO NOT TRUST\n";
  ...
  for (int unsigned i = 0; i < unitCells.size(); i++) {
    ...
    for (int unsigned j = 0; j < reconstructedNodeCoords.size(); j++) {
      ...
      reconstructedCopies[i].at(j).second =
          reconstructedCopies[i].at(j).second - v_a.scale(deltapos.x) -
          v_b.scale(deltapos.y) - v_c.scale(deltapos.z);
    };
  };
...
/* Prints information about the pore to the provided output stream, including
 nuber of nodes, the largest included sphere, positions and radii of all nodes
 */
void PORE::printPoreSummary(ostream &out, ATOM_NETWORK *atmNet) {
  ...
  out << nodes.size() << "  " << poresummary[0] << "  " << poresummary[1]
      << "  " << poresummary[2] << "  " << poresummary[3] << "  "
      << poresummary[4] << "\n";
 ...
 * the number of nodes, unitcells, and the nodes located in each unit cell.
 * Additional node information is outputted if requested.*/
void CHANNEL::print(ostream &out, bool dispNodeInfo) {
  ...
  for (unsigned int i = 0; i < unitCells.size(); i++) {
    ...
    out << "       Unit cell #: " << i << "\n"
        << "          Displacement: " << position.x << " " << position.y << " "
        << position.z << "\n"
        << "          New Node ids: ";
...
bool ReconstructorComparator::compare(pair<int, DELTA_POS> p1,
                                      pair<int, DELTA_POS> p2) {
...
/* Reconstructs the PORE by propagating paths until all nodes have been
 * accessed. Stores each node in the unit cell in which it is encountered.
...
void PORE::reconstruct() {
  ...
  stack.insert(pair<int, DELTA_POS>(
      0, DELTA_POS(0, 0, 0)));  // Pick the first node as the starting point
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) {
    map<DELTA_POS, vector<int> >::iterator iter =
        results.find(displacements.at(i));
    if (iter == results.end()) {
      vector<int> newList;
      newList.push_back(i);
      results.insert(
          pair<DELTA_POS, vector<int> >(displacements.at(i), newList));
    } else
...
void CHANNEL::writeToVMD(int n, fstream &output) {
  if (!output.is_open()) {
    cerr << "Error: File stream needed to print channel information was not "
            "open."
         << "\n"
         << "Exiting ..."
         << "\n";
    ...
    for (unsigned int i = 0; i < unitCells.size(); i++) {
      ...
      for (unsigned int j = 0; j < nodeIDs.size(); j++) {
        ...
        double xCoord =
            curNode.x + v_a.x * disp.x + v_b.x * disp.y + v_c.x * disp.z;
        double yCoord =
            curNode.y + v_a.y * disp.x + v_b.y * disp.y + v_c.y * disp.z;
        double zCoord =
            curNode.z + v_a.z * disp.x + v_b.z * disp.y + v_c.z * disp.z;

        // Command used to draw node
        output << "{sphere {" << xCoord << " " << yCoord << " " << zCoord
               << "} radius $nodeRadii(" << nodeIDs.at(j)
...
void CHANNEL::writeToVMD(string type, int n, fstream &output) {
  if (!output.is_open()) {
    cerr << "Error: File stream needed to print" << type
...
CHANNEL::CHANNEL(PORE *p) {
  ...
  basis[0][0] = p->basis[0][0];
  basis[0][1] = p->basis[0][1];
  basis[0][2] = p->basis[0][2];
  basis[1][0] = p->basis[1][0];
  basis[1][1] = p->basis[1][1];
  basis[1][2] = p->basis[1][2];
  basis[2][0] = p->basis[2][0];
  basis[2][1] = p->basis[2][1];
  basis[2][2] = p->basis[2][2];
}
 ...
 * a CHANNEL. As a result, this function identifies the CHANNELs that exist for
 *  the provided particle diameter and  stores them using the provided pointer
 *  to a vector of channels. In addition, the pointer to the vector of bools is
 *  used to a store bool for each VORONOI_NODE, where infoStorage[i] is true
 *  iff node #i is accessible. */
...
/** Stores the ids of all atoms that bound this channel using the provided
 * vector reference. An atom is considered to bound a channel if a node in the
 * channel is a member of the atom's Voronoi cell. */
void CHANNEL::findBoundingAtoms(ATOM_NETWORK *atmnet,
                                vector<BASIC_VCELL> &vcells,
                                vector<int> &atomIDs) {
  atomIDs.clear();
...
void PORE::buildDijkstraNetwork(DIJKSTRA_NETWORK *dnet) {
  ...
  //       DIJKSTRA_NODE node = DIJKSTRA_NODE(i, niter->x, niter->y, niter->z,
  //       niter->rad_stat_sphere); i++; niter++; dnet->nodes.push_back(node);
  //   }
  //
  //  // For each edge, store it in the DIJKSTRA node's list of connections that
  //  the connection
  //  // stems from
  //  vector<VOR_EDGE> ::iterator eiter = vornet->edges.begin();
  //  while(eiter != vornet->edges.end()){
  //      DELTA_POS pos = DELTA_POS(eiter->delta_uc_x, eiter->delta_uc_y,
  //      eiter->delta_uc_z); CONN conn = CONN(eiter->from, eiter->to,
  //      eiter->length, eiter->rad_moving_sphere,pos);
...
pair<double, pair<double, double> > CHANNEL::findFreeIncludedSphereDiameter() {
  ...
  for (int i = 0; i < nodes.size(); i++) {
    ...
    if (i == 0) {
      ...
      pair<double, pair<double, double> > didfdif =
          findFreeIncludedSphereDiameterforNode(i, maxdidfdif);
...
bool compareConnections(pair<int, int> C1, pair<int, int> C2) {
  return (compareConnections_ptr->at(C1.first)
              .connections.at(C1.second)
              .max_radius < compareConnections_ptr->at(C2.first)
                                .connections.at(C2.second)
                                .max_radius);
}

/* Return the largest free and included sphere diameters (df and dif)  starting
 * from a partiular node  of the current channel
 * the function also accepts point to a vector that flags nodes accessed nodes
 * during execution of the function It uses a pair<pair> object that storage the
 * current 'record' Di/df/dif This is to speed-up by early discart of paths with
 * more restriction than the current path
 */
pair<double, pair<double, double> >
CHANNEL::findFreeIncludedSphereDiameterforNode(
    int nodeid, pair<double, pair<double, double> > maxdidfdif) {
  // Define three access types
  ...
  vector<DELTA_POS> directions =
      vector<DELTA_POS>(lnodes.size(), DELTA_POS(0, 0, 0));
  ...
  maxdidfdif.first =
      2.0 * lnodes.at(nodeid).max_radius;  // .first is use to store the current
                                           // Di for the current (nodeid) node

  // Test if it is worth to run the funtion (e.g. starting node's radius is
  // smaller than the current Df)
  if (lnodes.at(nodeid).max_radius < maxdidfdif.second.first / 2.0) {
    return maxdidfdif;  // if not, just return the current best numbers
                        // (maxdidfdif)
  };
  ...
  maxdi = lnodes.at(nodeIndex).max_radius;  // (it is not really the max but
                                            // will be analyzed to get max)
  ...
  for (unsigned int i = 0; i < lnodes[nodeIndex].connections.size(); i++) {
    if (lnodes[nodeIndex].connections.at(i).max_radius >
        lnodes.at(lnodes[nodeIndex].connections.at(i).to).max_radius)
      lnodes[nodeIndex].connections.at(i).max_radius =
          lnodes.at(lnodes[nodeIndex].connections.at(i).to).max_radius;
  ...
  while (stack.size() != 0 && flag) {
    ...
    if (accessStatuses[best_node] == UNKNOWN) {
      ...
      // update df and di
      if (lnodes[best.first].connections.at(best.second).max_radius < df)
        df = lnodes[best.first].connections.at(best.second).max_radius;

      if (lnodes.at(best_node).max_radius > di)
        di = lnodes.at(best_node).max_radius;
      ...
      for (unsigned int i = 0; i < lnodes[best_node].connections.size(); i++) {
        if (lnodes[best_node].connections.at(i).max_radius >
            lnodes.at(lnodes[best_node].connections.at(i).to).max_radius)
          lnodes[best_node].connections.at(i).max_radius =
              lnodes.at(lnodes[best_node].connections.at(i).to).max_radius;
      ...
      if (!(directions[best_node])
               .equals(
                   directions[best.first] +
                   lnodes[best.first].connections.at(best.second).deltaPos)) {
        // if nodes are in different unit cells, a loop is found, the resulting
...
/* Return the largest free and included sphere diameters (df and dif) for a path
 * between two nodes within the current pore. Return -1 in nodes are not
 * connected. Node1 and Node2 are indexes provied w.r.t. the original network.
 * Use idMapping inside this function to obtain local indexes for the considered
 * PORE
 */

pair<double, double> PORE::getFreeIncludedSphereDiameterforNodePair(int node1,
                                                                    int node2) {
  // Define three access types
  const int ACCESSED = 1;
  const int UNKNOWN = -1;
  ...
  nodeIndex = node1;  // node1 is w.r.t pore node index (use PORE reverseMapping
                      // to retrieve original node
    ...
    if (accessStatuses[best_node] == UNKNOWN) {
      ...
      if (!(directions[best_node])
               .equals(
                   directions[best.first] +
                   lnodes[best.first].connections.at(best.second).deltaPos)) {
        // if nodes are in different unit cells (a loop is found); just update
        // df and continue
        if (df > nodes[best.first].connections.at(best.second).max_radius)
          df = nodes[best.first].connections.at(best.second).max_radius;
      };
      // otherwise do nothing
      // need to make sure heap is correct (which should be true as nothing was
      // added)
    }
...
/* Get information about the pocket: di, coordinates and encapsulating sphere
 * radius */
void PORE::getSimplifiedPocketInfo(ATOM_NETWORK *atmNet,
                                   std::vector<double> *pocketInfo) {
  double maxdi;
  double maxid = 0;
  pocketInfo->clear();
  ...
  Point pt = atmNet->xyz_to_abc(nodes.at(maxid).x, nodes.at(maxid).y,
                                nodes.at(maxid).z);
  pt = atmNet->shiftABCInUC(pt);
  pocketInfo->push_back(pt[0]);
  pocketInfo->push_back(pt[1]);
  pocketInfo->push_back(pt[2]);

  double pocketR;  // radii of pocket (sphere that encapsulates all nodes in the
                   // pocket)

  pocketR = maxdi * 0.5;
  for (unsigned int i = 0; i < nodes.size(); i++) {
    double dist = calcEuclideanDistance(nodes.at(maxid).x, nodes.at(maxid).y,
                                        nodes.at(maxid).z, nodes.at(i).x,
                                        nodes.at(i).y, nodes.at(i).z);
...
/* Functions gets restrictions between provided sements of Voronoi network
 * within the current PORE */
/* It takes the original number of segments (to be specified by user) and a list
   of original Voronoi nodes and their segment assigments (that is w.r.t. to
   pre-channel detection) than analyzed segment connectivity within the currnet
   pore.
*/
// void PORE::getRestrictingDiameters(int nSegments, vector<int> vorNetID,
// vector< vector<double> > *PLDtable){
void PORE::getRestrictingDiameters(
    int nSegments, vector<int> vorNetID, vector<vector<double> > *PLDtable,
    vector<vector<pair<int, int> > > *PLDEdgeTable, vector<double> *segmentDi,
    vector<int> *segmentDiNodeID, vector<double> *segmentDiFinal,
    vector<int> *segmentDiFinalNodeID) {
  // Define three access types
  const int ACCESSED = 1;
  const int UNKNOWN = -1;
  ...
  if (nSegments < 1) {
    cerr << "Number of semgents lower than 1. This function should not be "
            "called.\n"
         << "\n";
    abort();
  };
  ...
  vector<int> PoreNodesSegmentIDs;  // will store segment ID for each node in
                                    // the current PORE
  ...
  cout << "Current PORE contains " << nSegPresent
       << " nodes in predefined segments. Proceeding with Restricting Diameter "
          "Calcs\n";

  // Start analysis by accepting all nodes already assigned to segments (segment
  // seed nodes) Store information about largest node pre-assigned to each
  // segment
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) {
    if (PoreNodesSegmentIDs[i] > -1) {
      ...
      if (segmentDi->at(PoreNodesSegmentIDs[i]) < nodes.at(i).max_radius * 2) {
        ...
        segmentDiNodeID->at(PoreNodesSegmentIDs[i]) =
            reverseIDMappings.find(i)->second;
        segmentDiFinal->at(PoreNodesSegmentIDs[i]) =
            segmentDi->at(PoreNodesSegmentIDs[i]);
      };
    };
  };

  /* BUG BUG line (1ln) below, it probably clears segmentDiFinal too many times,
   * moving to the above loop */
  ...
  for (unsigned int n = 0; n < nodes.size(); n++) {
    if (accessStatuses[n] == ACCESSED) {
      for (unsigned int i = 0; i < lnodes[n].connections.size(); i++) {
        if (lnodes[n].connections.at(i).max_radius >
            lnodes.at(lnodes[n].connections.at(i).to).max_radius)
          lnodes[n].connections.at(i).max_radius =
              lnodes.at(lnodes[n].connections.at(i).to).max_radius;
  ...
  while (stack.size() != 0) {
    ...
    if (accessStatuses[best_node] == UNKNOWN) {
      // if the node (best_node) is not assigned to segment, assign it to the
      // current segment (determined by node best.first) check new connections,
      // add to heap or terminane
      ...
      if (nodes.at(best_node).max_radius * 2 >
          segmentDiFinal->at(PoreNodesSegmentIDs[best.first])) {
        segmentDiFinal->at(PoreNodesSegmentIDs[best.first]) =
            nodes.at(best_node).max_radius * 2;
        segmentDiFinalNodeID->at(PoreNodesSegmentIDs[best.first]) =
            reverseIDMappings.find(best_node)->second;
      };
      ...
      //            stack.reHeapify();
      //            I think reHeapify is not needed as heap.insert ensures
      //            proper sorting
    }

    else if (accessStatuses[best_node] == ACCESSED) {
      // selected connection leads to a node that is already assigned to a
      // segment
      ...
      if (PoreNodesSegmentIDs[best_node] != PoreNodesSegmentIDs[best.first]) {
        // record the largest resticting diameter between two segments of the
        // same PORE
        double r = lnodes[best.first].connections.at(best.second).max_radius;

        // begin debug
        // cout << "PLD table size: " << PLDtable->size() << "   " <<
        // PLDtable->at(PoreNodesSegmentIDs[best_node]).size() << "\n"; cout <<
        // "PoreNodesSegmentIDs[best_node]= " << PoreNodesSegmentIDs[best_node]
        // << "   PoreNodesSegmentIDs[best.first]= " <<
        // PoreNodesSegmentIDs[best.first] << "\n";
        //              if(PLDtable->size() > PoreNodesSegmentIDs[best_node] + 1
        //              || PLDtable->at(PoreNodesSegmentIDs[best_node]).size() >
        //              PoreNodesSegmentIDs[best.first] + 1)
        //    {cout << "alarm...\n";
        //};
        // end debug
        double stored_d = PLDtable->at(PoreNodesSegmentIDs[best_node])
                              .at(PoreNodesSegmentIDs[best.first]);
        if (2 * r > stored_d) {
          PLDtable->at(PoreNodesSegmentIDs[best_node])
              .at(PoreNodesSegmentIDs[best.first]) = 2 * r;
          PLDtable->at(PoreNodesSegmentIDs[best.first])
              .at(PoreNodesSegmentIDs[best_node]) = 2 * r;
          pair<int, int> p(reverseIDMappings.find(best_node)->second,
                           reverseIDMappings.find(best.first)->second);
          PLDEdgeTable->at(PoreNodesSegmentIDs[best_node])
              .at(PoreNodesSegmentIDs[best.first]) = p;
          PLDEdgeTable->at(PoreNodesSegmentIDs[best.first])
              .at(PoreNodesSegmentIDs[best_node]) = p;
        } else {
          // cerr << "Trying to store smaller diameter than already stored. This
          // shouldnt happen. Check the code\n";
        };
      } else {
  ...
  for (int i = 0; i < nSegments; i++) {
    if (segmentDi->at(i) != segmentDiFinal->at(i))
      cerr << "Segment Di(" << segmentDi->at(i)
           << ") is different than Segment Di Final (" << segmentDiFinal->at(i)
           << ") for segment " << i << ".\n";
  };
