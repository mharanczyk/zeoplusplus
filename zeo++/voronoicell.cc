 ...
 * stores information about the node coordinates. The first definition,
 * VOR_CELL, is used when visualization is important. The other, BASIC_VCELL, is
 * used in surface area/volume calculations to decrease overhead time.
 *
 * The file also contains all of the functions required to visualize an
 * ATOM_NETWORK and VORONOI_NETWORK using the ZeoVis tool.
 */

/* Store the provided vertices and their ids.   */
VOR_FACE::VOR_FACE(vector<Point> vertices, ATOM_NETWORK *atmnet,
...
/* Returns a vector of pairs of Points representing the start and end
 * coordinates of each edge that outlines the face.   */
vector<pair<Point, Point> > VOR_FACE::getEdgeCoords() {
  ...
  results.push_back(pair<Point, Point>(
      orderedVertices[orderedVertices.size() - 1], orderedVertices[0]));
  return results;
}

/* Using the provided ouput stream, write the geometric objects that would fill
 * the Voronoi face when drawn.   */
void VOR_FACE::writeVMDFilled(fstream &output) {
  ...
  while (n3 < orderedVertices.size()) {
    ...
    output << "{triangle {" << p1[0] << " " << p1[1] << " " << p1[2] << "} {"
           << p2[0] << " " << p2[1] << " " << p2[2] << "} {" << p3[0] << " "
           << p3[1] << " " << p3[2] << "} }"
           << "\n";
    n2++;
...
void VOR_CELL::addEdge(Point from, Point to) {
  ...
  if (edgeConnections[iter2->second].find(iter1->second) ==
      edgeConnections[iter2->second].end())
    edgeConnections[iter1->second].insert(iter2->second);
}

/* Add the face to the VOR_CELL. Adds all edges and vertices that have not yet
...
void VOR_CELL::writeVMDOutlined(fstream &output, int n) {
  ...
  for (int i = 0; i < numVertices; i++) {
    ...
    output << "{color $nodeColors(" << nodeID << ") }"
           << "\n";
    output << "{sphere {" << curPoint[0] << " " << curPoint[1] << " "
           << curPoint[2] << "} radius $nodeRadii(" << nodeID
...
void VOR_CELL::getVoronoiCells(vector<VOR_CELL> *cells,
                               vector<vector<VOR_FACE> > faceInfo,
...
int BASIC_VCELL::getNumNodes() { return nodeCoords.size(); }

Point BASIC_VCELL::getNodeCoord(int index) { return nodeCoords[index]; }

int BASIC_VCELL::getNodeID(int index) { return nodeIDs[index]; }

/* Removes the nodes from the cell which would overlap with a sphere center
   around the provided atom with a radius of r_probe + r_atom*/
void BASIC_VCELL::removeOverlappedNodes(int atomID, ATOM_NETWORK *atmnet,
                                        double r_probe) {
  vector<int> newIDs;
  ...
  for (unsigned int i = 0; i < nodeCoords.size(); i++) {
    ...
    if (calcEuclideanDistance(node[0], node[1], node[2], center.x, center.y,
                              center.z) >= (center.radius + r_probe)) {
...
void writeVMDUC(fstream &output, ATOM_NETWORK *atmnet) {
  ...
  DELTA_POS directions[3] = {DELTA_POS(1, 0, 0), DELTA_POS(0, 1, 0),
                             DELTA_POS(0, 0, 1)};
  DELTA_POS limits[3] = {DELTA_POS(0, 1, 1), DELTA_POS(1, 0, 1),
                         DELTA_POS(1, 1, 0)};
  for (unsigned int i = 0; i < 3; i++) {
    ...
    for (int a = 0; a < 2; a++) {
      for (int b = 0; b < 2; b++) {
        for (int c = 0; c < 2; c++) {
          if ((limit.x < a) || (limit.y < b) || (limit.z < c)) continue;
          ...
          double x2 = x1 + v_a.x * direction.x + v_b.x * direction.y +
                      v_c.x * direction.z;
          double y2 = y1 + v_a.y * direction.x + v_b.y * direction.y +
                      v_c.y * direction.z;
          double z2 = z1 + v_a.z * direction.x + v_b.z * direction.y +
                      v_c.z * direction.z;
 ...
 */
void writeVornet(fstream &output, ATOM_NETWORK *atmnet,
                 VORONOI_NETWORK *vornet) {
  ...
  for (unsigned int i = 0; i < vornet->edges.size(); i++) {
    ...
    atmnet->translatePoint(&end, curEdge.delta_uc_x, curEdge.delta_uc_y,
                           curEdge.delta_uc_z);
    output << "{line {" << start[0] << " " << start[1] << " " << start[2]
           << "} "
           << "{" << end[0] << " " << end[1] << " " << end[2] << "}"
           << "}"
           << "\n";
...
/* Writes the commands to the provided output stream necessary to display all
 * atoms and Voronoi nodes in ZeoVis. Labels each atom as atoms(i) and each node
 * as nodes(j).
 */
void writeVMDAtomsAndNodes(fstream &output, ATOM_NETWORK *atmnet,
                           VORONOI_NETWORK *vornet) {
  ...
  // Iterate over all nodes. Set the radius of the node to that of the maximum
  // static sphere
  for (unsigned int i = 0; i < vornet->nodes.size(); i++) {
    VOR_NODE curNode = vornet->nodes.at(i);
    output << "set nodes(" << i << ") {"
 ...
 */
void writeVMDEnvVars(fstream &output, ATOM_NETWORK *atmnet,
                     VORONOI_NETWORK *vornet) {
