...
struct block {
  double dis;
  double e;
  inline void first(double v, double d) {
    e = v > 0 ? v : 0;
    dis = d;
  }
...
class voronoi_network {
 public:
  ...
  void store_network(std::vector<VOR_NODE> &nodes, std::vector<VOR_EDGE> &edges,
  ...
  template <class v_cell>
  inline void add_to_network_rectangular(v_cell &c, int idn, double x, double y,
 ...
 private:
  ...
  bool search_previous(double gx, double gy, double x, double y, double z,
                       int &ijk, int &q, int &ci, int &cj, int &ck);
  bool safe_search_previous_rect(double x, double y, double z, int &ijk, int &q,
                                 int &ci, int &cj, int &ck);
  bool search_previous_rect(double x, double y, double z, int &ijk, int &q,
                            int &ci, int &cj, int &ck);
  template <class v_cell>
  void add_to_network_internal(v_cell &c, int idn, double x, double y, double z,
                               double rad, int *cmap);
  template <class v_cell>
  void add_to_network_rectangular_internal(v_cell &c, int idn, double x,
