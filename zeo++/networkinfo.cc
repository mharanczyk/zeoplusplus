...
/** Initialize Periodi Table with atom names to be used to clean atom names from
 * indexes
 * */
void initializePT() {
...
double lookupRadius(string atomType, bool radial) {
  ...
  if (info == radTable.end()) {
    cerr << "Unable to find radius for " << atomType
         << " in table. Please provide it "
         << "\n"
...
double lookupCovRadius(string atomType) {
  ...
  if (info == covRadTable.end()) {
    cerr << "Unable to find covalent radius for " << atomType
         << " in table. Please modify networkinfo.cc and recomplie the code "
         << "\n"
         << "Exiting ..."
         << "\n";
    exit(1);
...
double lookupMass(string atomType) {
  ...
  if (info == massTable.end()) {
    cerr << "Unable to find molar mass for " << atomType
         << " in table. Please provide it "
         << "\n"
         << "in a reference file or check you input file."
         << "\n"
...
int lookupAtomicNumber(string atomType) {
  ...
  if (info == atomicNumberTable.end()) {
    cerr << "Unable to find atomic number for " << atomType
         << " in table. Please provide it "
         << "\n"
...
bool isMetal(string atomType) {
  ...
  if (info == atomicCharacterTable.end()) {
    cerr << "Unable to find character information for " << atomType
         << " in table. Please modify networkinfo.cc and recomplie the code "
         << "\n"
         << "Exiting ..."
         << "\n";
