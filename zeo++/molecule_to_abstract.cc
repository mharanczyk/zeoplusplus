...
// C code to read an xyz file and convert it to an abstract linker - it can
// handle carboxy or Br sites, with the appropriate flag Arguments:
// real_molecule.xyz site_type output_abstract_molecule.xyz
...
const int CARBOXY = 0, BROMINE = 1, DIALCOHOL = 2, A_P_CATECHOL = 3,
          ALDEHYDE = 4;
...
void replace_sites_with_abstract(vector<ATOM> *molecule, vector<INT3> *indices,
                                 int site_type, string name);
...
int main(int argc, char *argv[]) {
  ...
  replace_sites_with_abstract(&real_molecule, &real_site_indices, site_type,
                              real_mol_name);
...
void replace_sites_with_abstract(vector<ATOM> *molecule, vector<INT3> *indices,
  ...
  if (site_type == CARBOXY) {  // replace the C atom with Q, and remove the O
                               // atoms; also remove any H atoms near to the O
    for (int i = 0; i < num_sites; i++) {
      ...
      for (int j = 0; j < num_atoms; j++) {  // find H atoms bonded to this O
        if (molecule->at(j).type == "H" && molecule->at(j).keep) {
          if (bonded(molecule->at(site.x), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH) ||
              bonded(molecule->at(site.z), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH)) {
  ...
  } else if (site_type == BROMINE) {  // else remove the Br atom and replace it
                                      // with Q positioned so as to be a carbon
                                      // bond length away from the adjacent atom
    if (num_sites != 2) {
      /*
            printf("ERROR: >2 Br sites (%d) in structure %s so cannot align them
         - not writing out\n", num_sites, name.c_str()); exit(EXIT_FAILURE);
      */
      printf(
          "WARNING: >2 Br sites (%d) in structure %s so cannot align them - "
          "will write out abstract molecule as it is\n",
          num_sites, name.c_str());
      for (int i = 0; i < num_sites; i++) {
        INT3 site = indices->at(i);
        molecule->at(site.x).type = "Q";  // replace the Br with Q
        molecule->at(site.x).label = "Q";
        molecule->at(site.x).set_xyz(
            molecule->at(site.y).xyz() +
            get_vector_from_to(molecule->at(site.y).xyz(),
                               molecule->at(site.x).xyz())
                .unit()
                .scale(MAX_CARBOXY_BOND_LENGTH));  // shift the Q
      }
    } else {  // if dibromo, we can also orient them so that the structure has a
              // chance to be built with nonlinear molecules
      ...
      // now, if both Br are anchored to the same atom, we have a problem - we
      // will have to handle it separately:
      if (site_0.y == site_1.y) {
        XYZ site_0_1 = get_vector_from_to(molecule->at(site_0.x).xyz(),
                                          molecule->at(site_1.x).xyz())
                           .unit()
                           .scale(MAX_CARBOXY_BOND_LENGTH);
        molecule->at(site_1.x).set_xyz(molecule->at(site_1.y).xyz() + site_0_1);
        molecule->at(site_0.x).set_xyz(molecule->at(site_0.y).xyz() +
                                       site_0_1.scale(-1));
      } else {
        XYZ anchor_anchor = get_vector_from_to(molecule->at(site_0.y).xyz(),
                                               molecule->at(site_1.y).xyz())
                                .unit()
                                .scale(MAX_CARBOXY_BOND_LENGTH);
        molecule->at(site_1.x).set_xyz(molecule->at(site_1.y).xyz() +
                                       anchor_anchor);
        molecule->at(site_0.x).set_xyz(molecule->at(site_0.y).xyz() +
                                       anchor_anchor.scale(-1));
      }
    }
    ...
    for (int i = 0; i < num_sites; i++) {
      ...
      molecule->at(site.x).label = "Q";
             ...
             A_P_CATECHOL) {  // replace the C (x) atom with Q, and remove the
                              // CH3 groups (y/z and surrounding H)
    for (int i = 0; i < num_sites; i++) {
      INT3 site = indices->at(i);
      ...
      for (int j = 0; j < num_atoms; j++) {  // find H atoms bonded to this O
        if (molecule->at(j).type == "H" && molecule->at(j).keep) {
          if (bonded(molecule->at(site.y), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH) ||
              bonded(molecule->at(site.z), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH)) {
            molecule->at(j).keep = false;
  ...
  } else if (site_type == DIALCOHOL) {  // replace the dummy Z (z) atom with Q,
                                        // and remove the H on y/x
    for (int i = 0; i < num_sites; i++) {
      INT3 site = indices->at(i);
      ...
      for (int j = 0; j < num_atoms; j++) {  // find H atoms bonded to this O
        if (molecule->at(j).type == "H" && molecule->at(j).keep) {
          if (bonded(molecule->at(site.y), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH) ||
              bonded(molecule->at(site.x), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH)) {
            molecule->at(j).keep = false;
          }
        }
...
void find_sites(vector<ATOM> *molecule, vector<INT3> *indices, int site_type) {
  ...
  if (site_type == CARBOXY) {  // CARBOXY SITES
    for (int i = 0; i < num_atoms; i++) {
      if (molecule->at(i).type == "O" &&
          !already_checked.at(
              i)) {  // found a new oxygen; potentially a connection site
        if (verbose) printf("found an O atom\n");
        INT3 trial_site;
        trial_site.x = i;
        already_checked.at(trial_site.x) = true;
        // is this a connection site? we need to find if there is one bond only
        // (carbon - ignore hydrogens), and if that carbon is bonded to an
        // oxygen with only one bond (that carbon - ignore hydrogens)
        bool cannot_be_site_first_O = false;
        int num_non_H_bonds_first_O = 0;
        for (int j = 0; j < num_atoms && !cannot_be_site_first_O;
             j++) {  // find how many non-H atoms are bonded to this O
          if (molecule->at(j).type != "H" && j != trial_site.x) {
            if (bonded(molecule->at(trial_site.x), molecule->at(j),
                       MAX_CARBOXY_BOND_LENGTH)) {
              if (verbose)
                printf("first O is bonded to %s with distance %.3f\n",
                       molecule->at(j).type.c_str(),
                       dist(molecule->at(trial_site.x), molecule->at(j)));
              num_non_H_bonds_first_O++;
              if (num_non_H_bonds_first_O > 1 || molecule->at(j).type != "C")
                cannot_be_site_first_O = true;
              else if (molecule->at(j).type == "C" &&
                       !already_checked.at(
                           j)) {  // this is a potential C from a carboxy
                trial_site.y = j;
              }
            }
          }
        ...
        if (verbose)
          printf("the first O atom has %d non-H bonds (exactly one required)\n",
                 num_non_H_bonds_first_O);
        // at this point, if we did not find too many bonds, this is still a
        // potential site, and we need to find the other oxygen - otherwise,
        // bypass the rest and move on to test the next oxygen
        bool site_completed = false;
        for (int j = 0;
             j < num_atoms && !cannot_be_site_first_O && !site_completed; j++) {
          if (molecule->at(j).type != "H" && j != trial_site.y) {
            if (bonded(molecule->at(trial_site.y), molecule->at(j),
                       MAX_CARBOXY_BOND_LENGTH)) {
              if (molecule->at(j).type == "O" &&
                  !already_checked.at(
                      j)) {  // this is a potential other O from a carboxy
                if (verbose)
                  printf("found a second O atom that is bonded to this C\n");
                trial_site.z = j;
                already_checked.at(trial_site.z) = true;
                bool cannot_be_site_second_O = false;
                int num_non_H_bonds_second_O = 0;
                for (int k = 0; k < num_atoms && !cannot_be_site_second_O;
                     k++) {  // find how many non-H atoms are bonded to this O
                  if (molecule->at(k).type != "H" && k != trial_site.z) {
                    if (bonded(molecule->at(trial_site.z), molecule->at(k),
                               MAX_CARBOXY_BOND_LENGTH)) {
                      if (verbose)
                        printf(
                            "second O is bonded to %s with distance %.3f\n",
                            molecule->at(k).type.c_str(),
                            dist(molecule->at(trial_site.z), molecule->at(k)));
                      num_non_H_bonds_second_O++;
                      if (num_non_H_bonds_second_O > 1 || k != trial_site.y)
                        cannot_be_site_second_O = true;
                    }
                  }
                }
                if (verbose)
                  printf(
                      "the second O atom has %d non-H bonds (exactly one "
                      "required)\n",
                      num_non_H_bonds_second_O);
                if (!cannot_be_site_second_O) {
                  // at this point, if we did not find too many bonds, this is a
                  // site!
                  indices->push_back(trial_site);
                  already_checked.at(trial_site.y) =
                      true;  // now safe to flag this carbon as having been
                             // checked already
                  site_completed = true;
                }
              }
        ...
        if (verbose) {
          if (!site_completed)
            printf("this O atom did not constitute part of a carboxy group\n");
          else
            printf("site found!\n");
        }
      }
    ...
    for (int i = 0; i < num_atoms; i++) {
      if (molecule->at(i).type == "Br" &&
          !already_checked.at(
              i)) {  // found a new bromine; potentially a connection site
        if (verbose) printf("found a Br atom\n");
        bool site_completed = false;
        INT3 trial_site;
        trial_site.x = i;
        trial_site.z = -1;  // this site is only one atom, and the one it is
                            // bonded to (y) - we don't need this third position
                            // - this output will be parsed appropriately later
        already_checked.at(trial_site.x) = true;
        // is this a connection site? we need to find if there is one bond only
        // (doesn't matter what element)
        bool cannot_be_site_Br = false;
        int num_bonds_Br = 0;
        for (int j = 0; j < num_atoms && !cannot_be_site_Br;
             j++) {  // find how many non-H atoms are bonded to this Br
          if (molecule->at(j).type != "H" && j != trial_site.x) {
            if (bonded(molecule->at(trial_site.x), molecule->at(j),
                       MAX_BROMINE_BOND_LENGTH)) {
              if (verbose)
                printf("this Br is bonded to %s with distance %.3f\n",
                       molecule->at(j).type.c_str(),
                       dist(molecule->at(trial_site.x), molecule->at(j)));
              num_bonds_Br++;
              trial_site.y = j;
        ...
        if (verbose)
          printf("the Br atom has %d bonds (exactly one required)\n",
                 num_bonds_Br);
        ...
        if (verbose) {
          if (!site_completed)
            printf("this Br atom did not constitute a connection site\n");
          else
            printf("site found!\n");
        }
      }
    ...
    for (int i = 0; i < num_atoms; i++) {
      if (molecule->at(i).type == "O" &&
        ...
        already_checked.at(trial_site.x) = true;
        // is this a connection site? we need to find if there is one bond only
        // (carbon - CHECK FOR hydrogens), and if that carbon is bonded to a
        // hydrogen
        bool cannot_be_site_first_O = false;
        int num_bonds_first_O = 0;
        for (int j = 0; j < num_atoms && !cannot_be_site_first_O;
             j++) {  // find how many atoms are bonded to this O
          if ((molecule->at(j).type != "H" && j != trial_site.x &&
               bonded(molecule->at(trial_site.x), molecule->at(j),
                      MAX_CARBOXY_BOND_LENGTH)) ||
              (molecule->at(j).type == "H" && j != trial_site.x &&
               bonded(molecule->at(trial_site.x), molecule->at(j),
                      MAX_HYDROGEN_BOND_LENGTH))) {
            if (verbose)
              printf("first O is bonded to %s with distance %.3f\n",
                     ...
                     dist(molecule->at(trial_site.x), molecule->at(j)));
            num_bonds_first_O++;
            if (num_bonds_first_O != 1)
              cannot_be_site_first_O = true;
            else if (molecule->at(j).type == "C" &&
                     !already_checked.at(
                         j)) {  // this is a potential C from an aldehyde
              trial_site.y = j;
            }
          }
        }
        if (verbose)
          printf("the first O atom has %d bonds (exactly one required)\n",
                 num_bonds_first_O);
        // at this point, if we did not find too many bonds, this is still a
        // potential site, and we need to find the hydrogen AND EXACTLY ONE
        // OTHER
        bool site_completed = false;
        ...
        for (int j = 0;
             j < num_atoms && !cannot_be_site_first_O && !site_completed; j++) {
          if (molecule->at(j).type == "H" && !already_checked.at(j) &&
              j != trial_site.x && j != trial_site.y &&
              bonded(molecule->at(trial_site.y), molecule->at(j),
                     MAX_HYDROGEN_BOND_LENGTH)) {
            // this is a potential H to complete the aldehyde
            if (verbose) printf("found an H atom that is bonded to this C\n");
            num_H++;
            id_H = j;
          } else if (molecule->at(j).type != "H" && j != trial_site.x &&
                     j != trial_site.y &&
                     bonded(molecule->at(trial_site.y), molecule->at(j),
                            MAX_CARBOXY_BOND_LENGTH)) {
            // this is a potential non-H to complete the aldehyde
            if (verbose)
              printf("found a non-H atom that is bonded to this C\n");
            num_non_H++;
          }
        }
        ...
        if (num_H == 1 && num_non_H == 1) {
          ...
          already_checked.at(trial_site.y) =
              true;  // now safe to flag this carbon as having been checked
                     // already
          indices->push_back(trial_site);
          site_completed = true;
        }
        if (verbose) {
          if (!site_completed)
            printf(
                "this O atom did not constitute part of an aldehyde group\n");
          else
            printf("site found!\n");
        }
      }
    ...
    for (int i = 0; i < num_atoms; i++) {
      if (molecule->at(i).type == "C" &&
          !already_checked.at(
              i)) {  // found a new carbon; potentially a connection site
        if (verbose) printf("found a C atom\n");
        INT3 trial_site;
        trial_site.x = i;
        already_checked.at(trial_site.x) = true;
        // is this a connection site? we need to find if there are four bonds:
        // two to CH3 and two to OC
        bool cannot_be_site_first_C = false;
        int num_non_H_bonds_first_C = 0;
        vector<int> bonded_ids;
        for (int j = 0; j < num_atoms && !cannot_be_site_first_C;
             j++) {  // find how many non-H atoms are bonded to this O
          if (molecule->at(j).type != "H" && j != trial_site.x &&
              bonded(molecule->at(trial_site.x), molecule->at(j),
                     MAX_CARBOXY_BOND_LENGTH)) {
            if (verbose)
              printf("first C (%d) is bonded to %s (%d) with distance %.3f\n",
                     trial_site.x, molecule->at(j).type.c_str(), j,
                     dist(molecule->at(trial_site.x), molecule->at(j)));
            num_non_H_bonds_first_C++;
            bonded_ids.push_back(j);
          }
        }
        if (num_non_H_bonds_first_C != 4)
          ...
          for (int j = 0; j < num_non_H_bonds_first_C; j++) {
            ...
            if (molecule->at(id).type == "O" &&
                (!already_checked.at(id))) {  // test for OC
              int num_other_bonds = 0;
              int other_bond_id = -1;
              for (int k = 0; k < num_atoms; k++) {
                if ((k != id && k != trial_site.x &&
                     molecule->at(k).type == "H" &&
                     bonded(molecule->at(id), molecule->at(k),
                            MAX_HYDROGEN_BOND_LENGTH)) ||
                    (k != id && k != trial_site.x &&
                     molecule->at(k).type != "H" &&
                     bonded(molecule->at(id), molecule->at(k),
                  ...
                  if (verbose)
                    printf(
                        "an O on this first C is bonded to %s with distance "
                        "%.3f\n",
                        molecule->at(k).type.c_str(),
                        dist(molecule->at(id), molecule->at(k)));
                  other_bond_id = k;
                  num_other_bonds++;
                }
              }
              if (num_other_bonds != 1)
                fail = true;
              else {
                OC_C_ids.push_back(other_bond_id);
                num_bonds_to_OC_first_C++;
              }
              ...
              for (int k = 0; k < num_atoms; k++) {
                if (k != id && k != trial_site.x &&
                  ...
                  if (verbose)
                    printf(
                        "a C on this first C is bonded to %s with distance "
                        "%.3f\n",
                        molecule->at(k).type.c_str(),
                        dist(molecule->at(id), molecule->at(k)));
                  num_H++;
              ...
              if (num_H != 3)
                ...
                bonded_CH3_ids.push_back(bonded_ids.at(j));
                num_bonds_to_CH3_first_C++;
              }
            }
          }
          if (num_bonds_to_OC_first_C != 2 || num_bonds_to_CH3_first_C != 2 ||
              fail) {
            printf(
                "NOTE: this first C fails here with %d bonds to OC and %d "
                "bonds to CH3 groups\n",
                num_bonds_to_OC_first_C, num_bonds_to_CH3_first_C);
            cannot_be_site_first_C = true;
          } else if (!bonded(molecule->at(OC_C_ids.at(0)),
                             molecule->at(OC_C_ids.at(1)),
                             MAX_CARBOXY_BOND_LENGTH))
            cannot_be_site_first_C = true;
        ...
        if (verbose) {
          if (cannot_be_site_first_C)
            printf(
                "this C atom did not constitute part of an a_p_catechol "
                "group\n");
          else
            printf("site found!\n");
        }
    ...
    for (int i = 0; i < num_atoms; i++) {
      if (molecule->at(i).type == "O" &&
        ...
        trial_site.x = i;
        already_checked.at(trial_site.x) = true;
        // is this a connection site? we need to find if it is an OH, bonded to
        // a C, which is bonded to another C-O-H
        bool cannot_be_site_first_O = false;
        ...
        for (int j = 0; j < num_atoms && !cannot_be_site_first_O;
             j++) {  // find bonds
          if ((!already_checked.at(j)) && molecule->at(j).type == "C" &&
              j != trial_site.x &&
              bonded(molecule->at(trial_site.x), molecule->at(j),
                     MAX_CARBOXY_BOND_LENGTH)) {
            if (verbose)
            ...
            num_C_bonds_first_O++;
            first_C_id = j;
          } else if ((!already_checked.at(j)) && molecule->at(j).type == "H" &&
                     j != trial_site.x &&
                     bonded(molecule->at(trial_site.x), molecule->at(j),
                            MAX_HYDROGEN_BOND_LENGTH)) {
            if (verbose)
        ...
        if (num_C_bonds_first_O != 1 || num_H_bonds_first_O != 1)
          cannot_be_site_first_O = true;
        else {  // we know this is an H-O-C, but does the C connect us to
                // another C-O-H? check bonding environment of first C
          already_checked.at(first_C_id) =
              true;  // we can flag this first C as being considered, since no
                     // other C-O-H group will contain it
          vector<int> second_C_id_possibilities;
          for (int j = 0; j < num_atoms && !cannot_be_site_first_O;
               j++) {  // find bonds
            if ((!already_checked.at(j)) && molecule->at(j).type == "C" &&
                j != first_C_id &&
                bonded(molecule->at(first_C_id), molecule->at(j),
                       MAX_CARBOXY_BOND_LENGTH)) {
              if (verbose)
                printf("first C is bonded to %s with distance %.3f\n",
                       molecule->at(j).type.c_str(),
                       dist(molecule->at(first_C_id), molecule->at(j)));
              second_C_id_possibilities.push_back(j);
            }
          }
          if (second_C_id_possibilities.size() < 1)
            cannot_be_site_first_O = true;
          else {  // there are some second C choices to consider
            bool site_found = false;  // allow to quit these loops early as soon
                                      // as a site is found
            for (int j = 0; j < second_C_id_possibilities.size() && !site_found;
                 j++) {
              // this second C must be bonded to an OH
              int candidate_second_C_id = second_C_id_possibilities.at(j);
              for (int k = 0;
                   k < num_atoms && !cannot_be_site_first_O && !site_found;
                   k++) {  // find bonds
                if ((!already_checked.at(k)) && molecule->at(k).type == "O" &&
                    bonded(molecule->at(candidate_second_C_id), molecule->at(k),
                           MAX_CARBOXY_BOND_LENGTH)) {
                  if (verbose)
                    printf(
                        "candidate second C is bonded to %s with distance "
                        "%.3f\n",
                        molecule->at(k).type.c_str(),
                        dist(molecule->at(candidate_second_C_id),
                             molecule->at(k)));
                  // if this k O is an OH, we are good to go
                  int num_H = 0;
                  for (int l = 0;
                       l < num_atoms && !cannot_be_site_first_O && !site_found;
                       l++) {  // find bonds
                    if (molecule->at(l).type == "H" &&
                        bonded(molecule->at(k), molecule->at(l),
                               MAX_HYDROGEN_BOND_LENGTH)) {
                      num_H++;
                      if (verbose)
                        printf(
                            "candidate second C's O is bonded to %s with "
                            "distance %.3f\n",
                            molecule->at(l).type.c_str(),
                            dist(molecule->at(l), molecule->at(k)));
                    }
                  }
                  if (num_H == 1) {  // satisfied that this is a site
                    trial_site.y = k;
                    // we need to find where the z site is, because it is not an
                    // atom! first find the centroid of site.x, site.y and the
                    // two carbons, first_C_id and candidate_second_C_id
                    XYZ site_centroid =
                        (molecule->at(candidate_second_C_id).xyz() +
                         molecule->at(first_C_id).xyz() +
                         molecule->at(trial_site.y).xyz() +
                         molecule->at(trial_site.x).xyz())
                            .scale(0.25);
                    double atom_centroid_distance =
                        dist(site_centroid.x, site_centroid.y, site_centroid.z,
                             molecule->at(trial_site.x));
                    XYZ projection_of_centroid = project_onto_line(
                        site_centroid, molecule->at(trial_site.x).xyz(),
                        molecule->at(trial_site.y).xyz());
                    XYZ new_atom_vector =
                        (projection_of_centroid - site_centroid).unit();
                    XYZ new_atom_pos =
                        site_centroid +
                        (new_atom_vector.scale(atom_centroid_distance));
        ...
        if (verbose) {
          if (cannot_be_site_first_O)
            printf(
                "this O atom did not constitute part of a dialcohol group\n");
          else
            printf("site found!\n");
        }
      }
...
void read_molecule(string filename, vector<ATOM> *atom_vector) {
  // this function inspects the filename, and decides which read function to
  // call - for xyz or incon format
  string extension_start = ".";
  unsigned pos = filename.rfind(extension_start);
  if (pos == -1) {
    printf(
        "ERROR: could not identify extension for the input file named \"%s\"\n",
        filename.c_str());
    exit(EXIT_FAILURE);
  }
  ...
  if (pos2 != -1) {
    ...
    if (pos3 != -1) {
      ...
      printf(
          "ERROR: could not parse either \"%s\" or \"%s\" in the extension for "
          "the input file named \"%s\"\n",
...
void read_incon(string filename, vector<ATOM> *atom_vector) {
  ...
  while (fgets(ch1, MAX_CHAR_ARRAY_LENGTH, input) != NULL &&
         !done_reading_atoms) {
    //    vector<string> token;
    //    token = split(ch1," \r\t");
    double x = 0, y = 0, z = 0;
    ...
    //    if(token.size()==5) { //a line comprising an atom name, a string, and
    //    three coords
    if (strcmp(atom_line_second_column, dummy) ==
        0) {  // line contains an atom in incon format!
      started_reading_atoms = true;
      ATOM a;
      //      char *name = new char[MAX_CHAR_ARRAY_LENGTH];
      //      char *dummy = new char[MAX_CHAR_ARRAY_LENGTH];
      //      status = sscanf(ch1, "%s %s %lf %lf %lf", name, dummy, &a.x, &a.y,
      //      &a.z);
      a.x = x;
      a.y = y;
      a.z = z;
      ...
      if (digit_pos == 0) {  // first character was a digit - this is no good
        if (started_reading_atoms)
          done_reading_atoms = true;  // if we have already started parsing atom
                                      // data, and encounter a line without a
                                      // valid atom name, then we should stop
      } else {
        if (digit_pos > 0) {
        ...
        if (verbose)
          printf("atom %s assigned radius %.3f\n", a.type.c_str(), a.radius);
        delete[] name;
        delete[] dummy;
      ...
      // num_tokens, ch1); printf("DEBUG: line did not have %s as the second
      // component: %s\n", atom_line_second_column, ch1);
      if (started_reading_atoms)
        done_reading_atoms =
            true;  // if we have already started parsing atom data, and
                   // encounter a line without atom data, then we should stop
    }
  }
  if (!done_reading_atoms) {
    printf(
        "ERROR: finished reading data but did not finishing parsing atom info "
        "correctly - managed to read %d atoms\n",
    ...
    if (fgets(ch, MAX_CHAR_ARRAY_LENGTH, input) != NULL) {
      ...
      if (digit_pos == 0) {  // first character was a digit - this is no good
        ...
        if (digit_pos > 0) {
          ...
          a.type = name_string;
        // printf("DEBUG: with name_string=%s, digit_pos=%d, therefore
...
void search_for_char(FILE *f, char target) {
        ...
        if(c==EOF) {
                printf("ERROR: The required character (%c, with int value %d)
was not found in this file.\n", target, (int)target); exit(EXIT_FAILURE);
        }
}
*/
bool bonded(ATOM a, ATOM b, double d) {
...
double get_rad(string s) {  // CCDC radii
  if (s == "H")
    ...
    printf(
        "WARNING: could not find radius for element type %s - using default of "
        "%.3f\n",
...
public:
  ...
  string name;
  //constructor
  ATOM(double x0, double y0, double z0, string s0, double r0); //separate
Cartesian values constructor ATOM(XYZ xyz0, string s0, double r0); //XYZ
constructor ATOM();
};
*/
...
double XYZ::angle_between(XYZ other) {
  ...
  if(cos_angle>1) {
    ...
    cos_angle = -1; //these steps are necessary for rounding issues when
cos_angle should be exactly 1 or -1, but falls very slightly on the wrong side
  }
  double output = acos(cos_angle);
  if(isnan(output)) {
    return 0;
  } else return output; //this step is necessary in the case of identity, which
produces NAN
}
...
XYZ project_onto_line(XYZ initial, XYZ line_start, XYZ line_end) {
  ...
  double movement_along_line =
(-1*initial_to_line_start_vector.dot_product(line_vector))/(line_length*line_length);
