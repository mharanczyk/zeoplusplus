...
//-----Program to read a list of hologram files and output n diverse structures,
// using the first structure as the 'seed'
...
main(int argc, char *argv[]) {
  ...
  if (argc != needed_args && argc != needed_args + optional_args) {
    printf(
        "Number of command line arguments should be %d. Please try again, "
        "e.g.:\n",
        needed_args);
    printf(
        "./a.out list_of_all_holos_files num_structures "
        "diversity_output_filename num_desired max_similarity DIFFUSION_RANGE "
        "SIMILARITY_MEASURE\n\t(optional args: run_index num_runs, for "
        "distributed seeding)\n");
    printf(
        "Program will identify diverse structures until either num_desired are "
        "chosen, or max_similarity is exceeded; give either criterion a "
        "negative value to ignore it\n");
  ...
  printf(
      "Program will identify diverse structures until either %d are chosen, or "
      "%.6f similarity is exceeded (if either value is <0, this condition will "
      "not be considered)\n",
  ...
  if (argc == needed_args + optional_args) {
    ...
    if (num_runs <= 0) {
      printf(
          "ERROR: num_runs should be positive, but has value %d. Please try "
          "again.\n",
          num_runs);
      exit(EXIT_FAILURE);
    }
    if (run_index >= num_runs || run_index < 0) {
      printf(
          "ERROR: run_index should be in range 0 to num_runs-1, i.e. 0 to %d, "
          "but has value %d. Please try again.\n",
    ...
    printf("Using first structure as seed\n");
  ...
  for (i = 0; i < num_structures; i++) {
    ...
    if (status == -1) {
      printf(
          "ERROR: could not read file name for structure index %d. Please try "
          "again.\n",
          i);
      exit(EXIT_FAILURE);
    }
    ...
    status = 0;
    while (status != -1) {
      status = fscanf(holo, "%d %d %d %d", &grid_point.x, &grid_point.y,
                      &grid_point.z, &quantity);
      if (status != -1) {
  ...
  // need a big array for each of the two structures being compared at any
  // moment; fill with values incorporating diffusion based on nearby entries
  ...
  int num_selected = 0;
  tuple t = {seed, -1};
  indices_of_selected.push_back(
      t);  // i.e. 'seed' the diversity selection with either the first valid
           // structure in the list (index 0), or the distributed seed index
  ...
  while ((num_selected < num_desired || num_desired < 0) &&
         (exceeded_max_similarity == 0 || max_similarity < 0)) {
    similarities[num_selected - 1] = new float[num_structures];
    for (j = 0; j < num_structures; j++) {
      similarities[num_selected - 1][j] = -1;
    }
    // 1) calculate the similarities between the most-recently selected
    // structure and all others (keeping track of previously calculated
    // similarities)
    for (j = 0; j < num_structures; j++) {
      if (is_valid[j]) {
        for (k = 0; k < NUM_BINS; k++) {
        ...
        float similarity = get_similarity(array_i, array_j, NUM_BINS,
                                          DIFFUSION_RANGE, SIMILARITY_MEASURE);
        similarities[num_selected - 1][j] = similarity;
      } else
        similarities[num_selected - 1][j] = -1;
    }
    // 2) find the structure which has the lowest maximum similarity w.r.t.
    // previously selected structures
    ...
    for (j = 0; j < num_structures; j++) {
      ...
      for (i = 0; i < num_selected; i++) {
        if ((similarities[i][j] > max_sim || max_sim < 0) &&
            similarities[i][j] >= 0) {
          max_sim = similarities[i][j];
          temp_comparator = i;
        }
      }
      if ((max_sim < current_lowest_max_sim || current_lowest_max_sim < 0) &&
          max_sim >= 0) {
      ...
      } else if (max_sim <= current_lowest_max_sim && num_candidates > 0 &&
                 max_sim >=
                     0) {  // tie-break scenario, where at least one
                           // structure has already been selected as valid
        num_candidates++;
        tuple t = {j, temp_comparator};
        candidates.push_back(t);
      }
    ...
    if (current_lowest_max_sim > max_similarity && !(max_similarity < 0)) {
      ...
      printf(
          "Not proceeding because max_similarity has been exceeded "
          "(%.6f>%.6f)\n",
          current_lowest_max_sim, max_similarity);
    } else {
      // 2b) in the case of a tie, we don't modify the similarities but instead
      // work with a different array of similarities
      ...
      while (num_candidates > 1) {
        ...
        num_tiebreaks++;
        // at this point rather than just the earliest, lowest value, we have a
        // vector of all structures with the same lowest value (tie-break by
        // increasing diffusion)
        if (DIFFUSION_RANGE + num_tiebreaks > NUM_BINS / 2) {
          // nothing changes from here onwards - if the structures are still
          // tied they are likely similar - it is easier for now to just run to
          // this point and choose the alphabetically first structure
          num_candidates = 1;
          printf(
              "%d structures all have max similarity of %.6f to any selected "
              "structure; force-ending tiebreak because increasing diffusion "
              "will have no effect\n",
              num_candidates, current_lowest_max_sim);
        } else {
          printf(
              "%d structures all have max similarity of %.6f to any selected "
              "structure; tiebreak based on increasing diffusion to %d and "
              "recalculating similarities:\n",
              num_candidates, current_lowest_max_sim,
              DIFFUSION_RANGE + num_tiebreaks);

          for (i = 0; i < num_selected;
               i++) {  // for each selected structure, remake the array based in
                       // more diffusion
            for (k = 0; k < NUM_BINS; k++) {
              for (l = 0; l < NUM_BINS; l++) {
                for (m = 0; m < NUM_BINS; m++) {
            ...
            build_array(array_i, hologram_array[i], NUM_BINS,
                        DIFFUSION_RANGE + num_tiebreaks);
            int n;
            for (n = 0; n < num_candidates; n++) {
              ...
              if (is_valid[j]) {
                ...
                build_array(array_j, hologram_array[j], NUM_BINS,
                            DIFFUSION_RANGE + num_tiebreaks);
                float similarity =
                    get_similarity(array_i, array_j, NUM_BINS, DIFFUSION_RANGE,
                                   SIMILARITY_MEASURE);
                tiebreak_similarities[i][j] =
                    similarity;  // write to [i][j] so we keep track of j
              } else
                tiebreak_similarities[i][j] = -1;
            }
          ...
          for (n = 0; n < num_candidates; n++) {
            ...
            float max_sim = -1;
            for (i = 0; i < num_selected; i++) {
              if ((tiebreak_similarities[i][j] > max_sim || max_sim < 0) &&
                  tiebreak_similarities[i][j] >= 0)
                max_sim = tiebreak_similarities[i][j];
            }
            printf("\tstructure %d (%s) has tiebreak similarity of %.6f\n", j,
                   file_locations[j], max_sim);
            if ((max_sim < tiebreak_current_lowest_max_sim ||
                 tiebreak_current_lowest_max_sim < 0) &&
                max_sim >= 0) {
            ...
            } else if (max_sim <= tiebreak_current_lowest_max_sim &&
                       tiebreak_num_candidates > 0 &&
                       max_sim >=
                           0) {  // tie-break scenario, where at least one
                                 // structure has already been selected as valid
      ...
      if (num_candidates <= 0) {
        printf(
            "ERROR: could not select the structure with the lowest maximum "
            "similarity (probably because only invalid structures remain)\n");
        exit(EXIT_FAILURE);
      } else {  // else == 1
      ...
      printf("Adding structure index %d, %s, similarity %.6f\n",
             current_selection, file_locations[current_selection],
             current_lowest_max_sim);
      num_selected++;
      printf("Number of structures selected so far = %d\n", num_selected);
      ...
      build_array(array_i, hologram_array[current_selection], NUM_BINS,
                  DIFFUSION_RANGE);
    }  // end else (haven't exceeded max_similarity)
  }
  if (num_desired != num_selected && exceeded_max_similarity == 0) {
    printf(
        "ERROR: finished loop without exceeding %.6f similarity, having "
        "selected %d structures, where %d were desired\n",
  ...
  for (i = 0; i < num_selected; i++) {
    if (indices_of_selected.at(i).comparator == -1) {
      ...
      fprintf(out, "%s %.6f %s\n",
              file_locations[indices_of_selected.at(i).candidate],
              lowest_max_sims[i],
              file_locations[indices_of_selected
                                 .at(indices_of_selected.at(i).comparator)
                                 .candidate]);
    }
  }
  fclose(out);
