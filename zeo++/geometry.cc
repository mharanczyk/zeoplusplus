...
bool increasing_magnitude(XYZ i, XYZ j) {
  return (i.magnitude() < j.magnitude());
}

/* Translate the coordinate by unit cell increments so that it lies within the 0
 * to 1 range.*/
...
XYZ trans_to_origuc(XYZ xyz) {
  return XYZ(trans_to_origuc(xyz.x), trans_to_origuc(xyz.y),
             trans_to_origuc(xyz.z));
}
...
double calcDeterminant(double matrix[3][3]) {
  return matrix[0][0] *
         ...
         matrix[2][0] *
             (matrix[1][2] * matrix[0][1] - matrix[1][1] * matrix[0][2]);
}
...
void invertMatrix(double matrix[3][3], double newMatrix[3][3]) {
  ...
  newMatrix[0][0] =
      invDet * (matrix[2][2] * matrix[1][1] - matrix[2][1] * matrix[1][2]);
  newMatrix[0][1] =
      invDet * -1 * (matrix[2][2] * matrix[0][1] - matrix[2][1] * matrix[0][2]);
  newMatrix[0][2] =
      invDet * (matrix[1][2] * matrix[0][1] - matrix[1][1] * matrix[0][2]);
  newMatrix[1][0] =
      invDet * -1 * (matrix[2][2] * matrix[1][0] - matrix[2][0] * matrix[1][2]);
  newMatrix[1][1] =
      invDet * (matrix[2][2] * matrix[0][0] - matrix[2][0] * matrix[0][2]);
  newMatrix[1][2] =
      invDet * -1 * (matrix[1][2] * matrix[0][0] - matrix[1][0] * matrix[0][2]);
  newMatrix[2][0] =
      invDet * (matrix[2][1] * matrix[1][0] - matrix[2][0] * matrix[1][1]);
  newMatrix[2][1] =
      invDet * -1 * (matrix[2][1] * matrix[0][0] - matrix[2][0] * matrix[0][1]);
  newMatrix[2][2] =
      invDet * (matrix[1][1] * matrix[0][0] - matrix[1][0] * matrix[0][1]);
}
...
const bool Point::equals(Point other) const {
  ...
  return abs(vals[0] - other[0]) < tol && abs(vals[1] - other[1]) < tol &&
         abs(vals[2] - other[2]) < tol;
}
...
const Point Point::unit() const {
  return Point(vals[0] / magnitude(), vals[1] / magnitude(),
               vals[2] / magnitude());
}
const Point Point::cross(Point other) const {
  return Point(vals[1] * other[2] - vals[2] * other[1],
...
ostream& operator<<(ostream& out, Point& obj) { obj.print(out); }
...
const double XYZ::magnitude() const { return sqrt(x * x + y * y + z * z); }
...
const XYZ XYZ::cross(const XYZ& other) const {
  return XYZ(y * other.z - z * other.y, z * other.x - x * other.z,
             x * other.y - y * other.x);
}
const double XYZ::dot_product(const XYZ& other) const {
  return x * other.x + y * other.y + z * other.z;
}
// double XYZ::angle_between(XYZ other) {const XYZ other_const = other; return
// angle_between(other_const);}
double XYZ::angle_between(const XYZ& other) const {
  ...
  if (isnan(output)) {
    ...
    return output;  // this step is necessary in the case of identity, which
                    // produces NAN
}
...
XYZ project_onto_line(XYZ initial, XYZ line_start, XYZ line_end) {
  ...
  double movement_along_line =
      (-1 * initial_to_line_start_vector.dot_product(line_vector)) /
...
XYZ project_onto_plane(XYZ initial, XYZ line_start, XYZ line_end, XYZ third) {
  ...
  XYZ projection_third_to_projection_1_vector =
      get_vector_from_to(projection_third, projection_1);
...
/** Convert coordinates relative to the unit cell (a,b,c) to absolute cartesian
    coordinates (x,y,z). */
Point abc_to_xyz(double a, double b, double c, const XYZ& v_a, const XYZ& v_b,
                 const XYZ& v_c) {
...
double calcEuclideanDistance(double x1, double y1, double z1, double x2,
                             double y2, double z2) {
  double dx = x1 - x2;
  double dy = y1 - y2;
  double dz = z1 - z2;
...
/** Calculates the number of intersections between the line that travels through
 * Points p1 and p2 and the sphere with the provided characterstics. Returns a
 * pair made up of the number of intersections and a vector containing the
 * intersection points. Refer to
 * http://local.wasp.uwa.edu.au/~pbourke/geometry/sphereline for more
 * information.
 */
pair<int, vector<Point> > findLineIntersections(Point p1, Point p2,
                                                Point circCenter, double rad) {
  double tolerance =
      0.00001;  // Tolerance for determining whether two numbers are equal

  double a = (p2[0] - p1[0]) * (p2[0] - p1[0]) +
             (p2[1] - p1[1]) * (p2[1] - p1[1]) +
             (p2[2] - p1[2]) * (p2[2] - p1[2]);
  double b = 2.0 * ((p2[0] - p1[0]) * (p1[0] - circCenter[0]) +
                    (p2[1] - p1[1]) * (p1[1] - circCenter[1]) +
                    (p2[2] - p1[2]) * (p1[2] - circCenter[2]));
  double c = circCenter[0] * circCenter[0] + circCenter[1] * circCenter[1] +
             circCenter[2] * circCenter[2] + p1[0] * p1[0] + p1[1] * p1[1] +
             p1[2] * p1[2] -
             2.0 * (circCenter[0] * p1[0] + circCenter[1] * p1[1] +
                    circCenter[2] * p1[2]) -
             rad * rad;
  ...
  if (abs(discriminant) < tolerance) {
    // Discriminant is zero. Line is tangent and therefore only has one
    // intersection
    numResults = 1;
    double u = (-b / (2.0 * a));
...
pair<int, vector<Point> > findLineSegIntersections(Point p1, Point p2,
                                                   Point circCenter,
                                                   double rad) {
  pair<int, vector<Point> > prelimResults =
      findLineIntersections(p1, p2, circCenter, rad);
  ...
  if (prelimResults.first == 0)
    ...
    for (int i = 0; i < prelimResults.first; i++) {
      ...
      if ((p1Dist <= lineSegLength + tolerance) &&
          (p2Dist <= lineSegLength + tolerance)) {
 ...
 * between the two provided points. Refer to
...
double calcSphereDistance(Point p1, Point p2, double circRad) {
  ...
  double phi1 = coords1.first;
  double phi2 = coords2.first;
  double lambda1 = coords1.second;
  double lambda2 = coords2.second;
  double deltaLambda = lambda1 - lambda2;

  double deltaSigma =
      atan((sqrt(pow(cos(phi1) * sin(deltaLambda), 2.0) +
                 pow(cos(phi2) * sin(phi1) -
                         sin(phi2) * cos(phi1) * cos(deltaLambda),
                     2))) /
           (sin(phi1) * sin(phi2) + cos(phi1) * cos(phi2) * cos(deltaLambda)));
  return circRad * deltaSigma;
}

/* Returns the projection of the provided Point onto the circle with the
...
Point projectPointOnSphere(Point p, double circRad, Point circCenter) {
  ...
  double factor = sqrt((circRad * circRad) /
                       (deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ));
  return Point(factor * deltaX + circCenter[0], factor * deltaY + circCenter[1],
               factor * deltaZ + circCenter[2]);
}

/* Returns a pair of doubles representing the longitude and latitude of the
 * given (x,y,z) point. Refer to
...
double distToPlane(Point pnt, Point p, Point normal) {
  if ((normal * (pnt - p)) < 0) return -(normal * (pnt - p));
