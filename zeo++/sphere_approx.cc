...
#define LARGE_RATIO \
  1.3  // ratio between radii with triggers high accuract setting (replacement
       // of an atom with a large cluster)

using namespace std;

// Constructor sets parameters for the small atom that is the center for the
// cluster This will be the origin for all translations
...
/* function that updates information about replaced atoms (small particle
   ...
   coordiantes) and copied to a new ATOM vector provided by a pointer */
void AtomCluster::copyReplacementAtoms(ATOM_NETWORK *atmnet, int atomID,
...
// Based on specified 3-letter code, generate vector of atoms for sphere cluster
// approximation This is the main function that calls all successive functions
void AtomCluster::replaceAtomByCluster(string cluster_type,
                                       double large_atom_radius) {
...
ATOM AtomCluster::translate_sphere(ATOM sphere, double x_step, double y_step,
                                   double z_step, int x_sign, int y_sign,
                                   int z_sign) {
  ATOM new_sphere_pos;
  new_sphere_pos.radius = sphere.radius;
  // sign: -1 = negative, 0 = no change, 1 = positive
  if (x_sign == -1)
    new_sphere_pos.x = sphere.x - x_step;
  else if (x_sign == 1)
    new_sphere_pos.x = sphere.x + x_step;
  else if (x_sign == 0)
    new_sphere_pos.x = sphere.x;

  if (y_sign == -1)
    new_sphere_pos.y = sphere.y - y_step;
  else if (y_sign == 1)
    new_sphere_pos.y = sphere.y + y_step;
  else if (y_sign == 0)
    new_sphere_pos.y = sphere.y;

  if (z_sign == -1)
    new_sphere_pos.z = sphere.z - z_step;
  else if (z_sign == 1)
    new_sphere_pos.z = sphere.z + z_step;
  else if (z_sign == 0)
    new_sphere_pos.z = sphere.z;
...
void AtomCluster::plusMinus_axes(double shift, int i) {
  ...
  atom_vector.at(i + 1) =
      translate_sphere(atom_vector.at(i + 1), shift, 0, 0, -1, 0, 0);
  atom_vector.at(i + 2) =
      translate_sphere(atom_vector.at(i + 2), 0, shift, 0, 0, 1, 0);
  atom_vector.at(i + 3) =
      translate_sphere(atom_vector.at(i + 3), 0, shift, 0, 0, -1, 0);
  atom_vector.at(i + 4) =
      translate_sphere(atom_vector.at(i + 4), 0, 0, shift, 0, 0, 1);
  atom_vector.at(i + 5) =
      translate_sphere(atom_vector.at(i + 5), 0, 0, shift, 0, 0, -1);
}
...
void AtomCluster::plusMinus_xy(double x, double y, int i) {
  ...
  atom_vector.at(i + 1) =
      translate_sphere(atom_vector.at(i + 1), x, y, 0, -1, 1, 0);
  atom_vector.at(i + 2) =
      translate_sphere(atom_vector.at(i + 2), x, y, 0, -1, -1, 0);
  atom_vector.at(i + 3) =
      translate_sphere(atom_vector.at(i + 3), x, y, 0, 1, -1, 0);
}
...
void AtomCluster::plusMinus_xz(double x, double z, int i) {
  ...
  atom_vector.at(i + 1) =
      translate_sphere(atom_vector.at(i + 1), x, 0, z, -1, 0, 1);
  atom_vector.at(i + 2) =
      translate_sphere(atom_vector.at(i + 2), x, 0, z, -1, 0, -1);
  atom_vector.at(i + 3) =
      translate_sphere(atom_vector.at(i + 3), x, 0, z, 1, 0, -1);
}
...
void AtomCluster::plusMinus_yz(double y, double z, int i) {
  ...
  atom_vector.at(i + 1) =
      translate_sphere(atom_vector.at(i + 1), 0, y, z, 0, -1, 1);
  atom_vector.at(i + 2) =
      translate_sphere(atom_vector.at(i + 2), 0, y, z, 0, -1, -1);
  atom_vector.at(i + 3) =
      translate_sphere(atom_vector.at(i + 3), 0, y, z, 0, 1, -1);
}
...
void AtomCluster::plusMinus_all(double x, double y, double z, int i) {
  ...
  atom_vector.at(i + 1) =
      translate_sphere(atom_vector.at(i + 1), x, y, z, -1, 1, 1);
  atom_vector.at(i + 2) =
      translate_sphere(atom_vector.at(i + 2), x, y, z, 1, -1, 1);
  atom_vector.at(i + 3) =
      translate_sphere(atom_vector.at(i + 3), x, y, z, 1, 1, -1);
  atom_vector.at(i + 4) =
      translate_sphere(atom_vector.at(i + 4), x, y, z, -1, -1, 1);
  atom_vector.at(i + 5) =
      translate_sphere(atom_vector.at(i + 5), x, y, z, -1, 1, -1);
  atom_vector.at(i + 6) =
      translate_sphere(atom_vector.at(i + 6), x, y, z, 1, -1, -1);
  atom_vector.at(i + 7) =
      translate_sphere(atom_vector.at(i + 7), x, y, z, -1, -1, -1);
}
...
ATOM AtomCluster::calc_center(vector<ATOM> old_coords, int i_0, int i_1,
                              int i_2, int i_3, int i_4, double shift) {
  ATOM origin = center;
...
void AtomCluster::calc_centerSpheres(double shift, int i) {
  ...
  new_center =
      calc_center(atom_vector, i + 4, i + 6, i + 8, i + 22, i + 24, shift);
  atom_vector.push_back(new_center);
  // Pentagon 2: (0, 0, T), (-1/2, +- T/2, (1+T)/2), (-(1+T)/2, +-1/2, T/2)
  new_center =
      calc_center(atom_vector, i + 4, i + 7, i + 10, i + 23, i + 26, shift);
  atom_vector.push_back(new_center);
  // Pentagon 3: (0, 0, -T), (1/2, +- T/2, -(1+T)/2), ((1+T)/2, +-1/2, -T/2)
  new_center =
      calc_center(atom_vector, i + 5, i + 9, i + 12, i + 25, i + 28, shift);
  atom_vector.push_back(new_center);
  // Pentagon 4: (0, 0, -T), (-1/2, +- T/2, -(1+T)/2), (-(1+T)/2, +-1/2, -T/2)
  new_center =
      calc_center(atom_vector, i + 5, i + 11, i + 13, i + 27, i + 29, shift);
  atom_vector.push_back(new_center);

  // mid-top 4 (w/ y vertices)
  // Pentagon 5: (+-1/2, T/2, (1+T)/2), (+-T/2, (1+T)/2, 1/2), (0, T, 0)
  new_center =
      calc_center(atom_vector, i + 2, i + 6, i + 7, i + 14, i + 15, shift);
  atom_vector.push_back(new_center);
  // Pentagon 6: (+-1/2, T/2, -(1+T)/2), (+-T/2, (1+T)/2, -1/2), (0, T, 0)
  new_center =
      calc_center(atom_vector, i + 2, i + 9, i + 11, i + 17, i + 19, shift);
  atom_vector.push_back(new_center);
  // Pentagon 7: (+-1/2, -T/2, (1+T)/2), (+-T/2, -(1+T)/2, 1/2), (0, -T, 0)
  new_center =
      calc_center(atom_vector, i + 3, i + 8, i + 10, i + 16, i + 18, shift);
  atom_vector.push_back(new_center);
  // Pentagon 8: (+-1/2, -T/2, -(1+T)/2), (+-T/2, -(1+T)/2, -1/2), (0, -T, 0)
  new_center =
      calc_center(atom_vector, i + 3, i + 12, i + 13, i + 20, i + 21, shift);
  atom_vector.push_back(new_center);

  // Middle 4 (w/ x vertices)
  // Pentagon 9: (T, 0, 0), (T/2, (1+T)/2, +-1/2), ((1+T)/2, 1/2, +-T/2)
  new_center =
      calc_center(atom_vector, i, i + 14, i + 17, i + 22, i + 25, shift);
  atom_vector.push_back(new_center);
  // Pentagon 10: (T, 0, 0), (T/2, -(1+T)/2, +-1/2), ((1+T)/2, -1/2, +-T/2)
  new_center =
      calc_center(atom_vector, i, i + 16, i + 20, i + 24, i + 28, shift);
  atom_vector.push_back(new_center);
  // Pentagon 11: (-T, 0, 0), (-T/2, (1+T)/2, +-1/2), (-(1+T)/2, 1/2, +-T/2)
  new_center =
      calc_center(atom_vector, i + 1, i + 15, i + 19, i + 23, i + 27, shift);
  atom_vector.push_back(new_center);
  // Pentagon 12: (-T, 0, 0), (-T/2, -(1+T)/2, +-1/2), (-(1+T)/2, -1/2, +-T/2)
  new_center =
      calc_center(atom_vector, i + 1, i + 18, i + 21, i + 26, i + 29, shift);
  atom_vector.push_back(new_center);
}
...
void AtomCluster::sphere_spiral(double n, double shift) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    ATOM new_sphere = translate_sphere(center, shift * x_shift, shift * y_shift,
                                       shift * z_shift, 1, 1, 1);
...
/* Function that analyzes distribution of atomic radii and execute replacement
   of large atoms with clusters of small ones */
void setupHighAccuracyAtomNetwork(ATOM_NETWORK *atmnet,
                                  std::string AccSetting) {
  double minr, maxr;

  /* analyze the original atom network */
  for (unsigned int i = 0; i < atmnet->atoms.size(); i++) {
    if (i == 0) {
      minr = atmnet->atoms.at(i).radius;
      maxr = minr;
    } else {
  ...
  cout << "Radii analysis: the smallest atom r = " << minr
       << " while the largest atoms r = " << maxr << ".\n";

  if (minr < SPHERE_APPROX_MIN_R || maxr > SPHERE_APPROX_MAX_R) {
    cerr << "HIGH ACCURACY CANNOT BE APPLIED!\n"
         << "The current version of high accuracy routine is using parameters "
            "derived from analysis of \n"
         << "CCDC database. The radii defined for the current system are "
            "outside of predefined range.\n"
         << "If you are a pro-user, change the source code (sphere_approx.cc) "
    ...
    for (unsigned int i = 0; i < atmnet->atoms.size(); i++) {
      if (atmnet->atoms.at(i).radius == minr) {
        ...
        if (AccSetting == "OCC" || AccSetting == "FCC" || AccSetting == "ACC" ||
            AccSetting == "AQC" || AccSetting == "DDH" || AccSetting == "TIH" ||
            AccSetting == "ICH" || AccSetting == "ICC" || AccSetting == "RIH" ||
            AccSetting == "S4" || AccSetting == "S10" || AccSetting == "S20" ||
            AccSetting == "S30" || AccSetting == "S40" || AccSetting == "S50" ||
            AccSetting == "S100" || AccSetting == "S500" ||
            AccSetting == "S1000" || AccSetting == "S10000") {
  ...
  };  // ends if statement checking if minr and maxr are within the accaptable
      // range
