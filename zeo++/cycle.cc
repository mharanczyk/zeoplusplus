...
void edge_finder(const VORONOI_NETWORK* vornet, int node_index,
        ...
        for (int i = 0; i < dnet->nodes.size(); ++i){
            ...
            for (; it != origNode.connections.end(); ++it, ++conn_ind){
                ...
                while (stack.size() != 0) {
                    ...
                    int conn_ind = 0;
                    for (; cit != currNode.connections.end(); ++cit,
++conn_ind){
                        //Disable the connection and the reverse connection
                        cit->max_radius = 0;
                        // Find the index of connection from to-node to
from-node CONN reverse_conn = *it; reverse_conn.from = it->to; reverse_conn.to =
it->from;
                        ...
                        rev_cit = find(dnet->nodes[it->to].connections.begin(),
                                       ...
                                       reverse_conn);
                        assert (rev_cit !=
dnet->nodes[it->to].connections.end()); rev_cit->max_radius = 0;
                   ...
                   int cycle_length, bool weight_flag, int weight_range)
{
    ...
    return compute_cycle(&dnet, cycles, cycle_length, weight_flag,
weight_range);
}
*/
...
bool compute_4cycle(DIJKSTRA_NETWORK* dnet, vector<CYCLE>* cycles,
  ...
  for (int i = 0; i < dnet->nodes.size(); ++i) {
    ...
    if (i == 0)
      cout << "org_node: " << org_node.x << " " << org_node.y << " "
           << org_node.z << endl;
    ...
    for (; it != org_node.connections.end(); ++it, ++conn_ind) {
      ...
      for (; oit != org_node.connections.end(); ++oit, ++oconn_ind) {
        ...
        if (inter_node_id == org_node_id || inter_node_id == end_node_id)
          continue;
        ...
        for (CnIt iit = inter_node.connections.begin();
             iit != inter_node.connections.end(); ++iit)
        ...
        for (; eit != end_node.connections.end(); ++eit, ++econn_ind) {
          ...
          if (final_node_id == end_node_id || final_node_id == org_node_id)
            continue;
...
void face_center(ATOM_NETWORK* atmnet, vector<XYZ>* face_centers) {
  ...
  for (vc_it = vcell.begin(); vc_it != vcell.end(); ++vc_it) {
    for (vf_it = vc_it->faces.begin(); vf_it != vc_it->faces.end(); ++vf_it) {
      ...
      for (ip_it = vf_it->orderedVertices.begin();
           ip_it != vf_it->orderedVertices.end(); ++ip_it) {
        ;
        // cout << "(";
        // ip_it->print(cout);
        // cout << ")  " ;
