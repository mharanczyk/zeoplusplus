...
int main(int argc, char *argv[]) {
  ...
  if (!error) {
    ...
    if (!error) {
      ...
      bool radial = true, outputZvis = false, buildBasicCells = false,
           useMass = true, highAccuracy = false,
           allowAdjustCoordsAndCell = false, stripAtomNamesFlag = false;

      string AccSetting;

      // by default, we want to populate the table with radii and masses, even
      // if later we will not reference them
      initializeRadTable();
      initializeMassTable();
      initializeCovRadTable();
      initializePT();

      // detect -e, which will direct the entire Zeo++ execution to a new route
      // (within jobs/tasks classes)
      for (int i = 0; i < numCommands; i++) {
        vector<string> command = commands[i];
        if (command[0].compare("-e") == 0) {
          cout << "Entering the new execution route\n";
          zeoJob job;
          bool setup_error = job.setup(
              commands, name, extension);  // setup run parameters for a job
      ...
      for (int i = 0; i < numCommands; i++) {
        ...
        if (command[0].compare("-r") == 0) {
          ...
          outputZvis = true;
        } else if (commands[i][0].compare("-sa") == 0 ||
                   commands[i][0].compare("-vol") == 0) {
      ...
      if (!error) {
        ...
        if (radial) loadRadii(&atmnet);

        initializeAtomicNumberTable();
        initializeAtomCharacterTable();

        loadMass(useMass, &atmnet);  // assigns masses to atoms (assigns 0s in
                                     // useMass flag = false)
        // High accuracy options - if the system contains particles of different
        // radii, the employed radical Voronoi decomposition may not be
        // accurate. it can be, however, improved if large particles are
        // replaced by clusters of smaller one (idea taken from Carolyn
        // Philiphs/Chris Rycroft).
        atmnet.copy(
            &orgAtomnet);  // keep a copy of original atomnet (if high accuracy
                           // is not set, this is the same as analyzed network)
        if (radial && highAccuracy) {
          // atmnet.copy(&orgAtomnet); // keep a copy of original atomnet
          // calling the following function will modify atmnet - replace large
          // atoms with clusters of small ones
        ...
        cout << "Starting Voronoi decomposition"
             << "\n";
        if (radial)
          rad_con = (container_periodic_poly *)performVoronoiDecomp(
              true, &atmnet, &vornet, cells, outputZvis, bvcells);
        else
          no_rad_con = (container_periodic *)performVoronoiDecomp(
              false, &atmnet, &vornet, cells, outputZvis, bvcells);
        ...
        for (int i = 0; i < numCommands && !error; i++) {
          vector<string> command = commands[i];
          cout << "Command " << i << "  " << command[0] << "\n";
          if (command[0].compare("-cssr") == 0) {
            if (strcmp(extension, ".v1") == 0) {
              cerr << "Can not output a .cssr file when a .v1 file is used as "
                      "input."
                   << "\n"
                   << "Exiting ..."
                   << "\n";
            ...
            if (!writeToCSSR((char *)filename.data(), &atmnet)) {
              error = true;
              break;
            }
            ...
            // if(!writeToCIF((char *)filename.data(), name, &atmnet))
            // {error=true; break;}
            if (!writeToCIF((char *)filename.data(), &atmnet)) {
              error = true;
              break;
            }
            ...
            bool is_duplicate_perimeter_atoms = false;
            ...
            string filename =
                processFilename(command, name, "_supercell.xyz", 0, 1);
            if (filename.empty()) {
              error = true;
              break;
            }
            ...
            bool is_supercell = false;
            bool is_duplicate_perimeter_atoms = true;
            ...
            string filename2 =
                processFilename(command, name, "_supercell_vis.xyz", 0, 1);
            if (filename2.empty()) {
              error = true;
              break;
            }
            ...
            if (!writeToVTK((char *)filename3.data(), &atmnet)) {
              error = true;
              break;
            }
            ...
            if (!writeToVTK((char *)filename.data(), &atmnet)) {
              error = true;
              break;
            }
            ...
            if (!writeToV1((char *)filename.data(), &atmnet)) {
              error = true;
              break;
            }
            ...
            if (!writeToNt2((char *)filename.data(), &vornet)) {
              error = true;
              break;
            }
            ...
            bool is_supercell = false;
            if (!writeToMOPAC((char *)filename.data(), &atmnet, is_supercell)) {
              error = true;
              break;
            }
          } else if (command[0].compare("-supermopac") == 0) {
            string filename =
                processFilename(command, name, "_supercell.mop", 0, 1);
            if (filename.empty()) {
              error = true;
              break;
            }
          ...
          } else if (command[0].compare("-res") ==
                     0) {  // free sphere calcluation
            string filename = processFilename(command, name, ".res", 0, 1);
            if (filename.empty()) {
              error = true;
              break;
            }
            calculateFreeSphereParameters(&vornet, (char *)filename.data(),
                                          false);
          } else if (command[0].compare("-resex") ==
                     0) {  // free sphere calcluaton with extended output
            string filename = processFilename(command, name, ".res", 0, 1);
            if (filename.empty()) {
              error = true;
              break;
            ...
            calculateFreeSphereParameters(&vornet, (char *)filename.data(),
                                          true);
          } else if (command[0].compare("-strinfo") == 0 ||
                     command[0].compare("-strinfoex") == 0) {
            // print information about the structure/topology
            // extended option saves framework/molecules with their ids next to
            // each atom
            string filename = processFilename(command, name, ".strinfo", 0, 1);
            string filenameExtOutput =
                processFilename(command, name, ".frameid", 0,
                                1);  // filename to save ext. output
            if (filename.empty()) {
              error = true;
              break;
            }
            if (command[0].compare("-strinfo") == 0)
              getStructureInformation((char *)filename.data(),
                                      (char *)filenameExtOutput.data(), &atmnet,
                                      false);  // last argument 4 ext. output
            else
              getStructureInformation((char *)filename.data(),
                                      (char *)filenameExtOutput.data(), &atmnet,
                                      true);
          } else if (command[0].compare("-oms") == 0 ||
                     command[0].compare("-omsex") == 0) {
            ...
            string filenameExtOutput = processFilename(
                command, name, ".omsex", 0, 1);  // filename to save ext. output
            if (filename.empty()) {
              error = true;
              break;
            }
            if (command[0].compare("-oms") == 0)
              getOMSInformation((char *)filename.data(),
                                (char *)filenameExtOutput.data(), &atmnet,
                                false);  // last argument 4 ext. output
            else
              getOMSInformation((char *)filename.data(),
            ...
            //      analyze_accessible_voronoi_pre_segment(&vornet, probeRad,
            //      &accessInfo, name);
            fstream output;
          ...
          // Rich: holograms as a separate flag, -holo
            ...
            if (command.size() == 3) {  // i.e. a directory was specified
              ...
              analyze_accessible_voronoi_pre_segment(
                  &vornet, probeRad, &accessInfo, name,
                  bin_directory);  // this line generates and outputs Voronoi
                                   // holograms with bins from a specified
                                   // directory
            }
            analyze_accessible_voronoi_pre_segment(
                &vornet, probeRad, &accessInfo,
                name);  // this line generates and outputs Voronoi holograms
                        // with default bins
          }
          ...
          */
            ...
            fstream output;
            output.open(filename.data(), fstream::out);
            output << filename.data() << "   " << channels.size()
                   << " channels identified of dimensionality ";
            ...
            for (unsigned int i = 0; i < channels.size(); i++) {
              pair<double, pair<double, double> > maxdidfdif =
                  channels[i].findFreeIncludedSphereDiameter();
              output << "Channel  " << i << "  " << maxdidfdif.first << "  "
                     << maxdidfdif.second.first << "  "
                     << maxdidfdif.second.second << "\n";
              if (maxdidfdif.first > di) di = maxdidfdif.first;
              if (maxdidfdif.second.first > df) df = maxdidfdif.second.first;
              if (maxdidfdif.second.second > dif)
                dif = maxdidfdif.second.second;
            }
            output << filename << " summary(Max_of_columns_above)   " << di
                   << " " << df << "  " << dif
                   << "  probe_rad: " << probe_radius
                   << "  probe_diam: " << 2 * probe_radius << "\n";
          ...
          // This include di and positions of nodes
            ...
            vector<bool> accessInfo;
            vector<PORE> pores;
            PORE::findChannelsAndPockets(&vornet, probe_radius, &accessInfo,
                                         &pores);

            int n_channels = 0;
            int n_pockets = 0;  // number of channels and pockets
            for (unsigned int i = 0; i < pores.size(); i++) {
              if (pores[i].dimensionality > 0) {  // Channels
                n_channels++;
              } else {  // Pockets
                n_pockets++;
              };
            ...
            fstream output;
            output.open(filename.data(), fstream::out);
            output << filename.data() << " Pores:  " << pores.size()
                   << " Channels:  " << n_channels << " Pockets:  " << n_pockets
                   << "\n";

            double volume = calcDeterminant(
                atmnet.ucVectors);  // Unit cell volume/Units of A^3
            int numSamples = (int)(volume * numSamplesAV);
            calcAV(&atmnet, &orgAtomnet, highAccuracy, probe_radius,
                   probe_radius, numSamples, true, output,
                   (char *)filename.c_str(), 0, 0, 0, 0, -1, -1, 0);
            calcASA(&atmnet, &orgAtomnet, highAccuracy, probe_radius,
                    probe_radius, calcDensity(&atmnet), numSamplesASA, true,
                    output, (char *)filename.c_str(), 0, 0, 0, 0);

            for (unsigned int i = 0; i < pores.size(); i++)
              if (pores[i].dimensionality > 0)
                pores[i].printPoreSummary(output, &atmnet);  // Channels

            for (unsigned int i = 0; i < pores.size(); i++)
              if (pores[i].dimensionality == 0)
                pores[i].printPoreSummary(output, &atmnet);  // Pockets
            ...
            /*
                        ...
                        for(unsigned int i=0; i < channels.size(); i++){
                          pair <double, pair<double,double> > maxdidfdif =
               channels[i].findFreeIncludedSphereDiameter(); output << "Channel
               " << i << "  "  << maxdidfdif.first << "  " <<
               maxdidfdif.second.first << "  " << maxdidfdif.second.second <<
               "\n";
                          };
            */
            output.close();
          } else if (command[0].compare("-poreinfoSummary") ==
                     0) {  // analyzes a series of .poreinfo files
            string filename_output =
                processFilename(command, name, ".poreinfoSummary", 1, 2);
            if (filename_output.empty()) {
              error = true;
              break;
            }
            string filename_InputData = string(command[1]);
            //            calculateAverageGrid(&atmnet, filename_InputData,
            //            filename_Gaussian_cube, angstrom_to_bohr, useMass);
            analyzePoreInfoFiles(&atmnet, filename_InputData, filename_output);
          } else if ((command[0].compare("-sa") == 0) ||
                     (command[0].compare("-saex") == 0) ||
                     (command[0].compare("-zsa") == 0) ||
                     (command[0].compare("-vsa") == 0) ||
                     (command[0].compare("-lsa") == 0)) {
            bool visualize = (command[0].compare("-zsa") == 0 ||
                              command[0].compare("-vsa") == 0 ||
                              command[0].compare("-lsa") == 0);
            bool visVisITflag = (command[0].compare("-vsa") == 0 ||
                                 command[0].compare("-lsa") == 0);
            ...
            fstream output;
            output.open(filename.data(), fstream::out);
            calcASA(&atmnet, &orgAtomnet, highAccuracy, chan_radius,
                    probe_radius, calcDensity(&atmnet), numSamples, true,
                    output, (char *)filename.data(), visualize, visVisITflag,
                    LiverpoolFlag, ExtendedOutputFlag);
            output.close();
          } else if ((command[0].compare("-vol") == 0) ||
                     (command[0].compare("-zvol") == 0) ||
                     (command[0].compare("-vvol") == 0) ||
                     (command[0].compare("-lvol") == 0) ||
                     (command[0].compare("-volpo") == 0)) {
            bool visualize = (command[0].compare("-zvol") == 0 ||
                              command[0].compare("-vvol") == 0 ||
                              command[0].compare("-lvol") == 0);
            bool VisITflag = (command[0].compare("-vvol") == 0 ||
                              command[0].compare("-lvol") == 0);
            ...
            if (command.size() > 5 && command.size() < 8 &&
                ProbeOccupiableFlag == true) {
              printf(
                  "Error: -volpo does not run with up to 4 arguments  but %d "
            ...
            double high_dist_range =
                -1;  // negative values indicate if we are not sampling only
                     // within a specific distance of the surface
            ...
            if (command.size() > 5) {  // specifying ranges
              ...
              if (low_dist_range < 0 || high_dist_range < 0) {
                printf(
                    "ERROR: cannot call -vol flag with negative distance "
                    "ranges (args were: %.3f %.3f)\n",
            ...
            string filename;
            if (command.size() == 5 ||
                command.size() == 7)  // specifying output filename
              filename = string(command[command.size() - 1]);
            else
              filename = string(name).append(suffix);
            fstream output;
            output.open(filename.data(), fstream::out);
            bool blockingMode = false;
            calcAV(&atmnet, &orgAtomnet, highAccuracy, chan_radius,
                   probe_radius, numSamples, true, output,
                   (char *)filename.data(), visualize, VisITflag, LiverpoolFlag,
                   blockingMode, low_dist_range, high_dist_range,
                   ProbeOccupiableFlag);
            output.close();
            ...
            if (numSamples < 0) {
              printf(
                  "ERROR: cannot call -block flag with negative numSamples "
                  "(arg was: %d)\n",
                  numSamples);
              //            exit(EXIT_FAILURE);
              error = true;
            ...
            calcAV(&atmnet, &orgAtomnet, highAccuracy, probe_radius,
                   probe_radius, numSamples, true, output,
                   (char *)filename.data(), 0, 0, 0, 1, low_dist_range,
                   high_dist_range, 0);
            output.close();
          }

          else if (command[0].compare("-gridGAI") == 0 ||
                   command[0].compare("-gridGAIBohr") ==
                       0) {  // saves distance grid in Gausian cube format
                             // supplementing it with accessibility info
            double probe_radius = strtod(command[1].data(), NULL);
            string filename_Gaussian_cube =
                processFilename(command, name, ".cube", 1, 2);
            bool angstrom_to_bohr = command[0].compare("-gridGAIBohr") == 0;
            if (filename_Gaussian_cube.empty()) {
              error = true;
              break;
            }
            generateGaussianGridWithAccessibilityInfo(
                &atmnet, &orgAtomnet, highAccuracy, probe_radius,
                filename_Gaussian_cube, angstrom_to_bohr, useMass);
          }

          else if (command[0].compare("-gridBOV") ==
                   0) {  // saves distance grid in BOV format
            string filename_f_dist =
                processFilename(command, name, "_f.distances", 0, 1);
            string filename_f_bov =
                processFilename(command, name, "_f.bov", 0, 0);
            string filename_g_dist =
                processFilename(command, name, "_g.distances", 0,
                                1);  // g and h grids are for testing only
            string filename_g_bov = processFilename(
                command, name, "_g.bov", 0, 1);  // the code is commented out
            string filename_h_dist =
                processFilename(command, name, "_h.distances", 0, 1);
            string filename_h_bov =
                processFilename(command, name, "_h.bov", 0, 1);
            if (filename_f_dist.empty() || filename_f_bov.empty() ||
                filename_g_dist.empty() || filename_g_bov.empty() ||
                filename_h_dist.empty() || filename_h_bov.empty()) {
              error = true;
              break;
            }
            generateBOVGrid(&atmnet, filename_f_dist, filename_g_dist,
                            filename_h_dist, filename_f_bov, filename_g_bov,
                            filename_h_bov);
          } else if (command[0].compare("-gridG") == 0 ||
                     command[0].compare("-gridGBohr") ==
                         0) {  // saves distance grid in Gausian cube format
            string filename_Gaussian_cube =
                processFilename(command, name, ".cube", 0, 1);
            bool angstrom_to_bohr = command[0].compare("-gridGBohr") == 0;
            if (filename_Gaussian_cube.empty()) {
              error = true;
              break;
            }
            generateGaussianGrid(&atmnet, filename_Gaussian_cube,
                                 angstrom_to_bohr, useMass);
          } else if (command[0].compare("-gridGprojdata") == 0 ||
                     command[0].compare("-gridGprojdataBohr") ==
                         0) {  // projects a file with datapoints (frac.
                               // coordinates) onto a gaussian grid (3d
                               // histogram)
            string filename_Gaussian_cube =
                processFilename(command, name, ".cube", 1, 2);
            bool angstrom_to_bohr =
                command[0].compare("-gridGprojdataBohr") == 0;
            if (filename_Gaussian_cube.empty()) {
              error = true;
              break;
            }
            string filename_InputData = string(command[1]);
            // cout << "projdata " << filename_InputData << "  " <<
            // filename_Gaussian_cube << "\n";
            calculateAverageGrid(&atmnet, filename_InputData,
                                 filename_Gaussian_cube, angstrom_to_bohr,
                                 useMass);
          } else if (command[0].compare("-gridGprojdataperframe") == 0 ||
                     command[0].compare("-gridGprojdataperframeBohr") ==
                         0) {  // projects a file with datapoints (frac.
                               // coordinates) onto a gaussian grid (3d
                               // histogram)
            ...
            string filename_Gaussian_cube =
                processFilename(command, name, ".cube", 1, 2);
            bool angstrom_to_bohr =
                command[0].compare("-gridGprojdataperframeBohr") == 0;
            if (filename_Gaussian_cube.empty()) {
              error = true;
              break;
            }
            ...
            // cout << "projdata " << filename_InputData << "  " <<
            // filename_Gaussian_cube << "\n";
            calculateAverageGridPerFrame(&atmnet, filename_InputData,
                                         filename_Gaussian_cube,
                                         angstrom_to_bohr, useMass);
          } else if ((command[0].compare("-psd") == 0) ||
                     (command[0].compare("-zpsd") == 0) ||
                     command[0].compare("-vpsd") == 0) {
            string suffix_histogram = ".psd_histo";
            string filename_histogram =
                processFilename(command, name, suffix_histogram, 3, 4);
            if (filename_histogram.empty()) {
              error = true;
              break;
            }
            double chan_radius = strtod(command[1].data(), NULL);
            ...
            bool visualize = (command[0].compare("-zpsd") == 0 ||
                              command[0].compare("-vpsd") == 0);
            ...
            if (visualize) {  // -vpsd or -zpsd, visualization run
              ...
              if (visVisITflag) {
                ...
                filename_points =
                    processFilename(command, name, suffix_points, 3, 4);
                filename_radii =
                    processFilename(command, name, suffix_radii, 3, 4);
                if (filename_points.empty() || filename_radii.empty()) {
                  error = true;
                  break;
                }
                // outfile.open(filename_points.data(), fstream::out);
                // nodesRadii.open(filename_radii.data(), fstream::out);
                calcPoreSizeDistr(&atmnet, &orgAtomnet, highAccuracy,
                                  chan_radius, probe_radius, numSamples, true,
                                  filename_histogram, filename_points,
                                  filename_radii, "", visualize, visVisITflag);
              }
              if (!visVisITflag) {
                // here goes ZeoVis
              }
            } else {  // -psd: regular PSD run, no visualization
              calcPoreSizeDistr(&atmnet, &orgAtomnet, highAccuracy, chan_radius,
                                probe_radius, numSamples, true,
                                filename_histogram, "", "", "", false, false);
            }
            // output.close();
            // outfile.close();
            // nodesRadii.close();
          ...
          else if ((command[0].compare("-ray_atom") == 0) ||
                   (command[0].compare("-zray_atom") == 0)) {
            ...
            fstream output;
            output.open(filename.data(), fstream::out);
            calcRaysInAV(&atmnet, &orgAtomnet, highAccuracy, chan_radius,
                         probe_radius, numSamples, output, visualize, atom);
            output.close();
          }

          else if ((command[0].compare("-ray_sphere") == 0) ||
                   (command[0].compare("-zray_sphere") == 0)) {
            ...
            output.open(filename.data(), fstream::out);
            calcRaysInAV(&atmnet, &orgAtomnet, highAccuracy, chan_radius,
                         probe_radius, numSamples, output, visualize, sphere);
            output.close();
          }

          else if ((command[0].compare("-ray_node") == 0) ||
                   (command[0].compare("-zray_node") == 0)) {
            ...
            output.open(filename.data(), fstream::out);
            calcRaysInAV(&atmnet, &orgAtomnet, highAccuracy, chan_radius,
                         probe_radius, numSamples, output, visualize, node);
            output.close();
          } else if ((command[0].compare("-ray_andrew_sphere") == 0) ||
                     (command[0].compare("-zray_andrew_sphere") == 0)) {
            ...
            output.close();
          } else if ((command[0].compare("-ray_andrew_atom") == 0) ||
                     (command[0].compare("-zray_andrew_atom") == 0)) {
            ...
            vector<CAGE> cages;
            output.open(filename.data(), fstream::out);
            identifyCages(&atmnet, &vornet, &dnet, probe_radius, visualize,
                          output, cages);
            output.close();
            ...
            vector<bool> accessInfo;
            vector<CHANNEL> channels;
            // VORONOI_NETWORK newNetwork;  pruneVoronoiNetwork(&vornet,
            // &newNetwork, probe_radius);
            ...
            for (unsigned int i = 0; i < channels.size(); i++) {
              ...
              FEATURE seg = FEATURE(nodeIDs, &dnet, channels[i].dimensionality,
                                    channels[i].basis);
              int numSegs = seg.createSegments(&atmnet, &newNetwork, &dnet,
                                               output, initIndex);
            ...
            for (unsigned int i = 0; i < channels.size(); i++) {
              ...
              FEATURE feat = FEATURE(nodeIDs, &dnet, channels[i].dimensionality,
                                     channels[i].basis);
              int numFeats = feat.createFeatures(&atmnet, &newNetwork, &dnet,
                                                 output, initIndex, name);
            ...
            int numSubs = 0;
            bool subSeed = false;  // Change to true to generate other possible
                                   // configuration
            bool success = substituteAtoms(&atmnet, &newNetwork, subSeed,
                                           &numSubs, radial);
            if (!success) {
              ...
              cout << "Substituted " << numSubs << " atoms when analyzing "
                   << name << "\n";
            }
          } else if (command[0].compare("-fsub") == 0) {
            string filename =
                processFilename(command, name, "_fsub.cssr", 1, 2);
            if (filename.empty()) {
              error = true;
              break;
            }
            ...
            bool subSeed = false;  // Substitute first Si when creating initial
                                   // 50/50 configuration
            bool success =
                fracSubstituteAtoms(&atmnet, &newNetwork, subSeed, frac,
                                    randSeed, &numSubs, &subFrac, radial);
            if (!success) {
              cerr << "Error: Atom substitution unsucessful."
                   << "\n"
                   << "Output not written to file."
              ...
              writeToCSSR((char *)filename.data(), &newNetwork);
              cout << "Substituted " << numSubs
                   << " Si atoms for a fraction of " << subFrac
                   << " when analyzing " << name << "\n";
            }
          }
          /* Maciek's version of substitution */
          /* This function has two modifications w.r.t -fsub. It allows chaging
             random seed to generate different configurations and removes
             topology check which ensured that every other atom is Si */
          /* usage: -fsubM fraction random_seed(0 for default) outname */
          else if (command[0].compare("-fsubM") == 0) {
            string filename =
                processFilename(command, name, "_fsub.cssr", 2, 3);
            if (filename.empty()) {
              error = true;
              break;
            }
            ...
            if (rsd != 0) {
              randSeed =
                  rsd;  // this is a quick fix to renerate random structures
              cout << "Overriding default random seed with " << rsd << endl;
            };
            bool subSeed = false;  // Substitute first Si when creating initial
                                   // 50/50 configuration
            bool success =
                fracSubstituteAtoms_Maciek(atmnet, newNetwork, subSeed, frac,
                                           randSeed, numSubs, subFrac, radial);
            if (!success) {
              cerr << "Error: Atom substitution unsucessful."
                   << "\n"
          ...
          // create skeleton diagram (prints Voronoi network to file, and
          // accessible Voronoi network to a separate file, in an adjacent cell
          // if specified with shift - xyz and vtk output files generated)
            ...
            if (command.size() == 5) {
              ...
              skel_a = strtod(command[2].data(), NULL),
              skel_b = strtod(command[3].data(), NULL),
              skel_c = strtod(command[4].data(), NULL);
            }
            visVoro(name, probeRad, skel_a, skel_b, skel_c, &vornet, &atmnet);
          }
          // extract spherical substructures: this is a functionality designed
          // for extracting local substructures of zeolites so that they can be
          // scanned for potential guest molecule binding sites this
          ...
          // type
            ...
            if (command.size() == 4) {
              ...
              element = command[3];
              printf(
                  "DEBUG: extracting spheres from Voronoi network of %s atoms "
                  "only\n",
                  element.c_str());
            }
            double probeRad = strtod(command[1].data(), NULL);
            double sphereRad = strtod(command[2].data(), NULL);
            // utilise new cellmulti function; the smallest supercell satisfying
            // sphereRad/2.0 is the same as the number of additional cells in
            // each direction required to extract sphereRad around any point in
            // the unit cell
            TRIPLET supercell = atmnet.getSmallestSupercell(sphereRad / 2.0);
            getLocalSubstructures(name, probeRad, sphereRad, supercell, element,
                                  &vornet, &atmnet, radial, usingElementOnly);
          }

          // flag to print to terminal the tetrahedrality of, e.g. Zn4 groups
          // inside a material
          else if (command[0].compare("-findTetrahedra") == 0) {
            if (command.size() != 2) {
              printf(
                  "Error: -findTetrahedra option accepts 1 (element type) "
                  "argument but %d arguments were supplied.\n",
                  (int)(command.size() - 1));
              printf("Exiting...\n");
              //            exit(1);
              error = true;
            ...
            // report
            printf("%s %d %s tetrahedra", name, (int)(tetras.size()),
                   element.c_str());
            for (int i = 0; i < (int)(tetras.size()); i++)
              printf(" %.3f", tetras.at(i));
            printf("\n");
          }
          ...
          // radius overlaps with itself periodically
            ...
            printf("%s supercell: %d %d %d = %d cells\n", name,
                   smallest_supercell.x, smallest_supercell.y,
                   smallest_supercell.z,
                   smallest_supercell.x * smallest_supercell.y *
                       smallest_supercell.z);
          }
            ...
            for (int i = 0; i < num_v; i++) {
              ...
              if (hasMetal && onlyMetal)
                printf("\tMETAL");
              else if (hasMetal)
                printf("\tMETAL-ORGANIC");
              else
                printf("\tORGANIC");
              printf("\n");
            }
            ...
            //      exit(EXIT_FAILURE);
            error = true;
            break;  // memory leaks avoided by allowing main to terminate,
