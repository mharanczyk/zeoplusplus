...
/* Matrix methods (determinant and invert) are moved to geometry.h and
 * geometry.cc */
...
class ATOM {
 public:
  ...
  // flag used in framework builder to reflect whether an atom should be ignored
  // (e.g. H atom that needs deleting)
  bool keep;

  // framework builder uses xyz arithmetic: to handle absence of this in ATOM
  // class, need a function to return XYZ and set it
  XYZ xyz();
...
class ATOM_NETWORK {
 public:
  ...
  bool allowAdjustCoordsAndCellFlag;  // flag indicating whether we are allowed
                                      // to randomly shift atom coordinates and
                                      // unit cell parameters if the Voronoi
                                      // volume check fails
  bool highAccuracyFlag;  // flag indicating if a atom network was altered (to
                          // enable high accuracy voronoi networks)
  double a, b, c;
  ...
  std::vector<XYZ> orphan_edge_starts,
      orphan_edge_ends;  // these are vertices in the cell that are not provided
                         // in the cgd file, i.e. they are symmetry images of
                         // basic vertices - we need them when both a) the cgd
                         // file does not give edges exhaustively so they need
                         // to be constructed from symmetry, and b) there is an
                         // edge in the cgd file for which we do not know the
                         // vertex it connects
  std::vector<int>
      vertex_basic_indices;  // vertices with the same basic index come from
                             // different symmetry operations on the same
                             // original position
  std::vector<int>
      vertex_symmetry_operators;  // the symmetry operation used to arrive at
                                  // this point (note that operation 0
                                  // corresponds, conveniently, to no operation)
  int sym_ID;
  std::string sym_name;
  ...
  void make(double a0, double b0, double c0, double alpha0, double beta0,
            double gamma0);
  ...
  /** Identifies tetrahedra of the given atom type, calculates their
   * tetrahedrality and returns as a vector of doubles */
  ...
  /** Convert coordinates relative to the unit cell (a,b,c) to absolute
  ...
  /* Calculates the minimum distance between the two points whose coordinates
   * are relative to the unit cell vectors. */
  double calcDistanceABC(double a1, double b1, double c1, double a2, double b2,
                         double c2) const;
  ...
  double calcDistanceXYZABC(double x1, double y1, double z1, double a2,
   ...
   * (x2,y2,z2).  */
  double calcDistanceXYZ(double x1, double y1, double z1, double x2, double y2,
                         double z2) const;

  /* Rich edit: for static point (x1,y1,z1), returns the closest periodic image
   * of point (x2,y2,z2).  */
  const XYZ getClosestPointInABC(double x1, double y1, double z1, double x2,
                                 double y2, double z2);

  /** Modify the provided (x,y,z) Point so that its coordinates reflect unit
   * cell translations by the provided amounts along each unit cell axis. */
   ...
   */
  const Point minimizePointDistance(Point origPoint, double dx, double dy,
                                    double dz);
  ...
  double CalculateTetrahedrality4Atoms(const ATOM &atm1, const ATOM &atm2,
                                       const ATOM &atm3,
...
void write_unit_cell(FILE *orig_uc, ATOM_NETWORK *cell, string name,
                     bool is_net);
void write_vertices(FILE *orig_v, ATOM_NETWORK *cell, string name, bool rename,
                    bool is_net);
...
void add_2c_dummy_edges(ATOM_NETWORK *basic_cell, ATOM_NETWORK *full_cell,
                        vector<CONNECTION> *two_way_connections);
...
class VOR_EDGE {
 public:
  ...
  VOR_EDGE(int myFrom, int myTo, double rad, int dx, int dy, int dz,
           double len);
};
...
class VOR_NODE {
 public:
  ...
  VOR_NODE(double myX, double myY, double myZ, double rad,
           std::vector<int> ids);
  double x, y, z;
  double rad_stat_sphere;
  std::vector<int> atomIDs;
  bool active;  // flag tells if a node is taken into consideration when
                // analyzing the framework e.g. this typically will set to false
                // after pruning the network using a probe radius
};
...
class VORONOI_NETWORK {
 public:
  ...
  VORONOI_NETWORK();
  VORONOI_NETWORK(const XYZ &, const XYZ &, const XYZ &,
                  const std::vector<VOR_NODE> &, const std::vector<VOR_EDGE> &);
  ...
  /** Returns a new VORONOI_NETWORK where all the edges between nodes that are
   * not contained in nodeIDs are removed. However, the nodes are not removed */
...
bool fracSubstituteAtoms_Maciek(ATOM_NETWORK &origNet, ATOM_NETWORK &newNet,
                                bool substituteSeed, double frac, int randSeed,
                                int &numSubstitutions, double &subFrac,
                                bool radial);
