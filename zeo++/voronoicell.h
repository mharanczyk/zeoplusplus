...
class VOR_FACE {
 public:
  ...
  VOR_FACE(std::vector<Point> vertices, ATOM_NETWORK *atmnet,
           VORONOI_NETWORK *vornet);

  /* Store the provided vertices and their ids.  */
  VOR_FACE(std::vector<Point> vertices, std::vector<int> vertexIDs);

  /* Returns a vector of pairs of integers and Points, where each entry
   * represents the pair (node id, node coordinates).*/
  ...
  /* Using the provided ouput stream, write the geometric objects that would
   * fill the Voronoi face when drawn.   */
  void writeVMDFilled(std::fstream &output);
};

/* Class used to reconstruct Voronoi cells from their constituent faces.  Used
 * to write configuration files for ZeoVis visualization tool.
 */
class VOR_CELL {
 public:
  std::vector<VOR_FACE> faces;  // List of faces in cell
  int numVertices;              // Number of vertices
  std::map<Point, int, bool (*)(Point, Point)>
      vertexIDs;                  // Vertex number associated with coordinate
  std::map<int, int> idMappings;  // (Vertex ID, Node ID) pairs
  std::map<int, std::vector<int> >
      reverseIDMappings;  // (Node ID, List of Vertex IDs) pairs
  std::map<int, Point>
      vertexCoords;  // Coordinates associated with each vertex ID
  std::vector<std::set<int> >
      edgeConnections;  // List of vertex id's connected to each vertex
  ...
  static void getVoronoiCells(std::vector<VOR_CELL> *cells,
                              std::vector<std::vector<VOR_FACE> > faceInfo,
 ...
 */
void writeVornet(std::fstream &output, ATOM_NETWORK *atmnet,
                 VORONOI_NETWORK *vornet);
 ...
 * as nodes(j).
 */
void writeVMDAtomsAndNodes(std::fstream &output, ATOM_NETWORK *atmnet,
                           VORONOI_NETWORK *vornet);
 ...
 */
void writeVMDEnvVars(std::fstream &output, ATOM_NETWORK *atmnet,
                     VORONOI_NETWORK *vornet);

/* Writes all of the information necessary to properly visualize the Voronoi and
 * atom network in ZeoVis to the file referred to by filename. Information
 * includes atoms, nodes, the unitcell, the voronoi network, voronoi cells
 * (outlined and filled) and environment variables.
 */
void writeZeoVisFile(char *filename, std::vector<VOR_CELL> *cells,
                     ATOM_NETWORK *atmnet, VORONOI_NETWORK *vornet);

void writeSpecialZeoVisFile(char *filename, std::vector<VOR_CELL> *cells,
                            ATOM_NETWORK *atmnet, VORONOI_NETWORK *vornet,
                            std::vector<BASIC_VCELL> &vcells);
