...
vector<vector<double> >
    PLDTable;  // vector storing PLD table (Df/PLD for each segment pair)

vector<vector<double> >
    TDDiTable;  // vector storing frames of Di information (Di for each segment)
vector<vector<vector<double> > >
    TDPLDTable;  // vector storing frames of PLD tables (Df/PLD for each segment
                 // pair)

vector<vector<int> >
    PLDActiveTable;  // highlights connectivity between segments to be monitored
...
pair<double, double> normal_distr_parameters(vector<double> valuelist) {
  ...
  for (unsigned int i = 0; i < valuelist.size(); i++) {
    mean_stddev.second += ((valuelist[i] - mean_stddev.first) *
                           (valuelist[i] - mean_stddev.first));
  };
...
int main(int argc, char* argv[]) {
  if (argc != 4) {
  ...
  } else {  // assume correct input parameters have been provided
    ...
    cout << "Proceeding with arguments: " << summaryfilename << " "
         << listfilename << " " << bin_size << "\n";
    ...
    if (sumfile.is_open() == false) {
      cerr << "Error: A file with summary/topology  (" << summaryfilename
           << ") failed to open. \n";
      return 1;
    } else {
      string buffer;
      ...
      for (int i = 0; i < n_segments; i++) {
        for (int j = 0; j < n_segments; j++) {
          ...
          sumfile >> d;
          if (d > 1.0) {
            PLDActiveTable.at(i).at(j) = 1;
          } else {
            PLDActiveTable.at(i).at(j) = 0;
          };
        };
         ...
         for(int j=i+1;j<n_segments; j++)
           ...
           if(PLDActiveTable.at(i).at(j) == 1)
             ...
             if(DiType[i]==DiType[j])
               {
               cerr << "Segments " << i << "  " << j << " of the same type are
       connected.\nEXIT\n"; return 1;
               };
             }
           };
    ...
    if (flist.is_open() == false) {
        ...
        for (int i = 0; i < n_segments; i++) {
          ...
          for (int j = i + 1; j < n_segments; j++) {
            if (PLDActiveTable.at(i).at(j) == 1) {
              ...
              //             if(i==0&&j==8)
              //             trackdf.push_back(TDPLDTable[k].at(i).at(j));
              //             if(i==0)
              //             trackdfall.push_back(TDPLDTable[k].at(i).at(j));

            } else {
              if (TDPLDTable[k].at(i).at(j) > 0) {
                //                cerr << "Unusual segment connection detected "
                //                << i << "  " << j << " with diameter " <<
                //                TDPLDTable[k].at(i).at(j) << "\n";
      ...
      cout << "Sizes of vectors: divalue, dfvalues and dfnonconvalues "
           << di1values.size() << "     " << dfvalues.size() << "   "
           << dfnonconvalues.size() << "\n";
      ...
      cout << "\n\nDi1 and  Dfi histogram, and normalized Di1, Df and Dfnoncon "
              "histogram\n";
      ...
      /*
        ...
        cout << "\n\n\nTD data\nFrame Di1 Di2  Df_D1-D2    Df1 Df2 Df3 Df4 (for
        cage D1)\n\n";

        for(unsigned int i=0; i<trackdi1.size(); i++)
           {
           cout << i <<  " "<< trackdi1[i] << "  " << trackdi2[i] << "  "
        <<trackdf[i] << "  " <<trackdfall[4*i] << "  "<<trackdfall[4*i+1] << "
        "<<trackdfall[4*i+2] << "  "<<trackdfall[4*i+3] << "\n ";
        ...
        for(unsigned int i=0;i<dihist.size();i++)
          ...
          cout << "      " << double(dihist[i])/double(trackdi1.size()) << " "
        << double(dfhist[i])/double(trackdf.size()) << "\n";
          };

      */

    };  // ends block corresponding to successful opening of file with frame
        // list
