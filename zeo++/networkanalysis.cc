...
PATH::PATH(DIJKSTRA_NODE cNode, double maxRadius, double maxIncRadius,
           double len) {
  currentNode = cNode;
...
/* Returns true iff the first PATH has a lower maximu radius than the second
 * PATH.*/
bool hasLowerMaxR(PATH n1, PATH n2) { return n1.max_radius < n2.max_radius; }

/** Construct a traversal network in the direction (dx,dy,dz) using the provided
 * source ids, sink ids, and classified connections. */
TRAVERSAL_NETWORK::TRAVERSAL_NETWORK(int dx, int dy, int dz, vector<int> srcIDs,
                                     vector<vector<CONN> > regConns,
                                     vector<vector<CONN> > srcConns,
                                     vector<vector<CONN> > sinkConns,
                                     DIJKSTRA_NETWORK *dnetwork) {
...
/** Construct a TRAVERSAL_ NETWORK in the direction (dx,dy,dz) relative to the
 * unit cell vectors. Identify all source nodes and classify the connections
 * based on their component change in unit cell in the direction of interest. */
TRAVERSAL_NETWORK::TRAVERSAL_NETWORK(int dx, int dy, int dz,
                                     DIJKSTRA_NETWORK *dnetwork) {
  direction = DELTA_POS(dx, dy, dz);
    ...
    for (unsigned int j = 0; j < currentNode.connections.size(); j++) {
      ...
      if (!component.isZero()) {
        if (direction.x != 0) {
          ...
          cerr << "Invalid argument reached when building TRAVERSAL_NETWORK. "
                  "Please contact the source code provider with your input"
               << "\n"
               << "Exiting..."
               << "\n";
          exit(1);
 ...
 * can take to traverse this portion of the network. The bool is true iff the
 * PATH is periodically viable (node #N to node #N).*/
pair<bool, PATH> TRAVERSAL_NETWORK::findMaxFreeSphere(map<int, int> *idAliases,
                                                      set<int> *sourceNodes) {
  PATH bestPath;
  bestPath.max_radius = -1;
  ...
  for (unsigned int i = 0; i < sourceNodeIDs.size(); i++) {
    ...
    for (unsigned int j = 0; j < viableConns.size(); j++) {
      ...
      PATH newPath = PATH(nextNode, nextConn.max_radius,
                          max(sourceNode.max_radius, nextNode.max_radius),
                          nextConn.length);
  ...
  while (heap.size() != 0) {
    ...
    for (unsigned int i = 0; i < regConns.size(); i++) {
      ...
      if (!haveVisited.at(nextConn.to)) {
        ...
        PATH nextPath =
            PATH(nextNode, min(best.max_radius, nextConn.max_radius),
                 max(best.max_inc_radius, nextNode.max_radius),
                 best.length + nextConn.length);
    ...
    for (unsigned int i = 0; i < sinkConns.size(); i++) {
      ...
      DIJKSTRA_NODE nextNode = dnet->nodes[nextConn.to];
      PATH nextPath = PATH(nextNode, min(best.max_radius, nextConn.max_radius),
                           max(best.max_inc_radius, nextNode.max_radius),
                           best.length + nextConn.length);
 ...
 *  check is made as to whether it is periodically viable (node #N to node
 * #N).*/
...
double findMaxIncludedSphere(VORONOI_NETWORK *vornet) {
  ...
  while (iter != vornet->nodes.end()) {
    if (iter->rad_stat_sphere > max) max = iter->rad_stat_sphere;
    iter++;
  }
  return max;
}

bool betterPath(pair<int, pair<DELTA_POS, double> > p1,
                pair<int, pair<DELTA_POS, double> > p2) {
...
double calculateNodalFreeSphere(DIJKSTRA_NETWORK *dnet, DELTA_POS filter) {
  ...
  while (nodeIndex < nodeProcessed.size()) {
    ...
    heap.insert(pair<int, pair<DELTA_POS, double> >(
        nodeIndex, pair<DELTA_POS, double>(displacement,
                                           dnet->nodes[nodeIndex].max_radius)));

    while (heap.size() != 0) {
      // Remove best remaining path
      pair<int, pair<DELTA_POS, double> > pathInfo = heap.pop();
      map<int, pair<DELTA_POS, double> >::iterator nodeInfo =
          visitedNodes.find(pathInfo.first);

      if (pathInfo.second.second < maxRadius) break;
      ...
      if (nodeInfo != visitedNodes.end()) {
        if (nodeInfo->second.first.equals(pathInfo.second.first)) {
          ...
          bestRadii[nodeIndex] =
              min(pathInfo.second.second, nodeInfo->second.second);
        ...
        while (connIter != currentNode.connections.end()) {
          int to = connIter->to;
          if (nodeProcessed[to]) {
            // Know the best radii for this path because the next node has
            // already been processed Change its node id to -1 to denote this
            heap.insert(pair<int, pair<DELTA_POS, double> >(
                -1, pair<DELTA_POS, double>(
                        DELTA_POS(0, 0, 0),
                        min(connIter->max_radius,
                            min(pathInfo.second.second, bestRadii[to])))));
          } else {
            DELTA_POS newDisplacement =
                pathInfo.second.first + (connIter->deltaPos * filter);
            heap.insert(pair<int, pair<DELTA_POS, double> >(
                to, pair<DELTA_POS, double>(newDisplacement,
                                            min(dnet->nodes[to].max_radius,
                                                min(pathInfo.second.second,
                                                    connIter->max_radius)))));
          }
          connIter++;
        }
 ...
 */
vector<vector<double> > calculateNodeOverlap(ATOM_NETWORK *atmnet,
                                             VORONOI_NETWORK *vornet) {
  unsigned int numNodes = vornet->nodes.size();
  vector<vector<double> > overlapFractions =
      vector<vector<double> >(numNodes, vector<double>(numNodes, 0));
  for (unsigned int i = 0; i < numNodes; i++) {
    ...
    for (unsigned int j = i + 1; j < numNodes; j++) {
      ...
      overlapFractions[i][j] = overlapFractions[j][i] =
          max(0.0, (sumRadii - atmnet->calcDistanceXYZ(n1.x, n1.y, n1.z, n2.x,
                                                       n2.y, n2.z)) /
                       sumRadii);
    }
  }
 ...
 */
vector<set<int> > clusterElements(set<int> cageNodes,
                                  vector<vector<double> > overlapFractions,
                                  double MIN_FRACTION) {
  ...
  while (nodeIter != cageNodes.end()) {
    ...
    if (!merged) {
      ...
      for (unsigned int i = 1; i < clusterIDs.size(); i++) {
        clusters[clusterIDs[0]].insert(clusters[clusterIDs[i]].begin(),
                                       clusters[clusterIDs[i]].end());
      }
      int removeCount = 0;
      for (unsigned int i = 1; i < clusterIDs.size(); i++) {
...
void CAGE::reconstructCage(set<int> ids, ATOM_NETWORK *atmnet,
                           VORONOI_NETWORK *vornet) {
  int idCount = 0;
  set<int>::iterator iter = ids.begin();
  Point pivot = atmnet->xyz_to_abc(
      vornet->nodes[*iter].x, vornet->nodes[*iter].y, vornet->nodes[*iter].z);
  vector<Point> nodeXYZCoords;
  nodeXYZCoords.push_back(Point(vornet->nodes[*iter].x, vornet->nodes[*iter].y,
                                vornet->nodes[*iter].z));
  ...
  while (iter != ids.end()) {
    ...
    Point other = atmnet->xyz_to_abc(
        vornet->nodes[*iter].x, vornet->nodes[*iter].y, vornet->nodes[*iter].z);
    atmnet->getDistCalc().minimum_periodic_distance(
        pivot[0], pivot[1], pivot[2], other[0], other[1], other[2], minDa,
        minDb, minDc);
    ...
    nodeIDs.push_back(*iter);
    nodeOffsets.push_back(DELTA_POS((int)(floor(offset[0] + 0.5)),
                                    (int)(floor(offset[1] + 0.5)),
                                    (int)(floor(offset[2] + 0.5))));
    nodeIndices.insert(pair<int, int>(*iter, idCount));
    iter++;
    idCount++;
  }
  ...
  for (unsigned int i = 0; i < nodeXYZCoords.size(); i++) {
    cx += nodeXYZCoords[i][0];
    cy += nodeXYZCoords[i][1];
    cz += nodeXYZCoords[i][2];
  }
  cx /= nodeXYZCoords.size();
  cy /= nodeXYZCoords.size();
  cz /= nodeXYZCoords.size();

  // Compute the radius of the cage
  iter = ids.begin();
  radius = DBL_MAX;
  for (unsigned int i = 0; i < nodeXYZCoords.size(); i++) {
    radius = min(radius, calcEuclideanDistance(cx, cy, cz, nodeXYZCoords[i][0],
                                               nodeXYZCoords[i][1],
                                               nodeXYZCoords[i][2]) +
                             vornet->nodes[*iter].rad_stat_sphere);
    iter++;
  }
}
...
/* Identify the cages present in Voronoi network for the given probe size. If
 * the visualization option is specified, commands necessary to visualize the
 * cages in ZeoVis is written to the provided file stream. Otherwise, the cages
 * are classifed according to radius and the relevant cage sizes and volume is
 * output using the file stream.
 */
void identifyCages(ATOM_NETWORK *atmnet, VORONOI_NETWORK *vornet,
                   DIJKSTRA_NETWORK *dnet, double probeRadius, bool visualize,
    ...
    while (heap.size() != 0) {
      ...
      if (nodeInfo != visitedNodes.end()) {
        ...
        while (connIter != currentNode.connections.end()) {
          ...
          if (nodeProcessed[to]) {
            // Know the best radii for this path because the next node has
            // already been processed Change the path's node id to -1 to denote
            // this
            heap.insert(pair<int, pair<DELTA_POS, double> >(
                -1, pair<DELTA_POS, double>(
                        DELTA_POS(0, 0, 0),
  ...
  vector<vector<double> > overlapFractions =
      calculateNodeOverlap(atmnet, vornet);

  // Cluster nodes in cages
  vector<set<int> > clusters =
      clusterElements(cageElements, overlapFractions, 0.1);
...
/* Determine cage interconnectivity information by determining which cages are
 * directly connected to one another. */
void simplifyCageNetwork(ATOM_NETWORK *atmnet, VORONOI_NETWORK *vornet,
                         DIJKSTRA_NETWORK *dnet, double probe_radius) {
  fstream output;
  ...
  vector<vector<set<DELTA_POS, bool (*)(DELTA_POS, DELTA_POS)> > >
      cageConnections =
          vector<vector<set<DELTA_POS, bool (*)(DELTA_POS, DELTA_POS)> > >(
              cages.size(),
              vector<set<DELTA_POS, bool (*)(DELTA_POS, DELTA_POS)> >(
                  cages.size(), emptySet));
  for (unsigned int i = 0; i < dnet->nodes.size(); i++) {
      ...
      while (connIter != dnet->nodes[i].connections.end()) {
        ...
        if ((fromID != -1) && (toID != -1)) {
          if (fromID != toID) {
            ...
            DELTA_POS cageDeltaPos = cages[toID].getNodeOffset(connIter->to) -
                                     (cages[fromID].getNodeOffset(i));
  ...
  for (unsigned int i = 0; i < cageConnections.size(); i++) {
    ...
    for (unsigned int j = 0; j < cageConnections[i].size(); j++) {
      if (cageConnections[i][j].size() > 0) {
        cout << "\t ->" << j << "\n";
        set<DELTA_POS, bool (*)(DELTA_POS, DELTA_POS)>::iterator iter =
            cageConnections[i][j].begin();
        while (iter != cageConnections[i][j].end()) {
          DELTA_POS dp = *iter;
          if ((i != j) || (dp.x != 0) || (dp.y != 0) || (dp.z != 0)) {
            cout << "\t\t" << dp.x << " " << dp.y << " " << dp.z << "\n";
            Point other =
                Point(cageCenters[j][0], cageCenters[j][1], cageCenters[j][2]);
            atmnet->translatePoint(&other, dp.x, dp.y, dp.z);
            output << "{line {" << cageCenters[i][0] << " " << cageCenters[i][1]
                   << " " << cageCenters[i][2] << "} "
