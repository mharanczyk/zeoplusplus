/**
 * In material.* files, there are functions to control execution of Material
 *class This is the main class to store information about a structure and its
 *porosity
 *
...
class MATERIAL {
 public:
  ...
  // free sphere stuff
  double Di, Df, Dif;
  vector<double> directionDf;
  vector<double>
      directionDif;  // Df and Dif along 3 crystallographic directions

  // accessible volume ()

  double AVprobeRadius;  // radius of the probe used for sampling (needed for
                         // blocking pockets)
  ...
  vector<Point>
      AVaxsPoints;  // stores accessible points (fractional coordinates)
  ...
  vector<int> AVcount_inChannel;  // stores the number of points assigned to
                                  // particular channel
  vector<int> AVcount_inPocket;   // stores the nubmer of points assigned to
                                  // particular pocket

  vector<double> AVaxsPointsPSD;  // stores diameter of pore associated with
                                  // each accessible point

  bool AVrequestBlockingPockets;  // flag that indicates that blocking spheres
                                  // need to be saved
  bool AVrequestPSD;  // flag that indicates that PSD calculation need to be
                      // executed
  ...
  double ASAprobeRadius;  // radius of the probe used for sampling (for inflated
                          // atoms)
  ...
  vector<Point>
      ASAaxsPoints;  // stores accessible points (fractional coordinates)
  ...
  vector<pair<double, double> >
      ASAhistogramSAperAtom;  // stores SA contributions per atom
  ...
  MATERIAL() {
    radial = true;
    outputZvis = false;
    buildBasicCells = false;
    useMass = true;
    allowAdjustCoordsAndCell = false;

    AVrequestBlockingPockets =
        false;             // by default dont save blocking spheres in AV run
    AVrequestPSD = false;  // by default do not run PSD after AV run

    highAccuracy = true;
    AccSetting = "DEF";
    cout << "-ha setting is default now (DEF setting). Please use \"-noha\" to "
            "override.\n";
  ...
  void PLDcalc(double r, double seg_r, string seg_file, ostream &output,
               char *filename);
