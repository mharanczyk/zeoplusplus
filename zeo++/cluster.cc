...
pair<DnIt, DnIt> get_first_closer_nonassigned_node(const XYZ pt,
                                                   const ATOM_NETWORK& atmnet,
                                                   vector<DIJKSTRA_NODE> nodes,
                                                   const float cutoff_dist) {
  DnIt it = nodes.begin();
  for (; it != nodes.end(); ++it) {
    double period_dist =
        atmnet.calcDistanceXYZ(it->x, it->y, it->z, pt.x, pt.y, pt.z);
    if (!it->active && period_dist < cutoff_dist) break;
  }
  return make_pair(it, nodes.end());
}

/* Function to partition high accuracy voronoi graph into clusters that are
 * close to voronoi nodes in low accuracy cluster. Args: Input: atmnet:
 * ATOM_NETWORK cutoff_dist: Maximum distance from regular voronoi network node
 * that belong to a cluster Output: Returns clusters as vector of VORONOI_NODES
 */
vector<vector<XYZ> > cluster_partition(ATOM_NETWORK* atmnet,
                                       const float cutoff_dist) {
  // Algorithm:
  ...
  for (VnIt nodeit = vornet.nodes.begin(); nodeit != vornet.nodes.end();
       ++nodeit) {
    ...
    pair<DnIt, DnIt> p =
        get_first_closer_nonassigned_node(  // Highaccuracy dijkstra
    ...
    while (!tovisit_ids.empty()) {
      ...
      for (; connit != curr_nd.connections.end(); ++connit) {
        ...
        double dist_to_low_acc_node = atmnet->calcDistanceXYZ(
            nodeit->x, nodeit->y, nodeit->z, to_node.x, to_node.y, to_node.z);
        if (find(visited_ids.begin(), visited_ids.end(), to_nodeid) ==
                visited_ids.end() &&
...
vector<XYZ> cluster_aggregate(const vector<vector<XYZ> >& clusters,
                              const ATOM_NETWORK* atmnet) {
  ...
  for (; clust_it != clusters.end(); ++clust_it) {
    ...
    for (; it != clust_it->end(); ++it) {
      ...
      distcalc.closest_periodic_image(c_pt.vals[0], c_pt.vals[1], c_pt.vals[2],
                                      curr_pt.vals[0], curr_pt.vals[1],
                                      curr_pt.vals[2], tmp_x, tmp_y, tmp_z);
...
/* cluster_aggregate: Reduces a set of points to a single point.
 * The reduction is done thr' computing the centroid of the points */
void cluster_aggregate(const vector<vector<XYZ> >& clusters,
                       const ATOM_NETWORK* atmnet,
                       vector<XYZ>* reduced_clusters) {
  MIN_PER_DISTANCE distcalc = atmnet->getDistCalc();
  // vector<XYZ> reduced_clusters;
...
void high_accuracy_vornodes_reduction(ATOM_NETWORK* atmnet,
                                      Vector_XYZ* vornodes) {
  vector<vector<XYZ> > clusters = cluster_partition(atmnet, 0.2);
 ...
 */
/*
VORONOI_NETWORK cluster_reduce(const VORONOI_NETWORK* vornet, const float
cutoff_dist)
{
    // Algorithm:
    ...
    // Generate new edges linking the remaining voronoi node in the cluster to
other clusters
    ...
    while (nodeiter != dnet.nodes.end()) {
        if (!nodeiter->active){ // Node still not assigned to any cluster
            // Check if the node belongs to any existing cluster by first
checking if a
            // connecting node belongs to cluster and then test the distance to
connecting node for (vector<CONN>::const_iterator conniter =
nodeiter->connections.begin(); conniter != nodeiter->connections.end();
++conniter){ int conn_node = conniter->to; for (vector< vector<int> >::iterator
clusteriter = clusters.begin(); clusteriter != clusters.end(); ++clusteriter) {
                    //If a connecting node belongs to cluster check otherwise
skip vector<int>::iterator it = find(clusteriter->begin(), clusteriter->end(),
conn_node); if (it != clusteriter->end() && conniter->length <= cutoff_dist){
            ...
            if (!nodeiter->active) { // Create a new cluster and assign node to
new cluster vector<int> cluster; cluster.push_back(nodeiter->id);
                clusters.push_back(cluster);
            }
        }
        ++nodeiter;
...
void prune_high_accuracy_voronoi_network(VORONOI_NETWORK* ha_vor,
  ...
  for (AtmIt it = low_atm_net->atoms.begin(); it != low_atm_net->atoms.end();
       ++it) {
  ...
  // for (id_it = high_atm_net->IDmapping.begin(); id_it !=
  // high_atm_net->IDmapping.end(); ++id_it){
  //    cout << *id_it << endl;
  //}
  ...
  cout << "Size of high accuracy atom network " << high_atm_net->atoms.size()
       << endl;

  if (print) {
    cout << "Radii analysis:" << endl;
    cout << "the smallest atom r = " << minr
         << ", while the largest atom r = " << maxr << endl;

    cout << "Length of vornet nodes before pruning: " << ha_vor->nodes.size()
         << endl;
    cout << "Length of vornet edges before pruning: " << ha_vor->edges.size()
         << endl;
  }
  ...
  for (AtmIt it = low_atm_net->atoms.begin(); it != low_atm_net->atoms.end();
    ...
    double a1 = it->a_coord;
    double b1 = it->b_coord;
    double c1 = it->c_coord;
    for (VnIt vnit = ha_vor->nodes.begin(); vnit != ha_vor->nodes.end();) {
      ...
      if (dist <=
          atm_rad - delta) {  // Delete the voronoi node and associated edges
        int node_id = vnit - ha_vor->nodes.begin();
        // Delete the edges with the node_id and decrement the node index when
        // its greater than the id of deleted node
        ...
        // cout << "Length of vornet edges after deleting node: " <<
        // ha_vor->edges.size() << endl;
        vnit = ha_vor->nodes.erase(vnit);
      } else
        ++vnit;
    }
    if (print) {
      cout << "Length of vornet nodes after pruning: " << ha_vor->nodes.size()
           << endl;
      cout << "Length of vornet edges after pruning: " << ha_vor->edges.size()
           << endl;
    }
  }
  return;
...
void nearest_largest_diameter_ha_vornet(VORONOI_NETWORK* ha_vornet,
  ...
  for (VnIt vit = vornet->nodes.begin(); vit != vornet->nodes.end(); ++vit) {
    ...
    double y2 = vit->y;
    double z2 = vit->z;
    for (VnIt vh_it = ha_vornet->nodes.begin(); vh_it != ha_vornet->nodes.end();
         ++vh_it) {
      double x1 = vh_it->x;
      double y1 = vh_it->y;
      double z1 = vh_it->z;
  ...
  // of low accuracy vornet, generate the edges connecting the nodes in the
  // order corresponding to that in the low accuracy voronoi network. Adjust
  // parameters of edges such as length, radius etc
}
...
void simplify_high_accuracy_vornet(VORONOI_NETWORK* ha_vornet,
  ...
  for (VeIt vit = ha_vornet->edges.begin(); vit != ha_vornet->edges.end();
       ++vit) {
  ...
  int i = 0;
  for (VnIt vit = ha_vornet->nodes.begin(); vit != ha_vornet->nodes.end();
       ++vit, ++i) {
    if (find(vbeg, vend, i) != vend) {
      red_vornet->nodes.push_back(*vit);
    } else {
      set<int> original_atmnet_ids;
      for (vector<int>::iterator it = vit->atomIDs.begin();
           it != vit->atomIDs.end(); ++it) {
...
void geometry_pruning(VORONOI_NETWORK* ha_vornet, ATOM_NETWORK* atmnet,
  ...
  for (VnIt vh_it = ha_vornet->nodes.begin(); vh_it != ha_vornet->nodes.end();
    ...
    if (red_vornet->nodes.size() == 0) {
      ...
      double y1 = vh_it->y;
      double z1 = vh_it->z;
      for (VnIt vit = red_vornet->nodes.begin(); vit != red_vornet->nodes.end();
           ++vit) {
        double x2 = vit->x;
        double y2 = vit->y;
        double z2 = vit->z;
...
void ha_prune_within_atom(VORONOI_NETWORK* ha_vornet, ATOM_NETWORK* atmnet,
  ...
  for (VnIt vh_it = ha_vornet->nodes.begin(); vh_it != ha_vornet->nodes.end();
    ...
    if (not near_flag) red_vornet->nodes.push_back(*vh_it);
  }
