...
void MATERIAL::AVcalc(double r, int sampleDensity, ostream &output,
                      char *filename) {
  runVoroFlat();
  accessAnalysis.AccessibilityClassSetup(&atmnet, &orgAtomnet, highAccuracy,
                                         rad_con, &vornet, &bvcells, &cells);
...
void MATERIAL::AVblockPockets(ostream &output) {
  if (AVdoneFlag == false) {
    ...
    if (AVblockDoneFlag == false)
      blockPockets(&atmnet, output, AVaxsPoints, AVaxsPointsChannelIDs,
                   AVinaxsPoints, AVinaxsPointsPocketIDs, AVprobeRadius);
...
void MATERIAL::AVreportPoints(ostream &output) {
  NEWreportPoints(output, &atmnet, &AVaxsPoints, &AVaxsPointsChannelIDs,
                  &AVinaxsPoints, &AVinaxsPointsPocketIDs, VisSetting);
};

void MATERIAL::AVreportPSDPoints(ostream &output) {
  NEWreportPointsValue(output, &atmnet, &AVaxsPoints, &AVaxsPointsChannelIDs,
                       &AVaxsPointsPSD, VisSetting);
};

/* SURFACE AREA FUNCTIONS */

void MATERIAL::ASAcalc(double r, int sampleDensity, ostream &output,
                       char *filename) {
  runVoroFlat();
  accessAnalysis.AccessibilityClassSetup(&atmnet, &orgAtomnet, highAccuracy,
                                         rad_con, &vornet, &bvcells, &cells);
...
void MATERIAL::ASAreportPoints(ostream &output) {
  NEWreportPoints(output, &atmnet, &ASAaxsPoints, &ASAaxsPointsChannelIDs,
                  &ASAinaxsPoints, &ASAinaxsPointsPocketIDs, VisSetting);
};

/* visVORO FUNCTION */
void MATERIAL::visualizeVoroNet(char *name, double r, int skel_a, int skel_b,
                                int skel_c) {
...
// PLDcalc uses seg_r to segment the void space into POREs, then takes the
// network available to r and analysis PLD between the segments identified for
// seg_r
void MATERIAL::PLDcalc(double r, double seg_r, string seg_file, ostream &output,
                       char *filename) {
  runVoroFlat();
  accessAnalysis.AccessibilityClassSetup(&atmnet, &orgAtomnet, highAccuracy,
                                         rad_con, &vornet, &bvcells, &cells);
  ...
  if (seg_r > 0) {  // segment the network based on provide radii of void
    accessAnalysis.calculatePLDbasedOnRadius(seg_r);
    accessAnalysis.reportPLD(output);
  } else {  // segment the network based on provided segment definitions from a
            // file
...
// PLDcalc uses molecules present in the periodic box to segment the void space
// into POREs, then takes the network available to r and analysie PLD between
// the segments
...
void MATERIAL::PLDvisualize(string basefilename, string visformat) {
  ...
  accessAnalysis.getPLDvisData(&NodesFracCoord, &NodesSegmentIDs, &NodesSize,
                               "INITSEGMAP");
  NEWreportPointsValue(output, &atmnet, &NodesFracCoord, &NodesSegmentIDs,
                       &NodesSize, visformat);
  ...
  if (visformat == "LIVERPOOL")
    filename_vis = basefilename + ".lpld_segmentpld";
  output.open(filename_vis.data(), fstream::out);
