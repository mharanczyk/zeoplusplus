...
// TO DO:
// - some net input files are highly irregular, and fail (address this somehow;
// perhaps "node" and "atom" cannot be handled in the ways specified in the cgd
// input routine?); example fails: oft, xaa
...
// global, const, struct
string global_net;
const bool write_old_ml_file =
    false;  // just for debugging, writes out ratio of different building blocks
            // in the structure in an old format
const int MIN_DIM = 2;
typedef struct {
  int seconds;
  int microseconds;
...
void construct_framework_from_alignment(
    vector<pair<int, int> > this_alignment, int index_of_this_framework,
    vector<int> *count_each_molecule, double *previous_lowest_deviation,
    ATOM_NETWORK *framework, int *assembly_method, int num_full_vertices,
    ATOM_NETWORK full_cell, vector<vector<int> > mol_index_at_each_basic_vertex,
    vector<vector<vector<MOLECULE> > > basic_oriented_molecules, string prefix,
    string net_name, vector<CONNECTION> two_way_connections,
    double unit_edge_length, double layer_separation_A, bool layer_override,
    int dimensionality, bool a_periodicity, bool b_periodicity,
    bool c_periodicity, bool disconnected_net);
bool iterate_alignment_code(
    vector<int> *example_alignment_code,
    vector<vector<pair<int, int> > > *alignment_options_each_basic_vertex);
seconds_microseconds get_time_difference(struct timeval start,
                                         struct timeval end);
...
int main(int argc, char *argv[]) {
  bool verbose = false;  // set to true for debugging purposes, to write out
                         // individually aligned building block files, write
                         // extra info to terminal, etc.
  ...
  if (argc < num_args) {
    ...
    printf("./this_code\n");
    printf(
        "net.cgd (a topology definition file - e.g. from the RCSR "
        "http://rcsr.anu.edu.au/)\n");
    printf(
        "augment_2c (0 or 1, representing whether to insert 2-c vertices along "
        "each edge, e.g. for pcu net MOF-5)\n");
    printf("output_file_prefix\n");
    printf(
        "OPTIONAL: at least one mol.xyz, if you want to build a framework - "
        "otherwise only the net will be returned\n");
    printf(
        "OPTIONAL: for 2D (layer) structures, a separation distance in "
        "Angstroms, i.e., the 3rd unit cell side length\n");
  ...
  // by default, we want to populate the table with radii and masses, even if
  // later we will not reference them
  initializeRadTable();
  initializeMassTable();
  ...
  if (argc > num_args) {  // attempt to read layer separation override
    ...
    if (l < TOLERANCE) {  // i.e. if too small to be valid, or if string was not
                          // a number (which gives 0 as result)
      // do nothing
    } else {
  ...
  if (verbose) {
    if (layer_override)
      printf("DEBUG: layer separation override of %.3f was requested\n",
             layer_separation_A);
    else
      printf("DEBUG: no layer separation override was provided\n");
  }
  vector<string> mol_names;
  for (int i = num_args; i < argc; i++) {
    if (layer_override && i == argc - 1) {
      // do nothing - we have determined that the final arg is a layer override
      // and not a file name
    } else {
      string m(argv[i]);
  ...
  bool edges_provided_exhaustively = read_cgd(
      ...
      &global_net);  // we need to know whether the edges were provided
                     // exhaustively in the input file or not, so that we can
                     // reconstruct the missing edges if required!
  ...
  // the dummy edges will be handled later - but only do this if the 2c vertices
  ...
  for (int i = 0; i < num_basic_vertices && !augmented_2c_already; i++) {
    if (basic_cell.vertices.at(i).edges.size() == 2)
      augmented_2c_already =
          true;  // are there already 2c vertices? if so, they were provided in
                 // the input net file, and so we don't need to augment
  }
  if (augment_2c) {
    if (!augmented_2c_already) {
      ...
      printf(
          "did not need to augment net with additional 2-c vertices as "
          "requested, since net file explicitly provided them\n");
  }
  fclose(cgd);
  ...
  num_basic_vertices =
      basic_cell.vertices
          .size();  // update this one incase we added 2c vertices
  ...
  // for each basic vertex, find its symmetric positions and push them to a new
  // cell
  ...
  for (int i = 0; i < num_basic_vertices; i++) {
    ...
    for (int j = 0; j < num_equiv; j++) {
      XYZ here = equiv.at(j);
      // for this symmetry position, we want to know if it is unique before we
      // commit it
      ...
      if (unique) {
        ...
        // this vertex is unique - that means that we also want to get its edges
        // by symmetry!
        int num_e = basic_cell.vertices.at(i).edges.size();
        for (int k = 0; k < num_e; k++) {
          XYZ test_e = basic_cell.vertices.at(i).edges.at(k);
          vector<XYZ> equiv_e =
              GetEquivalentPositions(basic_cell.sym_ID, &test_e);
          v.edges.push_back(equiv_e.at(
              j));  // this is nice - we know that we are working on symmetric
                    // vertex image with index j - so we can just keep the
                    // corresponding edge image - shifting it to unit cell by
                    // the same amount that we used for the vertex!
        }
        ...
        for (int k = 0; k < num_de; k++) {
          ...
          vector<XYZ> equiv_de =
              GetEquivalentPositions(basic_cell.sym_ID, &test_de);
          v.dummy_edges.push_back(equiv_de.at(
              j));  // this is nice - we know that we are working on symmetric
                    // vertex image with index j - so we can just keep the
                    // corresponding edge image - shifting it to unit cell by
                    // the same amount that we used for the vertex!
  ...
  if (verbose)
    printf(
        "by symmetry there are %d non-overlapping vertex positions; in "
        "fractional coords:\n",
        num_full_vertices);
  vector<bool>
  ...
  for (int i = 0; i < num_full_vertices; i++) {
    ...
    if (verbose)
      printf(
          "\t%.3f %.3f %.3f (basic index %d following symmetry operation %d)\n",
          here.x, here.y, here.z, basic_ID,
          full_cell.vertex_symmetry_operators.at(i));
    basic_indices_still_basic.at(basic_ID) = true;
    num_full_edges += full_cell.vertices.at(i).edges.size();
  }

  // now we can remove 'basic' vertices that we now know are redundant by
  // symmetry (probably only happens for 2-c augmented vertices, but run it
  // anyway in the general case, just in case
  ...
  for (int i = 0; i < num_basic_vertices; i++) {
    if (basic_indices_still_basic.at(i)) {
      ...
      new_IDs_for_old_basic_vertex_IDs.push_back(
          -1);  // this means the basic vertex was deleted, and we should not
                // refer to it in future - if we do, we would be referring to a
                // different data point, so putting -1 here makes sure that
                // doesn't happen (we would get seg fault instead of looking at
                // the wrong data point (which might have been seg fault
                // anyway))
  }
  ...
  for (int i = 0; i < num_full_vertices; i++) {
    ...
    full_cell.vertex_basic_indices.at(i) =
        new_IDs_for_old_basic_vertex_IDs.at(old_basic_ID);
  }

  // identify the two-way connections in the full_cell
  vector<CONNECTION> two_way_connections;
  bool disconnected_net =
      find_two_way_connections(&full_cell, &two_way_connections);
  if (disconnected_net)
    printf(
        "NOTICE: the net file comprises disconnected nets - the "
        "connection-based assembly method can not be performed\n");
  ...
  for (int i = 0; i < num_two_way_connections; i++)
    edge_traversed.push_back(false);
  ...
  for (int i = 0; i < 3; i++) is_periodic.push_back(false);
  int dimensionality = recursive_test_dimensionality(
      0, 0, 0, 0, &two_way_connections, &edge_traversed, &vertex_visit_a,
      &vertex_visit_b, &vertex_visit_c, &vertex_visited, &is_periodic);
  ...
  if (dimensionality < MIN_DIM) {
    printf(
        "ERROR: net dimensionality of %d is less than the minimum, %d - cannot "
        "handle this kind of net\n",
        dimensionality, MIN_DIM);
    exit(EXIT_FAILURE);
  }

  // use two-way connections to augment both the basic and full nets, if
  // necessary - but remember, not if we read a special net file which already
  // provides this data
  if (augment_2c && !augmented_2c_already)
    add_2c_dummy_edges(&basic_cell, &full_cell, &two_way_connections);
  ...
  for (int i = 0; i < num_basic_vertices; i++) {
    ...
    if (verbose)
      printf("DEBUG: basic vertex %d was at %.3f %.3f %.3f\n", i,
             basic_cell.vertices.at(i).abc.x, basic_cell.vertices.at(i).abc.y,
             basic_cell.vertices.at(i).abc.z);
    basic_cell.vertices.at(i).abc = here_uc;
    if (verbose)
      printf("DEBUG: basic vertex %d now at %.3f %.3f %.3f\n", i,
             basic_cell.vertices.at(i).abc.x, basic_cell.vertices.at(i).abc.y,
             basic_cell.vertices.at(i).abc.z);
    int num_e = basic_cell.vertices.at(i).edges.size();
    for (int j = 0; j < num_e; j++) {
      ...
      if (verbose)
        printf("DEBUG: basic vertex %d edge %d was at %.3f %.3f %.3f\n", i, j,
               basic_cell.vertices.at(i).edges.at(j).x,
               basic_cell.vertices.at(i).edges.at(j).y,
               basic_cell.vertices.at(i).edges.at(j).z);
      basic_cell.vertices.at(i).edges.at(j) = here_e + shift;
      if (verbose)
        printf("DEBUG: basic vertex %d edge %d now at %.3f %.3f %.3f\n", i, j,
               basic_cell.vertices.at(i).edges.at(j).x,
               basic_cell.vertices.at(i).edges.at(j).y,
               basic_cell.vertices.at(i).edges.at(j).z);
    }
    ...
    for (int j = 0; j < num_dummy_e; j++) {
      ...
      if (verbose)
        printf("DEBUG: basic vertex %d dummy_edge %d was at %.3f %.3f %.3f\n",
               i, j, basic_cell.vertices.at(i).dummy_edges.at(j).x,
               basic_cell.vertices.at(i).dummy_edges.at(j).y,
               basic_cell.vertices.at(i).dummy_edges.at(j).z);
      basic_cell.vertices.at(i).dummy_edges.at(j) = here_e + shift;
      if (verbose)
        printf("DEBUG: basic vertex %d dummy_edge %d now at %.3f %.3f %.3f\n",
               i, j, basic_cell.vertices.at(i).dummy_edges.at(j).x,
               basic_cell.vertices.at(i).dummy_edges.at(j).y,
               basic_cell.vertices.at(i).dummy_edges.at(j).z);
    }
  ...
  // replace "V" with a number in the xyz output files? this will color-code
  ...
  write_vertices(full_v, &full_cell, net_name, rename_vertices_by_connectivity,
                 true);  // is_net? true
  fclose(full_v);
  ...
  if (verbose) {  // extra output files for debugging etc.
    ...
    write_vertices(basic_v, &basic_cell, net_name,
                   rename_vertices_by_connectivity, true);  // is_net? true
    fclose(basic_v);
    printf("\t%d vertices (before symmetry operations) written to %s\n",
           num_basic_vertices, basic_v_name.c_str());

    // write full vertices and edges to cif (edges are shortened, so as to
    // present a 'valid' abstract cif containing non-overlapping dummy atoms)
  ...
  if (num_mols > 0) {
    ...
    for (int i = 0; i < num_mols; i++) {
      ...
      int num_dummy_sites =
          0;  // if a 2-c molecule contains a 'J' atom, this is used to specify
              // the position of the dummy site, i.e. the 'width' of the
              // molecule which is aligned along the dummy edge
        ...
        if (mol.atoms_type.at(j)[0] == 'Q')
          sites.push_back(
              j);  // allow anything beginning with 'Q' to be a site, so that
                   ...
                   // Q could refer to a carboxy connection, while Q1 refers to
                   // a nitrogen connection)
      ...
      printf(
          "\tread molecule ID %d from file %s containing %d sites (and %d "
          "dummy sites)\n",
          i, mol_names.at(i).c_str(), num_sites, num_dummy_sites);
      if (num_sites <= 0) {
        printf(
            "ERROR: molecule read from %s contains no connection sites\nNOTE: "
            "currently, sites are handled as follows: any element symbol "
            "beginning with 'Q' represents a site, and sites in adjacent "
            "molecules are then attempted to be perfectly overlaid; 'J' is "
            "uesd in 2-c molecules if you wish to specify the third, dummy "
            "direction used to align the 2-c molecule to the net\n",
            mol_names.at(i).c_str());
        exit(EXIT_FAILURE);
      } else if (num_sites != 2 && num_dummy_sites != 0) {
        printf(
            "ERROR: %d dummy sites were read, but these are only permitted if "
            "number of sites is 2 (%d sites were read)\n",
            num_dummy_sites, num_sites);
        exit(EXIT_FAILURE);
      } else if (num_dummy_sites != 0 && num_dummy_sites != 1) {
        printf(
            "ERROR: %d dummy sites were read, but only 0 or 1 are permitted\n",
            num_dummy_sites);
        exit(EXIT_FAILURE);
      ...
      // get centre of mass
      get_com(&mol);
      // if the molecule is 2-c, we need at least 3 sites to do rmsd alignment -
      // insert an orthogonal dummy site (if it was not explicitly provided)!
      ...
      count_each_molecule.push_back(
          0);  // this keeps track of how many of this molecule we used - so far
               // none, until we do alignment ...
    }

    // handle bipartite and higher order segmented frameworks - if there are
    // more molecules with a particular connectivity than there are distinct
    // basic vertices, we will need to split up the basic vertices
    vector<vector<int> > indices_of_n_site_basic_vertices,
        indices_of_n_site_full_vertices;
    for (int i = 0; i <= max_num_sites; i++) {
      vector<int> v;
    ...
    for (int i = 0; i < num_basic_vertices; i++) {
      ...
      if (num_sites <= 0) {
        printf(
            "ERROR: basic cell's vertex read from %s contains no connection "
            "sites (edges)\n",
            net_name.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    for (int i = 0; i < num_full_vertices; i++) {
      ...
      if (num_sites <= 0) {
        printf(
            "ERROR: full cell's vertex read from %s contains no connection "
            "sites (edges)\n",
            net_name.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    for (int i = 0; i < max_num_sites; i++) {
      ...
      int num_basic_vertices_here =
          indices_of_n_site_basic_vertices.at(i).size();
      int num_full_vertices_here = indices_of_n_site_full_vertices.at(i).size();
      if (num_mols_here > 0 || num_basic_vertices_here > 0) {
        if (verbose) {
          printf(
              "DEBUG: there are %d molecules and %d basic vertices with %d "
              "sites\n",
        ...
        if (num_mols_here <= 0) {
          printf(
              "ERROR: there are no molecules with %d sites to fit to basic "
              "vertices\n",
              i);
          exit(EXIT_FAILURE);
        } else if (num_basic_vertices_here <= 0) {
          printf(
              "ERROR: there are no basic vertices with %d sites for molecules "
              "to fit to\n",
              i);
          exit(EXIT_FAILURE);
        } else if (num_basic_vertices_here < num_mols_here) {
          printf(
              "there are fewer vertices in the net that are unique by symmetry "
              "(%d) than molecules (%d) with %d sites - a bipartite (or higher "
              "order) segmentation of the net is necessary\n",
              num_basic_vertices_here, num_mols_here, i);
          // which kinds of net segmentation can we deal with, or are valid?
          if (num_full_vertices_here <
              num_mols_here) {  // no matter what we do, there are just not
                                // enough full vertices for the number of
                                // molecules provided
            printf(
                "ERROR: too many molecules (%d) were provided to fit to the %d "
                "vertices (after symmetry operations) with %d sites; there is "
                "no segmentation of the net that will allow a framework to be "
                "constructed\nNOTE: it may be possible to construct this model "
                "by providing a supercell of the net instead\n",
                num_mols_here, num_full_vertices_here, i);
            exit(EXIT_FAILURE);
          } else if (num_basic_vertices_here !=
                     1) {  // e.g., if there are two n-c vertices and three n-c
                           // molecules, it is not clear how to do a
                           // segmentation
            printf(
                "ERROR: currently net segmentation is not handled for anything "
                "other than 1 (%d) symmetrically unique vertices - this may be "
                "introduced in a later release\n",
                num_basic_vertices_here);
            exit(EXIT_FAILURE);
          } else if (num_mols_here !=
                     2) {  // else, we know we have just one basic vertex - but
                           // if we have to assign anything other than two
                           // molecules to it, it is not clear how to do it -
                           // but we can potentially do it recursively
            printf(
                "ERROR: currently net segmentation is not handled for anything "
                "other than 2 (%d) molecules - this may be introduced in a "
                "later release\n",
                num_mols_here);
            exit(EXIT_FAILURE);
          } else {  // else, we know that we have enough full vertices, and
                    // exactly one basic vertex and two molecules
            // add basic vertices to the net until we have one per full vertex
            int basic_vertex_ID = indices_of_n_site_basic_vertices.at(i).at(
                0);  // copy a vertex - safe to take ID 0 since there is exactly
                     // one entry
            while (num_basic_vertices_here < num_full_vertices_here) {
              ...
              // push to basic cell, and update local variables to reference
              // this new vertex
            ...
            // now, these basic vertices are not yet assigned to any full
            // vertices in the framework - need to update the full vertices to
            // reference them
            if (verbose)
              printf(
                  "DEBUG: after adding copies of symmetrically unique "
                  "vertices, there are now %d to which the %d molecules can be "
                  "aligned; need to reference these symmetrically unique "
                  "vertices in the %d vertices after symmetry which have %d "
                  "sites\n",
                  num_basic_vertices_here, num_mols_here,
                  num_full_vertices_here, i);
            for (int j = 0; j < num_full_vertices_here; j++) {
              int this_full_vertex_ID =
                  indices_of_n_site_full_vertices.at(i).at(j);
              if (verbose)
                printf(
                    "\tvertex %d is originally based on symmetrically unique "
                    "vertex %d and symmetry operation %d\n",
                    this_full_vertex_ID,
                    full_cell.vertex_basic_indices.at(this_full_vertex_ID),
                    full_cell.vertex_symmetry_operators.at(
                        this_full_vertex_ID));
              int new_basic_vertex_ID =
                  indices_of_n_site_basic_vertices.at(i).at(j);
              full_cell.vertex_basic_indices.at(this_full_vertex_ID) =
                  new_basic_vertex_ID;
              if (verbose)
                printf(
                    "\t\tnow based on symmetrically unique vertex %d and "
                    "symmetry operation %d\n",
                    full_cell.vertex_basic_indices.at(this_full_vertex_ID),
                    full_cell.vertex_symmetry_operators.at(
                        this_full_vertex_ID));
            }
    ...
    if (verbose)
      printf(
          "DEBUG: attempting to create oriented molecules for each of %d basic "
          "vertices\n",
          num_basic_vertices);
    ...
    printf(
        "there are %d symmetrically unique vertices, which will be fit to "
    ...
    for (int i = 0; i < num_basic_vertices; i++) {
      ...
      if (verbose)
        printf(
            "DEBUG: symmetrically unique vertex ID %d has %d sites - there are "
            "%d molecules satisfying this number of sites\n",
            i, num_sites, num_mols_here);
      if (num_mols_here == -1) {
        printf(
            "ERROR: could not find a molecule with %d sites to fit to this "
            "vertex\n",
            num_sites);
        exit(EXIT_FAILURE);
      }
      for (int j = 0; j < num_mols_here; j++) {
        ...
        vector<MOLECULE> all_fits = get_multiple_best_RMSD_fits(
            original_molecules.at(
                indices_of_n_site_molecules.at(num_sites).at(j)),
            &basic_cell, i, global_net, prefix,
            mol_index);  // get all the permissible fits of our molecule with
                         // this vertex
        printf(
            "\tsymmetrically unique vertex %d has %d possible fits with "
            "molecule ID %d\n",
            i, (int)(all_fits.size()),
            indices_of_n_site_molecules.at(num_sites).at(j));
      ...
      mol_index_at_each_basic_vertex.push_back(
          mol_index_at_each_basic_vertex_here);
    }
    ...
    for (int bv = 0; bv < num_basic_vertices; bv++) {
      ...
      for (int mc = 0; mc < num_molecule_choices_this_basic_vertex; mc++) {
        int num_alignment_choices_this_molecule =
            basic_oriented_molecules.at(bv).at(mc).size();
        for (int ac = 0; ac < num_alignment_choices_this_molecule; ac++) {
          pair<int, int> p;
          p.first = mc;
          p.second = ac;
          if (verbose)
            printf("\tmolecule choice %d alignment choice %d\n", mc, ac);
          alignment_options_this_basic_vertex.push_back(p);
        }
      }
      if (verbose)
        printf("DEBUG: i.e. basic vertex %d has %d alignment options\n", bv,
               (int)(alignment_options_this_basic_vertex.size()));
      if (alignment_options_this_basic_vertex.size() <= 0) {
        printf(
            "ERROR: basic vertex %d has %d alignment options; must be at least "
            "one\n",
            bv, (int)(alignment_options_this_basic_vertex.size()));
        exit(EXIT_FAILURE);
      }
      alignment_options_each_basic_vertex.push_back(
          alignment_options_this_basic_vertex);
    }

    // at this point, we should have a sensible list of the candidate frameworks
    // to build - let's iterate over them and find the best
    ...
    vector<int> example_alignment_code;
    for (int bv = 0; bv < num_basic_vertices; bv++)
      example_alignment_code.push_back(0);
    bool continuing = true;
    while (continuing) {
      vector<pair<int, int> > this_alignment;
      for (int bv = 0; bv < num_basic_vertices;
           bv++) {  // fill in the example_alignment based on the
                    // example_alignment_code
        this_alignment.push_back(alignment_options_each_basic_vertex.at(bv).at(
            example_alignment_code.at(bv)));
      }
      ...
      for (int m = 0; m < num_mols && all_mols_used; m++) {
        ...
        for (int bv = 0; bv < num_basic_vertices && !this_mol_used; bv++) {
          if (m == indices_of_n_site_molecules
                       .at(basic_cell.vertices.at(bv).edges.size())
                       .at(this_alignment.at(bv).first))
            this_mol_used = true;
        }
        all_mols_used = this_mol_used;
      }
      if (all_mols_used) {  // this alignment uses all molecules - but are the
                            // connection sites correct (e.g., do not want to
        ...
        for (int i = 0; i < num_two_way_connections && possible; i++) {
          ...
          MOLECULE m1 = basic_oriented_molecules.at(basic_ID_m1)
                            .at(this_alignment.at(basic_ID_m1).first)
                            .at(this_alignment.at(basic_ID_m1).second);
          int basic_ID_m2 = full_cell.vertex_basic_indices.at(c.v2);
          MOLECULE m2 = basic_oriented_molecules.at(basic_ID_m2)
                            .at(this_alignment.at(basic_ID_m2).first)
                            .at(this_alignment.at(basic_ID_m2).second);
          ...
          if (m1.atoms_type.at(m1_atom_index) !=
              m2.atoms_type.at(m2_atom_index))
            possible = false;
        }
        if (verbose) {
          if (possible)
            printf("NOTICE (decider): this alignment should be possible\n");
          else
            printf("NOTICE (decider): this alignment should NOT be possible\n");
        }
          ...
          if (verbose) {
            ...
            for (int bv = 0; bv < num_basic_vertices; bv++) {
              printf("\t\tbv %d = molecule choice %d alignment choice %d\n", bv,
                     this_alignment.at(bv).first, this_alignment.at(bv).second);
            }
          }
          construct_framework_from_alignment(
              this_alignment, count, &count_each_molecule,
              &previous_lowest_deviation, &framework, &assembly_method,
              num_full_vertices, full_cell, mol_index_at_each_basic_vertex,
              basic_oriented_molecules, prefix, net_name, two_way_connections,
              unit_edge_length, layer_separation_A, layer_override,
              dimensionality, a_periodicity, b_periodicity, c_periodicity,
              disconnected_net);
          count++;
        }
      }
      // now, iterate
      continuing = iterate_alignment_code(&example_alignment_code,
                                          &alignment_options_each_basic_vertex);
    }
    if (count == 0)
      printf(
          "it was not possible to construct any framework models which "
          "utilized all provided molecules\n");
    else
      printf(
          "in total, %d molecule to basic vertex alignment combinations were "
          "used to construct frameworks\n",
          count);
    ...
    if (previous_lowest_deviation > SITE_DEVIATION_WARNING)
      poor_quality_framework = true;
    //    if(framework.vertices.size()>0) {
    if (framework.numAtoms > 0 &&
        (!poor_quality_framework || allow_poor_quality_framework)) {
      printf(
          "the best framework exhibits a maximum site deviation of %.3f, using "
          "the",
          previous_lowest_deviation);
      if (assembly_method == 0)
        printf(" net-");
      else
        printf(" connection-");
      ...
      if (write_old_ml_file) {
        FILE *ml_ratio_verbose =
            fopen((net_name + ".ml").c_str(), "w");  // for debugging etc.
      ...
      FILE *ratio =
          fopen(ratio_file_name.c_str(), "w");  // nicer version for general use
      if (ratio == NULL) {
        printf("ERROR: could not open output ratio file with name %s\n",
      ...
      */
      ...
      printf("\tcrystallographic structure file written to %s\n",
             frame_file_name.c_str());
      //...print cssr ...
      string frame_file_name_labeled =
          prefix + "_framework_labeled.cssr";  // write framework molecule to
                                               // cssr file with labels!
      writeToCSSRLabeled(const_cast<char *>(frame_file_name_labeled.c_str()),
                         &framework);
      printf(
          "\tcrystallographic structure file with labels (if they were "
          "provided in input files) written to %s\n",
          frame_file_name_labeled.c_str());
      //...print vtk ...
      string vtk_file_name =
          prefix + "_framework.vtk";  // write framework's unit cell
      writeToVTK(const_cast<char *>(vtk_file_name.c_str()), &framework);
      printf("\tunit cell written to %s\n", vtk_file_name.c_str());
      //...and print xyz
      string xyz_file_name =
          prefix + "_framework.xyz";  // write framework's atoms
      writeToXYZ(const_cast<char *>(xyz_file_name.c_str()), &framework, false,
                 false);  // falses indicate 'is_supercell' and
      ...
      if (poor_quality_framework && !allow_poor_quality_framework) {
        printf(
            "#####\nREJECTED: this framework has a site deviation larger than "
            "the tolerance (%.3f > %.3f) - framework could be incorrect, "
  ...
  seconds_microseconds time_so_far =
      get_time_difference(time_at_start, time_at_end);
...
void construct_framework_from_alignment(
    ...
    bool c_periodicity, bool disconnected_net) {
  bool verbose = false;
  // we want to use symmetry information on the selected rotated molecules to
  // get molecules oriented to each of the vertices in the full cell
  if (verbose)
    printf(
        "DEBUG: attempting to use symmetry on these oriented molecules for "
        "each of %d full vertices\n",
        num_full_vertices);
  ...
  for (int i = 0; i < num_full_vertices; i++) {
    ...
    if (verbose) {
      ...
      printf(
          "DEBUG: ... ... and this alignment indicates using molecule choice "
          "%d, which gives us molecule ID %d\n",
          mol_choice, mol_index);
      printf("DEBUG: ... ... with alignment choice %d\n", alignment_choice);
    }
    MOLECULE rotated = apply_symmetry_operator(
        basic_oriented_molecules.at(basic_ID)
            .at(mol_choice)
            .at(alignment_choice),
        ...
        &full_cell);  // apply the appropriate symmetry to the appropriate basic
                      ...
                      // apply the sym op in the underlying net fractional
                      // space!
    ...
    if (verbose) {  // write out each rotated molecule for debugging
      string rotmol_name = prefix + "_rotated_molecule_framework_ID_" +
                           convertToString(index_of_this_framework) +
                           "_basic_" + convertToString(basic_ID) + "_sym_" +
                           convertToString(sym_op) +
      ...
      write_molecule(rotmol, &rotated, net_name, basic_ID, sym_op,
                     true);  // oriented to basic vertex basic_ID with symmetry
                             // sym_op - this will be noted in the file - true
                             // indicates 'write out sites'
      fclose(rotmol);
      printf(
          "\tmolecule oriented to full vertex %d (basic vertex %d with "
          "symmetry operation %d) written to %s\n",
          i, basic_ID, sym_op, rotmol_name.c_str());
    }
  }
  if (verbose)
    printf("DEBUG: %d molecules oriented to full vertices were achieved\n",
           (int)(full_oriented_molecules.size()));

  // now, we have all the molecules we need in the correct orientation to
  ...
  */
  ...
  for (int method = 0; method < 2;
    ...
    if (disconnected_net && method == 1) {
      // do nothing - we cannot assemble disconnected nets with the
      // connection-based method
    } else {
      ATOM_NETWORK this_framework;
      vector<MOLECULE> assembled_molecules;
      if (method ==
          0) {  // NEW METHOD: framework based on the preserving the underlying
                // unit cell and positioning all vertices accordingly
        if (verbose)
          printf(
              "\ttrying net-based assembly method: based on preserving the "
              "underlying net unit cell\n");
        // method first needs to know the 'mean_edge_length' of the net, which,
        // as we are assuming edge-transitivity, is essentially just the
        // 'edge_length'
        vector<MOLECULE> assemble_two_molecules;
        double edge_length = determine_edge_length(
            &full_oriented_molecules, &full_cell, &assemble_two_molecules,
            &two_way_connections);
        // go ahead and build, if we can (a valid edge length was identified)
        if (edge_length > 0)
          this_framework = build_framework(&full_oriented_molecules, &full_cell,
                                           &assembled_molecules, edge_length,
                                           unit_edge_length);
      } else {  // OLD METHOD: framework based on 'snapping' vertices together -
                // may not preserve the unit cell
        if (verbose)
          printf(
              "\ttrying connection-based assembly method: based on connecting "
              "adjacent molecules and then inferring a unit cell\n");
        // method produces a 'mean_edge_length'
        double mean_edge_length = -1;
        // go ahead and build
        this_framework = connect_molecules(
            &full_oriented_molecules, &full_cell, &assembled_molecules,
            &two_way_connections, &mean_edge_length, dimensionality,
            a_periodicity, b_periodicity, c_periodicity, unit_edge_length);
      }
      // it's possible we returned an empty/incomplete framework in an error
      // case - need to handle this safely; only proceed if valid framework was
      // provided
      if (this_framework.valid) {
        // at this point, whichever assembly method we used, we have a valid
        // framework given that we have a framework, we need to do/check some
        // more things A) if this is a 2D (layer) structure, and an override for
        // layer separation distance was provided, use it
        if (dimensionality == 2 && layer_override) {
          if (verbose)
            printf(
                "DEBUG: using provided layer separation distance of %.3f A to "
                "override the layer separation provided in the net file\n",
                layer_separation_A);
          int num_override = 0;
          if (!a_periodicity) {
          ...
          if (num_override != 1) {
            printf(
                "ERROR: was expecting to override exactly 1 cell side length "
                "value, but %d were overwritten - this is a bug\n",
                num_override);
            exit(EXIT_FAILURE);
          }
        ...
        // B) check for collision in the framework (i.e. the final unit cell and
        // list of atoms within it) based on list of two way connections and xyz
        // coords of molecule atoms
        bool collision = true;  // assume not a good structure - only if the
                                // matrices are valid do we check for collisions
        if (this_framework.valid &&
            !bypass_collision_detection) {  // check again - layer change may
                                            // have caused a problem
          collision = check_for_collision(&this_framework, &assembled_molecules,
                                          &two_way_connections);
        }
        if (!collision ||
            bypass_collision_detection) {  // if no collisions were incurred in
                                           ...
                                           // 'badness' of the achieved
                                           // framework by looking at the
                                           // maximum distance (respecting the
                                           // periodic boundary) between sites
                                           // that should be connected
          if (verbose)
            printf(
                "DEBUG: no collision was detected in the framework - this is a "
                "candidate for writing out - determining site deviation\n");
          int num_two_way_connections = two_way_connections.size();
          double max_site_deviation = -1;
          for (int i = 0; i < num_two_way_connections; i++) {
            CONNECTION c = two_way_connections.at(i);
            XYZ xyz1 = assembled_molecules.at(c.v1).atoms_xyz.at(
                assembled_molecules.at(c.v1).sites.at(
                    assembled_molecules.at(c.v1).permutation.at(c.e1)));
            XYZ xyz2 = assembled_molecules.at(c.v2).atoms_xyz.at(
                assembled_molecules.at(c.v2).sites.at(
                    assembled_molecules.at(c.v2).permutation.at(c.e2)));
            ...
            if (d > max_site_deviation || max_site_deviation < 0)
              max_site_deviation = d;
          }
          if (verbose)
            printf(
                "DEBUG: maximum site deviation for alignment option %d and "
                "assembly method %d (0 is net-based, 1 is connection-based) is "
                "%.3f\n",
                index_of_this_framework /*alignment_option*/, method,
                max_site_deviation);
          ...
          for (int i = 0; i < num_full_vertices; i++)
            if (full_cell.vertices.at(i).edges.size() < lowest_c ||
                lowest_c == 0) {
              chosen_full_vertex = i;
              lowest_c = full_cell.vertices.at(i).edges.size();
            }
          MOLECULE anchor = assembled_molecules.at(chosen_full_vertex);
          XYZ anchor_CoM(0, 0, 0);
          for (int s = 0; s < anchor.sites.size(); s++)
            anchor_CoM = anchor_CoM + anchor.atoms_xyz.at(anchor.sites.at(s));
          anchor_CoM = anchor_CoM.scale(1.0 / anchor.sites.size());
          XYZ anchor_CoM_abc =
              this_framework.xyz_to_abc_returning_XYZ(anchor_CoM);
          XYZ target_abc = full_cell.vertices.at(chosen_full_vertex).abc;
          XYZ abc_shift = target_abc - anchor_CoM_abc;
          //        for(int a=0; a<this_framework.vertices.size(); a++)
          //        this_framework.vertices.at(a).abc =
          //        trans_to_origuc(this_framework.vertices.at(a).abc+abc_shift);
          for (int a = 0; a < this_framework.atoms.size(); a++) {
            XYZ orig_atom_abc(this_framework.atoms.at(a).a_coord,
                              this_framework.atoms.at(a).b_coord,
                              this_framework.atoms.at(a).c_coord);
            XYZ new_atom_abc = trans_to_origuc(orig_atom_abc + abc_shift);
            XYZ new_atom_xyz =
                this_framework.abc_to_xyz_returning_XYZ(new_atom_abc);
          ...
          printf(
              "\t\ta collision was detected during this framework alignment "
              "option - this is not a candidate for writing out\n");
      } else if (verbose)
        printf(
            "\t\tan incomplete or empty framework was returned - ignoring this "
            "alignment combination\n");
    }
  }

  // now, we have evaluated all possible frameworks, and have a list of
  // candidates - find the one with the lowest max_site_deviation and write it
  // out
  int num_valid_frameworks = all_frameworks.size();
  if (num_valid_frameworks == 0) {
    if (verbose)
      printf(
          "NOTICE: no valid frameworks could be achieved for alignment option "
          "%d using either assembly method\n",
          index_of_this_framework);
  } else if (num_valid_frameworks != all_max_site_deviations.size()) {
    printf(
        "ERROR: %d valid frameworks were produced but an unequal number %d of "
        "maximum site deviations were calculated\n",
    ...
    for (int i = 0; i < num_valid_frameworks; i++) {
      ...
      if (verbose) {
        if (all_framework_assembly_methods.at(i) == 0)
          printf("\tnet-");
        else
          printf("\tconnection-");
        printf(
            "based assembly method results in a maximum site deviation of "
            "%.3f\n",
            dev);
      }
      if (dev < best_deviation || best_deviation < 0) {
        best_deviation = dev;
        index_best_framework = i;
      }
    }
    // now, only if the best framework here is better that anything we had
    // before, do we 'return' it using the pointer
    if (best_deviation < (*previous_lowest_deviation) ||
        (*previous_lowest_deviation) < 0) {
      if (verbose) {
        printf(
            "\tthe new best framework so far has site deviation %.3f, using "
            "the",
            best_deviation);
        if (all_framework_assembly_methods.at(index_best_framework) == 0)
          printf(" net-");
        else
          printf(" connection-");
        printf("based assembly method\n");
      ...
      (*assembly_method) =
          all_framework_assembly_methods.at(index_best_framework);
    }
  }
}
...
// this function attempts to iterate on a provided alignment code, returning
// false if there are no more to explore
bool iterate_alignment_code(
    vector<int> *example_alignment_code,
    vector<vector<pair<int, int> > > *alignment_options_each_basic_vertex) {
  bool done = false;
  bool success = false;
  // iterating the code involves iteratively incrementing the first entry, and
  // if it is not valid, setting it to zero and iterating the next
  int iteration_position = 0;
  while (!done) {
    example_alignment_code->at(iteration_position)++;
    if (example_alignment_code->at(iteration_position) >=
        alignment_options_each_basic_vertex->at(iteration_position)
            .size()) {  // overstepped - set to zero and try the next one
