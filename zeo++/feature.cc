...
int FEATURE::createFeatures(ATOM_NETWORK *uc, VORONOI_NETWORK *origVNet,
                            DIJKSTRA_NETWORK *origDNet, std::fstream &output,
                            int initIndex, char *name) {
  nfeatures = 0;
  ...
  if (nfeatures == 0) {
    std::cerr
        << "Error occurred during channel segmentation. No features were found."
        << "\n"
        << "Exiting..."
        << "\n";
    exit(1);
  ...
  std::vector<std::vector<int> > nodesForFeatures =
      std::vector<std::vector<int> >(nfeatures, std::vector<int>());
  std::vector<FEATURE> featChannels = std::vector<FEATURE>();
  for (unsigned int i = 0; i < feature_node_mapping.size(); i++) {
    nodesForFeatures[feature_node_mapping[i]].push_back(
  ...
  for (int i = 0; i < nfeatures; i++) {
    ...
    std::vector<bool> infoStorage;
    std::vector<CHANNEL> channels;
    findChannels(&featNet, &infoStorage, &channels);
    int dim;
    if (channels.size() != 0) {
      dim = channels[0].dimensionality;
      featChannels.push_back(
          FEATURE(nodesForFeatures[i], origDNet, dim, channels[0].basis));
    } else {
      dim = 0;
      featChannels.push_back(
          FEATURE(nodesForFeatures[i], origDNet, 0, zeroVecs));
    }
    featChannels[i].writeToVMD("feature", initIndex + i, output);
    std::cout << "@@ " << name << " Feature " << initIndex + i
              << " volume = " << featChannels[i].calculateVolume() << "\n";
    featChannels[i].calculateSurfaceArea(uc, &nodes, &feature_node_mapping, i,
                                         name, initIndex + i);
  }
  return nfeatures;
}

int FEATURE::createSegments(ATOM_NETWORK *uc, VORONOI_NETWORK *origVNet,
                            DIJKSTRA_NETWORK *origDNet, std::fstream &output,
                            int initIndex) {
  nsegments = 0;
  ...
  if (nsegments == 0) {
    std::cerr
        << "Error occurred during channel segmentation. No segments were found."
        << "\n"
        << "Exiting..."
        << "\n";
    exit(1);
  ...
  std::vector<std::vector<int> > nodesForSegments =
      std::vector<std::vector<int> >(nsegments, std::vector<int>());
  ...
  for (int i = 0; i < nsegments; i++) {
    ...
    if (channels.size() != 0) {
      dim = channels[0].dimensionality;
      segChannels.push_back(
          CHANNEL(nodesForSegments[i], origDNet, dim, channels[0].basis));
    } else {
      dim = 0;
      segChannels.push_back(
          CHANNEL(nodesForSegments[i], origDNet, 0, zeroVecs));
    }
...
void FEATURE::segmentChannel(ATOM_NETWORK *uc) {
  ...
  */
  ...
  // segment_identify_connections(); // analyzes connectivity of each segment to
  // find segment's 'faces' - connection between segments
  ...
  for(i=0;i<(int)segment_conn.size();i++)
     {
     cout << segment_conn.at(i).from_seg << "   " << segment_conn.at(i).to_seg
  << "   " <<segment_conn.at(i).max_radius << "   " << segment_conn.at(i).merged
  << "\n";
     };

  cout << "\n" << "Total number of segment connecting edges :  "<<
  segment_conn_fulllist.size()  <<"\n";
  ...
  */
  ...
  for(i=0;i<(int)feature_conn.size();i++)
     {
     cout << feature_conn.at(i).from_seg << "   " << feature_conn.at(i).to_seg
  << "   " <<feature_conn.at(i).max_radius << "\n";
     };
  */
...
/* this function needs to have a pointer to unit cell to run distance
 * calcluations please call setUnitCellPointer() prior to this function */
void FEATURE::segment_distBasedSegmentation(ATOM_NETWORK *uc) {
  ...
  do {
    ...
    while (segment_findMaxNode() > -1) {
      ...
      segments[cur_maxId] =
          nsegments;  // assign new segment number to a nw segment
      segment_max_r.push_back(
          nodes.at(cur_maxId).max_radius);  // store maximum r in a segment

      // now assign all nodes with close distance to the current node
      for (i = 0; i < (int)nodes.size(); i++) {
        if (UnitCellPtr->calcDistanceXYZ(
                nodes.at(cur_maxId).x, nodes.at(cur_maxId).y,
                nodes.at(cur_maxId).z, nodes.at(i).x, nodes.at(i).y,
                nodes.at(i).z) < scale_factor * cur_r) {
...
int FEATURE::validateSegment(int segid) {
  ...
  while (nvisited < (int)nodelist.size()) {
    ...
    for (i = 0; i < (int)nodes.at(id).connections.size(); i++) {
      ...
      if (segments[conn_to] == segid &&
          visited[conn_to] == -1)  // if connected to other node within same
                                   // segment and not visited, add to heap
      {
...
void FEATURE::segment_growSegment_start(ATOM_NETWORK *uc, int nodeid) {
  ...
  segment_max_r.push_back(
      nodes.at(nodeid).max_radius);  // store maximum r in a segment
...
public:
  ...
  double max_radius;  // Radius of largest spherical probe that can travel along
edge DELTA_POS deltaPos; // Change in unit cell
*/

/* initnodeid is id of a nod that was a seed for this segment */
void FEATURE::segment_growSegment_cont(ATOM_NETWORK *uc, int initnodeid,
                                       int nodeid) {
  int i, j;
  int conn_to;
  ...
  /* this if makes sure that segment is grown only within the sphere marked by
   * the shere located on the seed node */
  if (uc->calcDistanceXYZ(nodes.at(initnodeid).x, nodes.at(initnodeid).y,
                          nodes.at(initnodeid).z, nodes.at(nodeid).x,
                          nodes.at(nodeid).y, nodes.at(nodeid).z) <
      SPHERE_SCALE * nodes.at(initnodeid).max_radius) {
    ...
    for (i = 0; i < (int)nodes.at(nodeid).connections.size(); i++) {
      ...
      if (segments[conn_to] == -1)  // investigate connection to nodes not yet
                                    // assigned to any segments
      {
      ...
      else if (segments[conn_to] != -1 &&
               segments[conn_to] !=
                   nsegments)  // do if encounter segment-segment boundary
      {
        ...
        segment_conn_fulllist.push_back(
            segcon);  // store every intersegment connection
                      // this will be used to identify features
                      // connecting different segments

        if (segment_conn.size() == 0) {
          ...
          for (j = 0; j < (int)segment_conn.size(); j++) {
            ...
            if ((segment_conn.at(j).from_seg == nsegments &&
                 segment_conn.at(j).to_seg == segments[conn_to]) ||
                (segment_conn.at(j).from_seg == segments[conn_to] &&
                 segment_conn.at(j).to == nsegments)) {
              update_conn_flag = 1;
              if (segment_conn.at(j).max_radius <
                  nodes.at(nodeid).connections.at(i).max_radius)
                segment_conn.at(j).max_radius =
                    nodes.at(nodeid).connections.at(i).max_radius;
            };
            /*        else
                    {
...
/* segment detection algorithm identified only one (major) connection to other
 * segment (with an edge with the largest corresponding r) we need to
 * post-process intersegment connecting nodes (ones with edges connecting to
 * other segments) to obtain this info */

/* INFO
  vector <SEGCONN> segment_conn_fulllist; // lists all connections (edges)
  briging different segments
                                          // this will be used to determine
  topology of segments vector < vector<int> > segment_faces_conn;   // list of
  'faces' for each segment, faces are clusters of edges that connect to another
  segment vector < vector<double> > segment_faces_max_r; // max r for each
  segment's face
*/
void FEATURE::segment_identify_connections() {
  ...
  vector<int>
      face_nodes_in_segment;  // list IDs of nodes assigned to segment and
                              // located on segment's "face", that is nodes with
                              // connections to other segments
  vector<int>
      connected_segment_for_face_nodes_in_segment;  // for each node, if it is a
                                                    // face node, store segment
                                                    // id to which it is
                                                    // connected
  vector<SEGCONN> edges_from_segment;  // list of edges that connect the current
                                       // segment to other segments
  SEGCONN segc;                        // inter-segment connection
  vector<int>
      edges_in_face;  // for each edge connecting to other semgent assign to a
                      // face, this svector stores face id for each edge
  ...
  for (i = 0; i < nsegments; i++) {
    ...
    connected_segment_for_face_nodes_in_segment.resize(nodes_in_segment.size(),
                                                       -1);
    for (j = 0; j < (int)nodes_in_segment.size(); j++) {
      for (k = 0; k < (int)nodes.at(nodes_in_segment[j]).connections.size();
           k++) {
        if (segments[nodes.at(nodes_in_segment[j]).connections.at(k).to] != i) {
          face_nodes_in_segment[j]++;
          connected_segment_for_face_nodes_in_segment[j] =
              segments[nodes.at(nodes_in_segment[j]).connections.at(k).to];

          segc.from = nodes_in_segment[j];  // this is ID id in channel)
          segc.to =
              nodes.at(nodes_in_segment[j]).connections.at(k).to;  // node id in
                                                                   // channel
          segc.from_seg = i;
          segc.to_seg =
              segments[nodes.at(nodes_in_segment[j]).connections.at(k).to];
          segc.max_radius =
              nodes.at(nodes_in_segment[j]).connections.at(k).max_radius;
          segc.deltaPos =
              nodes.at(nodes_in_segment[j]).connections.at(k).deltaPos;
          edges_from_segment.push_back(segc);
        };
      };
    };

    // now connected_segment_for_face_nodes_in_segment has a list of segments
    // each node in the current segment is conneed to (-1 if not) we need now to
    // find "clusters" of these nodes that will correspond to different
    // connections; edges_from_segment stores all edges going out from a segment
    // to other segments
                    ...
                    //   while(nedges_tobe_assigned>-1)
    {
      for (k = 0; k < (int)edges_from_segment.size(); k++) {
        if (edges_in_face[k] == -1)  // find first edge not assigned to face
        {
          ...
          segment_identify_connections_recurr(
              &nodes_in_segment, &edges_in_face, &nedges_tobe_assigned, nfaces,
              neigh_seg, &edges_from_segment, k);
          nfaces++;
        };
        //        nfaces++;
      };
    ...
    /*
       cout << "Segment " << i <<": " << nodes_in_segment.size() << " nodes, "
       << edges_in_face.size() << " edges, " << nfaces << " faces, nodes-to-go "
       << nedges_tobe_assigned << "\n";
       for(k=0;k<(int)edges_from_segment.size();k++)
          {
          cout << "Edge " << edges_from_segment[k].from << "  " <<
       edges_from_segment[k].to << "  Segments:" <<
       edges_from_segment[k].from_seg << "  " << edges_from_segment[k].to_seg <<
       "    "; edges_from_segment[k].deltaPos.print();
          };
    */
...
/* segment_identify_connections_recurr() identifed edges that belong to
 * particular face
 * */
void FEATURE::segment_identify_connections_recurr(
    vector<int> *nodeid, vector<int> *edgeinface, int *nodes_togo,
    int curr_face, int curr_neigh_seg, vector<SEGCONN> *edges, int curr_edge) {
  int i;
  // check node at current edge, if not assigned to face, assign, consider its
  // neighbors

  for (i = 0; i < (int)edges->size(); i++) {
    if (i != curr_edge &&
        edgeinface->at(i) == -1)  // only consider if not yet assigned to face
    {
      // only consider pairs of edges that connect to the same segment
      if (curr_neigh_seg == edges->at(i).to_seg &&
          edges->at(curr_edge).deltaPos.equals(edges->at(i).deltaPos)) {
        // for two edges connecting to another segment, two edges are on the
        // same face only if starting nodes of this face overlap
        ...
        {  // starting nodes of edges overlap, assign edge to the current face
          ...
          *nodes_togo = (*nodes_togo)--;
          segment_identify_connections_recurr(nodeid, edgeinface, nodes_togo,
                                              curr_face, curr_neigh_seg, edges,
                                              i);
        };
...
void FEATURE::update_node_segmentinfo(vector<int> *segid, int *curr_id,
                                      int flag) {
  int i;
...
/* This function does perform "fake" merging - all segments are being copied as
 * features this is done to use all datastrucutres created for features to
 * analyze segments
 */
...
void FEATURE::merge_singlenode_segments() {
  ...
  vector<int>
      tobeextended;  // flag for nodes that will be extended by toher nodes
  ...
  for (i = 0; i < nsegments; i++) {
    ...
    if (nodelist.size() == 1) {
      if (tobeextended[nodelist[0]] ==
          0)  // if the current node is not planned to be extended with other
              // node, merge with others
      {
        tobemerged_segments[i] = 1;  // mark current segment as one to e merged

        for (k = 0; k < (int)nodes.at(nodelist[0]).connections.size();
             k++)  // iterate over all connections to find closes node
        {
          if (k == 0) {
        ...
        if (tobemerged[mink] > -1)
          mink = tobemerged[mink];  // if node to which we are going to merge
                                    // the current node also will be merged,
                                    // merge into the other node
...
/* merge_segments function goes over all identified segments and tries to merge
 * some into larger blocks */
...
void FEATURE::merge_segments() {
   ...
   * Merging criteria - restriction is smaller than % of average max radii for
   * each segment
   * */
  for (i = 0; i < (int)segment_conn.size(); i++) {
    if (segment_conn.at(i).max_radius >
        FEAT_NODE_R_EDGE_R_RATIO * 0.5 *
            (segment_max_r[(segment_conn.at(i).from_seg)] +
             segment_max_r[(segment_conn.at(i).to_seg)]))
  ...
  for (i = 0; i < (int)segment_merge_list.size(); i++) {
    if (feature_max_r[segment_merge_list[i]] < segment_max_r[i])
      feature_max_r[segment_merge_list[i]] = segment_max_r[i];
  };
...
void FEATURE::merge_newSegmentGrow(int segid) {
  ...
  for (k = 0; k < (int)segment_conn.size(); k++) {
    if ((segment_conn.at(k).from_seg == segid ||
         segment_conn.at(k).to_seg == segid) &&
        segment_conn.at(k).merged == 1) {
      if (segment_conn.at(k).from_seg == segid) {
        conn_to = segment_conn.at(k).to_seg;
      } else {
        conn_to = segment_conn.at(k).from_seg;
      };
...
void FEATURE::segment_saveVis(fstream &output) {
  ...
  if (!output.is_open()) {
    cerr << "Error: File stream needed to print segment information was not "
            "open."
         << "\n"
         << "Exiting ..."
         << "\n";
      ...
      for (unsigned int i = 0; i < unitCells.size(); i++) {
        ...
        for (unsigned int j = 0; j < nodeIDs.size(); j++) {
          ...
          if (segments.at(nodeIDs.at(j)) == k) {
            ...
            //        output << "draw sphere {" << xCoord <<  " " << yCoord << "
            //        " << zCoord << "} radius " <<  curNode.max_radius << "\n";
            output << "draw sphere {" << xCoord << " " << yCoord << " "
                   << zCoord << "} radius " << 0.1 << "\n";
...
double FEATURE::calculateVolume() {
  ...
  srand(randSeed2);
  SPHERE sph;
  vector<SPHERE> object;  // a vector of spheres that define object
                          // (segemnt/feature/channel)
  ...
  for (unsigned int i = 0; i < unitCells.size(); i++) {
    ...
    for (unsigned int j = 0; j < nodeIDs.size(); j++) {
      ...
      if (dimensionality <
          1) {  // if feature is not a channel, use reconstrcted feature
        ...
        sph.x = nodes.at(nodeIDs.at(j)).x;
        sph.y = nodes.at(nodeIDs.at(j)).y;
        sph.z = nodes.at(nodeIDs.at(j)).z;
        //           cout << "draw sphere { " << sph.x << "  " << sph.y << "  "
        //           << sph.z << " } radius 0.1 " << "\n";
      };
      ...
      if (object.size() == 1) {  // first sphere added
        min.x = sph.x - sph.r;
        max.x = sph.x + sph.r;
        min.y = sph.y - sph.r;
        max.y = sph.y + sph.r;
  ...
  for (i = 0; i < N_MC_SAMPLE; i++) {
    ...
    for (j = 0; j < (int)object.size(); j++) {
      if (calcEuclideanDistance(pt.x, pt.y, pt.z, object[j].x, object[j].y,
                                object[j].z) < object[j].r) {
        counter++;
        break;
      };
    };
...
void FEATURE::calculateVolumeForAllFeatures(VORONOI_NETWORK *origVNet,
                                            DIJKSTRA_NETWORK *origDNet) {
  vector<vector<int> > nodesForFeatures =
      vector<vector<int> >(nfeatures, vector<int>());
...
double FEATURE::calculateSurfaceArea_OLD(ATOM_NETWORK *uc,
                                         vector<DIJKSTRA_NODE> *ChanNodeList,
                                         vector<int> *featnodemap, int curr_f,
                                         char *name, int fid) {
  ...
  vector<double> SA_holo_otherf;  // as above but excluding surface that is
                                  // covered by other segments
  ...
  vector<SPHERE>
      other_object;  // vectors of spheres corresponding to other segments
  ...
  */
  ...
  for (j = 0; j < (int)ChanNodeList->size(); j++) {
    if (featnodemap->at(j) == curr_f) {
      for (i = 0; i < (int)(ChanNodeList->at(j).connections.size()); i++) {
        if (featnodemap->at(ChanNodeList->at(j).connections.at(i).to) !=
            curr_f) {
          int flag = 0;
          for (k = 0; k < (int)conn_feat.size(); k++) {
            if (conn_feat[k] ==
                featnodemap->at(ChanNodeList->at(j).connections.at(i).to))
              flag = 1;
          };
          if (flag == 0)
            conn_feat.push_back(
  ...
  for (i = 0; i < (int)conn_feat.size(); i++)
    for (j = 0; j < (int)ChanNodeList->size(); j++) {
      if (featnodemap->at(j) == i) {
        Point node;
        node[0] = ChanNodeList->at(j).x;
        node[1] = ChanNodeList->at(j).y;
        node[2] = ChanNodeList->at(j).z;
  ...
  cout << "Outside spheres list contain " << other_object.size()
       << " spheres (of " << ChanNodeList->size() << " nodes)."
       << "\n";
  ...
  for (step = 0; step < NBINS_SA_HOLO;
       step++)  // loop over all bins for surface area holograms
  {
    ...
    for (j = 0; j < (int)object.size(); j++) {
      if (object[j].r > r_tr) {
        counter++;
        break;
      };  // at least one sphere larger than r_tr, sampling makes sense
    };

    if (counter > 0) {  // sampling makes sense
      ...
      for (int i = 0; i < (int)object.size(); i++) {
        ...
        if (object[i].r - r_tr > 0)
          for (int j = 0; j < N_MC_SA_SAMPLE; j++) {
            ...
            double xpoint =
                sin(phi) * cos(theta);  // Don't need abs(sin(phi))
                                        // becase phi is from 0 to pi
            ...
            if (dimensionality < 1) {  // if the feature is not a channel, just
                                       // check overlap with other spheres

              for (k = 0; k < (int)object.size(); k++) {
                if (k != i) {
                  if (calcEuclideanDistance(pt.x, pt.y, pt.z, object[k].x,
                                            object[k].y, object[k].z) <
                      (object[k].r - r_tr) - 0.00001) {
                    overlap = true;
                    break;
                  };
                };
              ...
              if (overlap == false)
                for (int jj = 0; jj < (int)other_object.size(); jj++) {
                  ...
                  double dist = uc->calcDistanceXYZ(
                      p[0], p[1], p[2], other_object[jj].x, other_object[jj].y,
                      other_object[jj].z);
              ...
              // Convert (x,y,z) coordinates to ones relative to the unit cell
              // vectors
              ...
              */
              if (overlap != true) {
                for (k = 0; k < (int)object.size(); k++) {
                  if (k != i) {
                    Point new_abc =
                        uc->xyz_to_abc(object[k].x, object[k].y, object[k].z);
                    double centerDist = uc->calcDistanceABC(
                        new_abc[0], new_abc[1], new_abc[2], abc_coords[0],
                        abc_coords[1], abc_coords[2]);
              ...
              if (overlap == false)
                for (int jk = 0; jk < (int)other_object.size(); jk++) {
                  ...
                  double dist = uc->calcDistanceXYZ(
                      p[0], p[1], p[2], other_object[jk].x, other_object[jk].y,
                      other_object[jk].z);
        ...
        //     cout << " count= " << count << "    cont_otherf= " <<
        //     count_otherf << "\n";

        SA_holo[step] += ((double)count / (double)N_MC_SA_SAMPLE) * 4.0 * PI *
                         pow(object[i].r - r_tr, 2);
        SA_holo_otherf[step] +=
            ((double)count_otherf / (double)N_MC_SA_SAMPLE) * 4.0 * PI *
            pow(object[i].r - r_tr, 2);
...
double FEATURE::calculateSurfaceArea(ATOM_NETWORK *uc,
                                     vector<DIJKSTRA_NODE> *ChanNodeList,
                                     vector<int> *featnodemap, int curr_f,
                                     char *name, int fid) {
  long int i, j, k, counter = 0;
  ...
  // Create object (current feature) and a list of spheres that overlap with the
  // current feature
  ...
  for (j = 0; j < (int)ChanNodeList->size(); j++) {
    if (featnodemap->at(j) == curr_f) {
      ...
      sph.x = ChanNodeList->at(j).x;
      sph.y = ChanNodeList->at(j).y;
      sph.z = ChanNodeList->at(j).z;
  ...
  for (i = 0; i < (int)conn_feat.size(); i++)
    for (j = 0; j < (int)ChanNodeList->size(); j++) {
      if (featnodemap->at(j) == i) {
        sph2.x = ChanNodeList->at(j).x;
        sph2.y = ChanNodeList->at(j).y;
        sph2.z = ChanNodeList->at(j).z;
        sph2.r = ChanNodeList->at(j).max_radius * SPHERE_SCALE;

        int flag = 0;
        for (k = 0; k < (int)object.size(); k++) {
          Point pp;
          pp[0] = ChanNodeList->at(j).x;
          pp[1] = ChanNodeList->at(j).y;
          pp[2] = ChanNodeList->at(j).z;
          Point pt_s = uc->minimizePointDistance(pp, object[k].x, object[k].y,
                                                 object[k].z);  // pt_s shifted
          double d = calcEuclideanDistance(
              pt_s[0], pt_s[1], pt_s[2], object[k].x, object[k].y, object[k].z);
      ...
      for (int i = 0; i < (int)object.size(); i++) {
        ...
        if (object[i].r - r_tr > 0)
          for (int j = 0; j < N_MC_SA_SAMPLE; j++) {
            ...
            for (k = 0; k < (int)object.size(); k++) {
              if (k != i) {
                //             Point pt_s=minimizePointDistance(pt,object[k].x,
                //             object[k].y, object[k].z, uc); // pt_s shifted
                //             double centerDist =
                //             calcEuclideanDistance(pt_s.x,pt_s.y,pt_s.z,object[k].x,
                //             object[k].y, object[k].z);

                double centerDist = uc->calcDistanceXYZ(
                    pt[0], pt[1], pt[2], object[k].x, object[k].y, object[k].z);
            ...
            if (overlap == false)
              for (int jk = 0; jk < (int)other_object.size(); jk++) {
                //           Point
                //           pt_s=minimizePointDistance(pt,other_object[jk].x,other_object[jk].y,other_object[jk].z,uc);
                //            double dist =
                //            calcEuclideanDistance(pt_s.x,pt_s.y,pt_s.z,other_object[jk].x,other_object[jk].y,other_object[jk].z);

                double dist =
                    uc->calcDistanceXYZ(pt[0], pt[1], pt[2], object[jk].x,
                                        object[jk].y, object[jk].z);
