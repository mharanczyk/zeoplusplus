...
class UNIT_CELL {
 ...
 public:
  ...
  UNIT_CELL(double vax, double vbx, double vby, double vcx, double vcy,
            double vcz);
  ...
  /** Transform the coordinates relative to the xyz coordinate system vectors
   * into ones relative to the unit cell vectors and store them using the
   * provided pointers. */
  void xyz_to_abc(double x, double y, double z, double &a, double &b,
                  double &c);
};

/* Class used to store the displacement and energy of a path when it encounters
 * a particular node.*/
class DISP_INFO {
 public:
  bool isReal;
...
const TRIPLET DIRECTIONS[6] = {TRIPLET(1, 0, 0), TRIPLET(-1, 0, 0),
                               TRIPLET(0, 1, 0), TRIPLET(0, -1, 0),
                               TRIPLET(0, 0, 1), TRIPLET(0, 0, -1)};
...
bool hasHigherEnergy(std::pair<TRIPLET, DISP_INFO> p1,
                     std::pair<TRIPLET, DISP_INFO> p2);
