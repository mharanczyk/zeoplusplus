...
void parseFilename(const char *fileName, char *name, char *extension) {
  ...
  if (index == string::npos) {
    ...
    strncpy(name, prefix.data(), prefix.size());
    name[prefix.size()] = '\0';
    strncpy(extension, suffix.data(), suffix.size());
    extension[suffix.size()] = '\0';
  }
}

/** Ensures that the provided file is of type .arc, .car, .cuc, .cssr or .v1 or
 * .cif. Otherwise, an error message is displayed and the program is aborted. */
bool checkInputFile(char *filename) {
  string file(filename);
  string fileTypes[] = {".cuc", ".arc", ".cssr", ".obcssr", ".v1",
                        ".cif", ".car", ".dlp",  ".pdb"};
  int numTypes = 8;

  for (int i = 0; i < numTypes; i++) {
    if (file.find(fileTypes[i]) != string::npos) return true;
  }
...
bool readCIFFile(char *filename, ATOM_NETWORK *cell, bool radial) {
  ...
  int symmetry_Int_Table_number =
      -1;  // set to dummy value so we can check if anything was read
  bool symmetry_equiv_pos_site_id_Flag =
      false;  // this is to read symmetry lines from lines that have sym. id.

  // Try opening the file if it opens proceed with processing
  ifstream ciffile;
  cout << "Opening File: " << filename << endl;
  ciffile.open(filename);
  bool read_a = false, read_b = false, read_c = false, read_alpha = false,
       read_beta = false, read_gamma = false,
       initialized_cell = false;  // keep track of when all cell params are
                                  // parsed, so the cell can be created
  if (ciffile.is_open()) {
    while (!ciffile.eof()) {
      if (read_a && read_b && read_c && read_alpha && read_beta && read_gamma &&
          !initialized_cell) {
        cell->initialize();
      ...
      token = split(line, " ()\r\t");
    exception:  // I needed an easy way to jump out of the _loop command if an
                // unknown command was found
      if (token.size() > 0) {
        ...
        if (token[0].substr(0, 5).compare(list[0]) == 0) {  // name of unit cell
          ...
          read_gamma = true;
        } else if (token[0].compare(list[16]) ==
          ...
          //	        while (token[0].at(0)=='_') { //collect all of the
          // collumn labels
          ...
          if (!tokenized) {
            printf(
                "\n#####\n##### WARNING: parsed a loop in cif file, but data "
                "was not present\n#####\n\n");
          } else {
            // printf("DEBUG: exited loop successfully, having read data line
            // \"%s\"\n", line.c_str()); collecting the data associated with
            // each column and put in correct place
            token = split(line, " ,'\r\t");  // This is needed to split data
            bool need_to_convert_to_fractional = false;
            while (token.size() > 0) {
              if (token[0].at(0) == '_' || token[0].compare("loop_") == 0 ||
                  token[0].at(0) == '#') {
                goto exception;  // unexpected input
              }
              if (token.size() != column_labels.size() &&
                  column_labels[0].compare(list[8]) != 0 &&
                  column_labels[0].compare(list[9]) != 0 &&
                  column_labels[0].compare(list[20]) != 0) {
                goto exception;  // unexpected input
              }
              for (unsigned int i = 0; i < column_labels.size(); i++) {
                switch (strCmpList(list, column_labels[i])) {
                  ...
                  case 9:  //_space_group_symop_operation_xyz have the same
                           // meaning
                           // printf("SYM DEBUG: symmetry section found!\n");
                    if (!((token.size() == 3 &&
                           symmetry_equiv_pos_site_id_Flag == false) ||
                          (token.size() == 4 &&
                           symmetry_equiv_pos_site_id_Flag == true))) {
                      cerr << "Error: Expected 3 strings for "
                              "_symmetry_equiv_pos_as_xyz (or 4 if "
                              "_symmetry_equiv_pos_site_id is present)"
                           << endl;
                      //	              abort();
                      ciffile.close();
                      return false;
                  ...
                  case 12:
                    atom_x.push_back(
                        trans_to_origuc(convertToDouble(token[i])));
                    break;
                  case 13:
                    atom_y.push_back(
                        trans_to_origuc(convertToDouble(token[i])));
              ...
              // now we've finished parsing this line, we might need to convert
              // Cartesian to fractional coords
              if (need_to_convert_to_fractional) {
                Point tempcoor;
                int this_ID = atom_x.size() - 1;
                tempcoor = cell->xyz_to_abc(
                    atom_x.at(this_ID), atom_y.at(this_ID), atom_z.at(this_ID));
                // printf("DEBUG: atom at Cartesian %.3f %.3f %.3f written to
                // fractional %.3f %.3f %.3f\n",
                // atom_x.at(this_ID),atom_y.at(this_ID),atom_z.at(this_ID),
    ...
    // If no symmetry info was provided, we can assume that none is required
    // (i.e., structure has 'P 1' symmetry), ONLY IF we did not read a
    ...
    if (sym_x.size() == 0 || sym_y.size() == 0 || sym_z.size() == 0) {
      ...
      if (symmetry_Int_Table_number >= 0 && symmetry_Int_Table_number != 1) {
        ...
        fprintf(
            ...
            "proceed with insufficient symmetry information\nExiting ...\n",
    ...
    // Now determine whether it is correct to use atom_label or atom_type
    // (atom_label used only if atom_type has no data)
    ...
    // Parse out the numbers from atom labels and types, if specified (defined
    // in network.h)
    ...
    if (atom_label.size() == 0 && CIF_USE_LABEL == true) {
      cerr << "Error: No ''_atom_site_label'' or ''_atom_site_type_symbol'' "
              "information "
           << endl;
      cerr << "This structure appears to be invalid: there are no atom "
              "identifying element types or labels - if this is not the case, "
              "then this is a bug; please contact the developers."
           << endl;
      return false;
    }
    ...
    if (atom_x.size() != atom_y.size() || atom_y.size() != atom_z.size() ||
        atom_x.size() == 0) {
      cerr << "Error: Atom coordinates not read properly (" << atom_x.size()
           << " x coords, " << atom_y.size() << " y coords, " << atom_z.size()
           << " z coords)" << endl;
      //      abort();
      return false;
    }
    ...
    //    cell->initialize(); //this should now happen much earlier, during
    //    parsing
    ...
    for (unsigned int i = 0; i < atom_x.size(); i++) {  // atoms
      ...
      for (unsigned int j = 0; j < sym_x.size(); j++) {  // symmetries
        tempatom.a_coord = trans_to_origuc(
            symbCalc(sym_x[j], atom_x[i], atom_y[i], atom_z[i]));
        tempatom.b_coord = trans_to_origuc(
            symbCalc(sym_y[j], atom_x[i], atom_y[i], atom_z[i]));
        tempatom.c_coord = trans_to_origuc(
            symbCalc(sym_z[j], atom_x[i], atom_y[i], atom_z[i]));
        tempcoor = cell->abc_to_xyz(tempatom.a_coord, tempatom.b_coord,
                                    tempatom.c_coord);
        ...
        for (unsigned int k = 0; k < cell->atoms.size(); k++) {
          if (cell->calcDistance(cell->atoms[k], tempatom) < thresholdLarge) {
            match = 0;
            /*
                            if (tempatom.a_coord-thresholdLarge <
               cell->atoms[k].a_coord && cell->atoms[k].a_coord <
               tempatom.a_coord+thresholdLarge) if
               (tempatom.b_coord-thresholdLarge < cell->atoms[k].b_coord &&
               cell->atoms[k].b_coord < tempatom.b_coord+thresholdLarge ) if
               (tempatom.c_coord-thresholdLarge < cell->atoms[k].c_coord &&
               cell->atoms[k].c_coord < tempatom.c_coord+thresholdLarge ){
                        match=0;
            */
          }
        }
  ...
  if (input == NULL) {
    ...
    while (found_final == 0) {
      if (fgets(this_line, 500, input) != NULL) {
        ...
        if (status != -1) {
          if (strcmp(str1, "FINAL") == 0 && strcmp(str2, "GEOMETRY") == 0 &&
              strcmp(str3, "OBTAINED") == 0) {
            found_final = 1;
          }
        }
      } else {
    ...
    while (found_atoms == 0) {
      if (fgets(this_line, 500, input) != NULL) {
        int status = sscanf(this_line, "%s %lf %s %lf %s %lf %s %lf", element,
                            &x, str1, &y, str2, &z, str3, &charge);
        if (status == 8) {  // i.e. exactly 8 fields are required to be read
          found_atoms = 1;
        }
      } else {
        printf(
            "ERROR: finished parsing ARC file before finding individual atom "
            "information\n");
        //        exit(EXIT_FAILURE);
        fclose(input);
        return false;
    ...
    cell->gamma = gamma * 360.0 /
                  (2.0 * PI);  // required since it expects to store degrees
    ...
    for (int i = 0; i < numAtoms; i++) {
      Point newCoords = cell->xyz_to_abc(
          cell->atoms.at(i).x, cell->atoms.at(i).y, cell->atoms.at(i).z);
      cell->atoms.at(i).a_coord = trans_to_origuc(newCoords[0]);
      cell->atoms.at(i).b_coord = trans_to_origuc(newCoords[1]);
      cell->atoms.at(i).c_coord = trans_to_origuc(newCoords[2]);
      newCoords =
          cell->abc_to_xyz(cell->atoms.at(i).a_coord, cell->atoms.at(i).b_coord,
                           cell->atoms.at(i).c_coord);
      cell->atoms.at(i).x = newCoords[0];
      cell->atoms.at(i).y = newCoords[1];
      cell->atoms.at(i).z = newCoords[2];
      // cout << i << "  " << cell->atoms.at(i).type << "  " << newCoords[0] <<
...
bool readCUCFile(char *filename, ATOM_NETWORK *cell, bool radial) {
  ...
  if (!input.is_open()) {
    ...
    while (!input.eof()) {
      ...
      Point newCoords =
          cell->abc_to_xyz(newAtom.a_coord, newAtom.b_coord, newAtom.c_coord);
...
bool readCSSRFile(char *filename, ATOM_NETWORK *cell, bool radial) {
  ...
  if (input.is_open() == true) {
    ...
    string numStr;
    bool longCSSR =
        false;  // this flag wiill enable switching to a different read routine
    ...
    if (longCSSR == false) {
      ...
      for (j = 0; j < cell->numAtoms; j++) {
        ...
        if (CartCoords == false) {
          input >> newAtom.specialID >> newAtom.type >> newAtom.a_coord >>
              newAtom.b_coord >> newAtom.c_coord;
          newAtom.a_coord = trans_to_origuc(newAtom.a_coord);
          newAtom.b_coord = trans_to_origuc(newAtom.b_coord);
          newAtom.c_coord = trans_to_origuc(newAtom.c_coord);
          Point newCoords = cell->abc_to_xyz(newAtom.a_coord, newAtom.b_coord,
                                             newAtom.c_coord);
          newAtom.x = newCoords[0];
          newAtom.y = newCoords[1];
          newAtom.z = newCoords[2];
        } else {  // read-in cartesian
          input >> newAtom.specialID >> newAtom.type >> newAtom.x >>
              newAtom.y >> newAtom.z;
          Point newCoords = cell->xyz_to_abc(newAtom.x, newAtom.y, newAtom.z);
          newAtom.a_coord = newCoords[0];
          newAtom.b_coord = newCoords[1];
          newAtom.c_coord = newCoords[2];
          newAtom.a_coord = trans_to_origuc(newAtom.a_coord);
          newAtom.b_coord = trans_to_origuc(newAtom.b_coord);
          newAtom.c_coord = trans_to_origuc(newAtom.c_coord);
          newCoords = cell->abc_to_xyz(newAtom.a_coord, newAtom.b_coord,
                                       newAtom.c_coord);
          newAtom.x = newCoords[0];
          newAtom.y = newCoords[1];
          newAtom.z = newCoords[2];
        ...
        cell->atoms.push_back(newAtom);
        //        getline(input,garbage);
        // read these numbers after the coords, since the final field contains
        // charge information
        int empty_int = 0;
      ...
      while (!input.eof()) {
        ...
        if (CartCoords == false) {
          input >> newAtom.type >> newAtom.a_coord >> newAtom.b_coord >>
              newAtom.c_coord;
          newAtom.a_coord = trans_to_origuc(newAtom.a_coord);
          newAtom.b_coord = trans_to_origuc(newAtom.b_coord);
          newAtom.c_coord = trans_to_origuc(newAtom.c_coord);
        ...
        input >> newAtom.charge;
        cell->atoms.push_back(newAtom);
        //        cout << na << "  " << newAtom.type << " " << newAtom.a_coord
        //        << "  " << newAtom.b_coord << "  " << newAtom.c_coord << endl;
        na++;
      };
      cell->numAtoms = na;
...
bool readOBCSSRFile(char *filename, ATOM_NETWORK *cell, bool radial) {
  ...
  if (input.is_open() == true) {
    ...
    cout << "Attempt to read OpenBabel CSSR file. Atom connectivity and charge "
...
bool readPDBFile(char *filename, ATOM_NETWORK *cell, bool radial) {
  ...
  if (input.is_open() == true) {
    ...
    if (PBCline.compare("CRYST1") != 0) {
      cerr << "This .pdb files does not contain CRYST1 in the second line. "
              "File format not compatible. Exiting...\n";
      return false;
    };
    ...
    while (!end) {
      ...
      if (str1.compare("ENDMDL") == 0) {
        ...
        input >> newAtom.x >> newAtom.y >> newAtom.z;
        input >> str2 >> str3 >>
            str4;  // ignore 2 numbers and a string with label
...
bool readV1File(char *filename, ATOM_NETWORK *cell, bool radial) {
  ...
  if (!input.is_open()) {
    ...
    cell->c = sqrt(cell->v_c.x * cell->v_c.x + cell->v_c.y * cell->v_c.y +
                   cell->v_c.z * cell->v_c.z);
    cell->beta = acos(cell->v_c.x / cell->c) * 360.0 / (2.0 * PI);
    cell->gamma = acos(cell->v_b.x / cell->b) * 360.0 / (2.0 * PI);
    cell->alpha =
        360.0 / (2 * PI) *
        acos((cell->v_c.y / cell->c * sin(2.0 * PI * cell->gamma / 360.0)) +
             cos(2.0 * PI / 360.0 * cell->gamma) *
                 cos(2.0 * PI / 360.0 * cell->beta));
    ...
    for (i = 0; i < cell->numAtoms; i++) {
      ...
      newAtom.a_coord = trans_to_origuc(abcCoords[0]);
      newAtom.b_coord = trans_to_origuc(abcCoords[1]);
      newAtom.c_coord = trans_to_origuc(abcCoords[2]);
      newAtom.radius = lookupRadius(newAtom.type, radial);
      cell->atoms.push_back(newAtom);
    }
    input.close();
...
/** Read the VORONOI_NETWORK information located in the provided input stream
 * and store it using the provided VORONOI_NETWORK pointer. The input stream
 * must have a file format corresponding to a .net or .nt2 format. */
void readNet(istream *input, VORONOI_NETWORK *vornet) {
  ...
  while (true) {
    (*input) >> garbage;
    if (strcmp(garbage.data(), "Edge") == 0) break;
...
bool writeToCSSR(char *filename, ATOM_NETWORK *cell) {
  ...
  if (!output.is_open()) {
    ...
    output << "\t\t\t\t" << cell->a << "  " << cell->b << "  " << cell->c
           << "\n";
    output << "\t\t" << cell->alpha << "  " << cell->beta << "  " << cell->gamma
           << "  SPGR =  1 P 1\t\t OPT = 1"
           << "\n";
    ...
    for (i = 0; i < cell->numAtoms; i++) {
      atm = cell->atoms.at(i);
      output << " " << i + 1 << " " << cell->atoms.at(i).type << " "
             << atm.a_coord << " " << atm.b_coord << " " << atm.c_coord
             << "  0  0  0  0  0  0  0  0  " << atm.charge << "\n";
    }
    output.close();
    return true;
  }
...
bool writeToCSSRLabeled(char *filename, ATOM_NETWORK *cell) {
  ...
  if (!output.is_open()) {
    ...
    for (i = 0; i < cell->numAtoms; i++) {
      atm = cell->atoms.at(i);
      output << " " << i + 1 << " " << cell->atoms.at(i).label << " "
             << atm.a_coord << " " << atm.b_coord << " " << atm.c_coord
             << "  0  0  0  0  0  0  0  0  " << atm.charge << "\n";
    }
    output.close();
...
string get_formula(ATOM_NETWORK const *const cell) {
  ...
  for (vector<ATOM>::const_iterator it = cell->atoms.begin();
       it != cell->atoms.end(); ++it) {
  ...
  string formula;
  for (map<string, int>::iterator it = typecount.begin(); it != typecount.end();
       ++it) {
...
bool writeToCIF(char *filename, ATOM_NETWORK *cell) {
  ...
  if (!output.is_open()) {
    ...
    cout << "Writing atom network information to " << filename << "\n";
    // output << "data_" << origname << endl; //we need this data_(name) line
    // otherwise the cif file will not be considered valid in some software
    // packages Instead of supplying argument to write data_..., using timestamp
    // with number of atoms in the structure
    ...
    output << "_symmetry_space_group_name_H-M\t\t"
           << "'P1'" << endl;
    output << "_symmetry_Int_Tables_number\t\t"
           << "1" << endl;
    ...
    for (unsigned int i = 0; i < cell->atoms.size(); i++) {
      ...
      output << temp->specialID << "\t" << temp->type << "\t"
             << trans_to_origuc(temp->a_coord) << "\t"
             << trans_to_origuc(temp->b_coord) << "\t"
             << trans_to_origuc(temp->c_coord) << endl;
    }
    output.close();
    return true;
  }
...
/** Write the information stored within the VORONOI_NETWORK in a .nt2
    ...
    less than the provided threshold. For the default 0, all nodes and endges
   are included*/
  ...
  if (!output.is_open()) {
    ...
    while (niter != vornet->nodes.end()) {
      if (niter->rad_stat_sphere > minRad) {
        output << i << " " << niter->x << " " << niter->y << " " << niter->z
               << " " << niter->rad_stat_sphere;
        ...
        for (unsigned int j = 0; j < niter->atomIDs.size(); j++) {
          ...
          if (j < niter->atomIDs.size() - 1) output << " ";
        }
        output << "\n";
      }
      i++;
    ...
    while (eiter != vornet->edges.end()) {
      if (eiter->rad_moving_sphere > minRad) {
        output << eiter->from << " -> " << eiter->to << " "
               << eiter->rad_moving_sphere << " " << eiter->delta_uc_x << " "
               << eiter->delta_uc_y << " " << eiter->delta_uc_z << " "
               << eiter->length << "\n";
      }
      eiter++;
    }
...
/** Write the voronoi noide information within the VORONOI_NETWORK in .xyz
    ...
    less than the provided threshold. For the default 0, all nodes are
   included*/
bool writeToXYZ(char *filename, VORONOI_NETWORK *vornet, double minRad) {
  ...
  if (!output.is_open()) {
    ...
    for (vector<VOR_NODE>::const_iterator iter = vornet->nodes.begin();
         iter != vornet->nodes.end(); iter++)
      if (iter->rad_stat_sphere > minRad)
        output << "X  " << iter->x << " " << iter->y << " " << iter->z << " "
               << iter->rad_stat_sphere << "\n";
  }
  output.close();
  return true;
}
...
/** Write the information within the provided ATOM_NETWORK in a .xyz
    file format to the provided filename. */
// updated this function to permit supercell (2x2x2) and duplication of atoms on
// the unit cell perimeter, if desired
bool writeToXYZ(char *filename, ATOM_NETWORK *cell, bool is_supercell,
                bool is_duplicate_perimeter_atoms) {
  ...
  if (!output.is_open()) {
    ...
    // 1) construct a new vector of atoms, which contains any supercell
    // projections and duplicates
      ...
      for (int a = 0; a < num_cells; a++) {
        for (int b = 0; b < num_cells; b++) {
          for (int c = 0; c < num_cells; c++) {
            ...
            // now we have this atom, be it the original or a supercell
            // projection, find the duplicates
            if (is_duplicate_perimeter_atoms) {
              ...
              if (atom.a_coord > ((double)(num_cells)) - dupe_thresh &&
                  atom.b_coord > ((double)(num_cells)) - dupe_thresh) {
                ATOM dupe = atom;
                dupe.a_coord = atom.a_coord - num_cells;
                dupe.b_coord = atom.b_coord - num_cells;
                new_atoms.push_back(dupe);
              ...
              if (atom.a_coord > ((double)(num_cells)) - dupe_thresh &&
                  atom.c_coord > ((double)(num_cells)) - dupe_thresh) {
                ATOM dupe = atom;
                dupe.a_coord = atom.a_coord - num_cells;
                dupe.c_coord = atom.c_coord - num_cells;
                new_atoms.push_back(dupe);
    ...
    for (int i = 0; i < new_atoms.size(); i++) {
      Point p =
          cell->abc_to_xyz(new_atoms.at(i).a_coord, new_atoms.at(i).b_coord,
                           new_atoms.at(i).c_coord);
      output << new_atoms.at(i).type << " " << p[0] << " " << p[1] << " "
             << p[2] << "\n";
    }
  }
  output.close();
...
/** Write the boundary of the unit cell expressed within the provided
   ATOM_NETWORK in a .vtk file format to the provided filename. */
bool writeToVTK(char *filename, ATOM_NETWORK *cell) {
  ...
  if (!output.is_open()) {
    ...
    Point p;
    p = cell->abc_to_xyz(0, 0, 0);
    corners.push_back(p);
    p = cell->abc_to_xyz(0, 0, 1);
    corners.push_back(p);
    p = cell->abc_to_xyz(0, 1, 0);
    corners.push_back(p);
    p = cell->abc_to_xyz(0, 1, 1);
    corners.push_back(p);
    p = cell->abc_to_xyz(1, 0, 0);
    corners.push_back(p);
    p = cell->abc_to_xyz(1, 0, 1);
    corners.push_back(p);
    p = cell->abc_to_xyz(1, 1, 0);
    corners.push_back(p);
    p = cell->abc_to_xyz(1, 1, 1);
    corners.push_back(p);
    // Write header, information about the cell, and footer
    output << "# vtk DataFile Version 2.0\nvtk format representation of unit "
              "cell boundary\nASCII\nDATASET POLYDATA\nPOINTS 8 double\n";
    for (int i = 0; i < 8; i++)
      output << corners.at(i)[0] << " " << corners.at(i)[1] << " "
             << corners.at(i)[2] << "\n";
    output << "LINES 12 36\n2 0 1\n2 0 2\n2 1 3\n2 2 3\n2 4 5\n2 4 6\n2 5 7\n2 "
              "6 7\n2 0 4\n2 1 5\n2 2 6\n2 3 7\n";
  }
...
/** Write the information within the provided ATOM_NETWORK in a .mop
    file format to the provided filename. */
/** In .mop file, unit cell vectors are listed as Tv and the shape of the unit
 * cell is kept fixed **/
  ...
  if (!output.is_open()) {
    ...
    */
      ...
      for (int a = 0; a < num_cells; a++) {
        for (int b = 0; b < num_cells; b++) {
          for (int c = 0; c < num_cells; c++) {
            ...
            Point p =
                cell->abc_to_xyz(atom.a_coord, atom.b_coord, atom.c_coord);
            output << atom.type << "  " << p[0] << " +1 " << p[1] << " +1 "
                   << p[2] << " +1\n";
          }
        }
      }
    ...
    if (cell->v_a.y == 0.0) {
      ...
      output << cell->v_a.y * num_cells << " +1 ";
    };
    if (cell->v_a.z == 0.0) {
      output << " 0.0 0 \n";
    } else {
      output << cell->v_a.z * num_cells << " +1 \n";
    };
    ...
    if (cell->v_b.x == 0.0) {
      ...
      output << cell->v_b.x * num_cells << " +1 ";
    };
    output << cell->v_b.y * num_cells << " +1 ";
    if (cell->v_b.z == 0.0) {
      output << " 0.0 0 \n";
    } else {
      output << cell->v_b.z * num_cells << " +1 \n";
    };
    ...
    if (cell->v_c.x == 0.0) {
      ...
      output << cell->v_c.x * num_cells << " +1 ";
    };
    if (cell->v_c.y == 0.0) {
      output << " 0.0 0 ";
    } else {
      output << cell->v_c.y * num_cells << " +1 ";
    };
