 ...
 */
void setup_rotation(double ref_xlist[][3], double mov_xlist[][3], int n_list,
                    double mov_com[3], double mov_to_ref[3], double R[3][3],
                    double* E0) {
  int i, j, n;
  double ref_com[3];
  ...
  *E0 = 0.0;
  ...
  for (n = 0; n < n_list; n++) {
    ...
    for (i = 0; i < 3; i++)
      *E0 +=
          mov_xlist[n][i] * mov_xlist[n][i] + ref_xlist[n][i] * ref_xlist[n][i];
 ...
 */
int diagonalize_symmetric(double matrix[3][3], double eigen_vec[3][3],
                          double eigenval[3]) {
  int n_rot, i, j, k;
  double vec[3][3];
  double val;
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) eigen_vec[i][j] = vec[j][i];
 ...
 */
int calculate_rotation_matrix(double R[3][3], double U[3][3], double E0,
                              double* residual) {
  int i, j, k;
