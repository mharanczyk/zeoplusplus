...
/** Using |Point_0 - center| - radius^2 = 0 and Point_0 = t(Ray_dir) + Ray_pnt.
   Solve for t which is the distance. **/
void Sphere::hitSphere(ray r, hitdata& hitsphere) {
  Point newCenter =
      center - r.base;  // map the rand_ray to the origin and move system
  double temp = (r.vector * newCenter) * (r.vector * newCenter) -
                (newCenter * newCenter) + radius * radius;
  if (temp > 0)  // check inside of sqrt is not negative
  {
    if ((r.vector * newCenter) - sqrt(temp) >
        0)  // If it hits the sphere twice pick the closest
    {
      hitsphere.hit = true;
      hitsphere.dist = (r.vector * newCenter) - sqrt(temp);
      hitsphere.hitpoint = r.base + r.vector.scale(hitsphere.dist);
    } else if ((r.vector * newCenter) + sqrt(temp) >
               0)  // Ray was inside sphere (this is possible in some cases)
    {
      hitsphere.hit = true;
      hitsphere.dist = (r.vector * newCenter) + sqrt(temp);
      hitsphere.hitpoint = r.base + r.vector.scale(hitsphere.dist);
...
/** Using normal dot (Point_Plane - Point_0| = 0 and Point_Plane = t(Ray_dir) +
 * Ray_pnt. Solve for t which is the distance to plane. **/
void Plane::hitPlane(ray r, hitdata& hitplane) {
  if (normal * r.vector >=
      0) {  // ray is either parallel or infront of the plane
    return;
  }

  double dist = ((normal * point) - (normal * r.base)) / (normal * r.vector);
  if (dist < 0) {  // There is no reason that the distance should be negative
    cerr << "Error: Distance = " << dist
         << " This means ray got outside of unitcell" << endl;
...
double Plane::distToPlane(Point pnt) { return fabs(normal * (pnt - point)); }

/** Returns a class hitdata which has important information of the closest
 * object hit. Function calls hitSphere for each sphere in the list **/
  ...
  for (unsigned int i = 0; i < s.size(); i++) {
    ...
    if (hitsphere.hit == true) {
      if (hitobject.hit == false ||
          hitobject.dist > hitsphere.dist)  // Conditions for this to be closest
                                            // sphere found so far
      {
        hitobject.hit = true;
...
/** Returns a class hitdata which has important information of the closest
 * object hit. Function calls hitPlane for each plane in the list **/
hitdata findClosestPlane(vector<Plane>& p, ray& r) {
  ...
  for (unsigned int i = 0; i < p.size(); i++) {
    ...
    if (hitplane.hit == true) {
      if (hitobject.hit == false ||
          hitobject.dist > hitplane.dist)  // Conditions for this to be closest
                                           // sphere found so far
      {
        hitobject.hit = true;
...
/** Calculates the normals to the faces of the unit cell order is v_a x v_b :
 * v_b x v_c : v_a x v_c as unit vectors**/
void calcPlanesToCell(ATOM_NETWORK* cell, vector<Plane>& p) {
  ...
  // Each face with point abc point 1,1,1 (Notice that normals are reversed this
  // is so that a vector can easily be determined if it hit within the unit cell
...
/** Since all the opperations in Zeo++ are done within one unit cell it is
 * important to duplicate the atoms that would overlap with the cell walls on
 * the other side **/
void duplicateSpheresOnFace(ATOM_NETWORK* cell, vector<Sphere>& spheres,
                            vector<Plane>& faces) {
  Sphere temp_sphere;
  bool touch_face[6];
  ...
  unsigned int size =
      spheres.size();  // because I will be adding spheres to list (there is a
  ...
  for (unsigned int i = 0; i < size; i++) {
    ...
    permutation_a.clear();
    permutation_a.push_back(0.0);
    permutation_b.clear();
    permutation_b.push_back(0.0);
    permutation_c.clear();
    permutation_c.push_back(0.0);
    ...
    for (unsigned int a = 0; a < permutation_a.size(); a++) {
      for (unsigned int b = 0; b < permutation_b.size(); b++) {
        for (unsigned int c = 0; c < permutation_c.size(); c++) {
          if (permutation_a[a] != 0 || permutation_b[b] != 0 ||
              permutation_c[c] != 0) {
            temp_sphere.center = spheres[i].center;
            temp_sphere.center =
                temp_sphere.center +
                cell->abc_to_xyz(Point(permutation_a[a], permutation_b[b],
                                       permutation_c[c]));
...
/** This function converts nodes to spheres it allows for a more flexible
 * implementation of ray_tracing thus making higher abstractions easier we are
 * only concerned with the accessible nodes thus only these will be copied **/
void convertNodeToSphere(VORONOI_NETWORK& vornet, vector<Sphere>& nodes,
  ...
  nodes.clear();
#if DEBUG
  cout << "Adding: " << vornet.nodes.size() << " nodes (if all accessible)."
       << endl;
#endif
  ...
  for (unsigned int i = 0; i < vornet.nodes.size(); i++) {
    if (accessInfo[i] == true) {
      ...
      temp_sphere.center =
          Point(temp_vornode.x, temp_vornode.y, temp_vornode.z);
...
/** This function converts atoms to spheres it allows for a more flexible
 * implementation of ray_tracing thus making higher abstractions easier **/
...
/** This function finds a nodes radius that encompasses the point raypoint. It
 * will not count the previous node that the raypoint was in by using the
 * reference id**/
bool findSphereOfPoint(Point p, vector<Sphere>& spheres, int& id) {
  for (unsigned int i = 0; i < spheres.size(); i++) {
    if ((calcEuclideanDistance(p, spheres[i].center) < spheres[i].radius) &&
        (int)i != id) {
      id = i;
      return true;
    }
...
/**Generates a random vector direction and unitizes it. Since we want a random
 * ray orientation in cartesian coordinates, sphereical coordinates are used to
 * create a point **/
Point genRandomVec() {
...
/** This function generates a random point within the unit cell with values 0-1
 * for a,b, and c **/
...
/** This function will travel within a sphere until it hits a point that is no
 * longer within spheres. Returns hitdata on the point it hits**/
void rayTraceInsideSphere(ATOM_NETWORK* cell, vector<Sphere>& spheres, ray r,
                          hitdata& hitobject) {
#if DEBUG
  ...
  if (findSphereOfPoint(r.base, spheres, hitobject.id) ==
      false)  // This function also updates the id of the new sphere in
              // hitsphere
  {
  ...
  if (hitsphere.hit == false) {
    ...
    if (fabs(calcEuclideanDistance(r.base, spheres[hitobject.id].center) -
             spheres[hitobject.id].radius) < threshold) {
      // It was round-off error so lets continue
      Sphere epsilon_sphere = spheres[hitobject.id];
      if (((r.base - spheres[hitobject.id].center) * r.vector) >
          0) {  // vector is headed away from sphere
        epsilon_sphere.radius += threshold;
      } else {  // vector is headed into the sphere
        epsilon_sphere.radius += -threshold;
      }
      epsilon_sphere.hitSphere(r, hitsphere);
    }
    if (hitsphere.hit ==
        false)  // Epsilon Case has been covered so thid should not happen
    {
      cerr << "Error: Ray did not hit a sphere. findSphereOfPoint said that "
              "one was within the radius. And it failed to be within an "
              "epsilon. Output in vmd style for easy visulization"
           << endl;
      cout << "draw sphere {" << spheres[hitobject.id].center << "} radius "
           << spheres[hitobject.id].radius << " resolution 10\n";
  ...
  r.base = cell->shiftXYZInUC(
      hitsphere.hitpoint);  // Move back to unit cell and set values
  hitobject.hitpoint = hitsphere.hitpoint;
...
/** This is a recursive function that will keep on extending a ray unitl it hits
 * an atom or exceeds the maximum distance allowed (MAXRAYDIST)**/
void rayTraceToSphere(ATOM_NETWORK* cell, vector<Sphere>& spheres, ray r,
                      vector<Plane>& faces, hitdata& hitobject) {
#if DEBUG
  cout << "Current Ray Length: " << hitobject.dist << endl;
#endif
  ...
  // I am moving the point just slightly outside of the unit cell to be shifted
  // back in
...
/** Returns data from shooting a bunch of rays from accesible regions in the
 * network until they hit an atom. Since the cell is periodic a max ray length
 * can be set so ray does not shoot forever*/
void calcRaysInAV(ATOM_NETWORK* hiaccatmnet, ATOM_NETWORK* orgatmnet,
                  bool highAccuracy, double r_probe_chan, double r_probe,
                  int numSamples, ostream& output, bool visualize,
                  string option) {
  ATOM_NETWORK* cell;
  ...
  if (highAccuracy)
    ...
    cell = hiaccatmnet;  // this ensures that the loops below loop over original
                         // atoms and not "high-acuracy" clusters, which are
                         // used only for voronoi decomposition
  ...
  for (unsigned int i = 0; i < atoms.size(); i++) {
    cout << "Center: " << atoms[i].center << " Radius: " << atoms[i].radius
         << endl;
    ;
  }
  cout << "List of Nodes: " << endl;
  for (unsigned int i = 0; i < nodes.size(); i++) {
    cout << "Center: " << nodes[i].center << " Radius: " << nodes[i].radius
         << endl;
    ;
  }
  cout << "List of Cell Wall Faces: " << endl;
  for (unsigned int i = 0; i < faces.size(); i++) {
    cout << "Point: " << faces[i].point << " Normal: " << faces[i].normal
         << endl;
    ;
  }
#endif
  ...
  if (option.compare("atom") == 0) {
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      } else  // point is accessible
      {
        ...
        opposite_rand_ray.vector =
            opposite_rand_ray.vector.scale(hitobject.dist);
  ...
  if (option.compare("sphere") == 0) {
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      if (hitobject.hit == false) {
        rand_ray.vector = Point(0, 0, 0);
        inaxsray.push_back(rand_ray);
      } else {  // It was traveling within a sphere
        rand_ray.vector = rand_ray.vector.scale(hitobject.dist);
  ...
  if (option.compare("andrew_sphere") == 0) {
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      while (distance <
             MAXRAYDIST)  // Iterate Ray untill it reaches max distance
      {
        if (inside_node == false) {
          ...
          rand_ray.base = cell->shiftXYZInUC(hitobject.hitpoint +
                                             rand_ray.vector.scale(threshold));
  ...
  if (option.compare("andrew_atom") == 0) {
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      inside_accessible_region =
          accessAnalysis.isVPointAccessible(rand_ray.base);
             ...
             MAXRAYDIST)  // Iterate Ray untill it reaches max distance
      {
        if ((inside_atom == false) && (inside_accessible_region == true)) {
        ...
        } else if ((inside_atom == false) &&
                   (inside_accessible_region ==
                    false)) {  // not within an atom but in inaccessible region
          rayTraceToSphere(cell, atoms, rand_ray, faces, hitobject);
          ...
          rand_ray.vector = init_ray.vector;
          rand_ray.base = cell->shiftXYZInUC(hitobject.hitpoint +
                                             init_ray.vector.scale(threshold));
          // Reset all hitobject data
          hitobject.dist = 0.0;
          hitobject.hit = false;
          hitobject.hitpoint = Point(0, 0, 0);
...
// Returns data from shooting a bunch of rays from accesible regions in the
network until they hit an atom. Since the cell is periodic a max ray length can
be set so ray does not shoot forever void calcRaysInAV(ATOM_NETWORK *cell,
ATOM_NETWORK *orgcell, bool highAccuracy, double r_probe_chan,double r_probe,int
numSamples, ostream &output, bool visualize,string option){
...
&channels); srand(randSeed); //randSeed can be set in network.h
...
accessiblePoint(rand_ray.base,new_rad_con,r_probe,cell,vorcells,accessInfo);
      ...
      for (int i=0; i<numSamples; i++)
        {
          ...
          while(distance < MAXRAYDIST) //Iterate Ray untill it reaches max
distance
            {
              if (inside_node == false)
                {
                  ...
                  rand_ray.vector = rand_ray.vector.scale(hitobject.dist);
                  rand_ray.base = init_ray.base +
init_ray.vector.scale(distance); inaxsray.push_back(rand_ray);

                  //Update Total Info
                  distance += hitobject.dist;
                  rand_ray.vector = init_ray.vector;
                  rand_ray.base =
cell->shiftXYZInUC(hitobject.hitpoint+rand_ray.vector.scale(threshold));
                ...
                { //Shoot Through Nodes
                  ...
                  rand_ray.vector = rand_ray.vector.scale(hitobject.dist);
                  rand_ray.base = init_ray.base +
init_ray.vector.scale(distance); axsray.push_back(rand_ray);
...
cell->shiftXYZInUC(hitobject.hitpoint+rand_ray.vector.scale(threshold));
                  ...
                  hitobject.hit = false;
                  hitobject.hitpoint = Point(0,0,0);
                  //Notice that I am not resetting hitobject.id and setting
object hitobject.object = &(nodes[hitobject.id]);
...
cell->shiftXYZInUC(hitobject.hitpoint+rand_ray.vector.scale(threshold));
              ...
              else if ((inside_atom == false) && (inside_accessible_region ==
false)) { //not within an atom but in inaccessible region
                  rayTraceToSphere(cell,atoms,rand_ray,faces,hitobject);
...
init_ray.vector.scale(distance); inaxsray.push_back(rand_ray);
                  ...
                  rand_ray.vector = init_ray.vector;
                  rand_ray.base =
cell->shiftXYZInUC(hitobject.hitpoint+init_ray.vector.scale(threshold));
                  //Reset all hitobject data
                  hitobject.dist = 0.0;
                  hitobject.hit = false;
                  hitobject.hitpoint = Point(0,0,0);
              ...
              else // Ray is inside of atoms
                {
                  ...
                  rand_ray.vector = rand_ray.vector.scale(hitobject.dist);
                  //rand_ray.base = init_ray.base +
init_ray.vector.scale(distance);
                  //This is where I would push data on traveling through atoms
                  //but it is not here yet
...
object hitobject.object = &(nodes[hitobject.id]);
                  ...
                  //So at this point I know that I am outside an atom however am
                  //I in an accesible region or not?
                  inside_atom = false; //Now outside of atoms and in accessible
region inside_accessible_region =
accessiblePoint(rand_ray.base,new_rad_con,r_probe,cell,vorcells,accessInfo);
                }
            }
        }
...
/** Outputs a data file Ray_Info.txt that gives important information
    about the rays that were found to be accessible such as Point, Vector, and
   Magnitude **/
void reportRayInfo(vector<ray>& axsray) {
  ...
  if (rayinfo.good() == true) {
    ...
    for (unsigned int i = 0; i < axsray.size(); i++) {
      ...
      //      rayinfo << rand_ray.base << " "<< rand_ray.vector << " " <<
      //      rand_ray.vector.magnitude()  << endl;
      rayinfo << rand_ray.base.vals[0] << " " << rand_ray.base.vals[1] << " "
              << rand_ray.base.vals[2] << " " << rand_ray.vector.vals[0] << " "
              << rand_ray.vector.vals[1] << " " << rand_ray.vector.vals[2]
              << " " << rand_ray.vector.magnitude() << endl;
    }
  } else {
...
void reportRays(ostream& output, vector<ray>& axsray, vector<ray>& inaxsray,
                bool colors) {
  if (colors == true) {
    ...
    output << "{color blue}"
           << "\n";
    for (unsigned int i = 0; i < axsray.size(); i++) {
      if (axsray[i].vector.magnitude() < 6 &&
          axsray[i].vector.magnitude() >= 3) {
        ray rand_ray = axsray.at(i);
        Point addition = rand_ray.base + rand_ray.vector;
        output << "{line {" << rand_ray.base << "} {" << addition << "}}"
               << "\n";
    ...
    output << "{color lime}"
           << "\n";
    for (unsigned int i = 0; i < axsray.size(); i++) {
      if (axsray[i].vector.magnitude() >= 9 &&
          axsray[i].vector.magnitude() < 12) {
        ray rand_ray = axsray.at(i);
        Point addition = rand_ray.base + rand_ray.vector;
        output << "{line {" << rand_ray.base << "} {" << addition << "}}"
               << "\n";
    ...
    output << "{color orange}"
           << "\n";
    for (unsigned int i = 0; i < axsray.size(); i++) {
      if (axsray[i].vector.magnitude() >= 12 &&
          axsray[i].vector.magnitude() < 20) {
        ray rand_ray = axsray.at(i);
        Point addition = rand_ray.base + rand_ray.vector;
        output << "{line {" << rand_ray.base << "} {" << addition << "}}"
               << "\n";
...
void reportAtoms(ostream& output, vector<Sphere>& s) {
  output << "{color red}"
         << "\n";
  for (unsigned int i = 0; i < s.size(); i++) {
    output << "{sphere {" << s[i].center << "} radius " << s[i].radius
           << " resolution 50}\n";
  }
}
...
void reportPlane(ostream &output, vector<Plane> p){
  ...
  for (unsigned int i=0; i < p.size(); i++){
    output << "draw traingle {" <<  point << "} radius " << temp.radius << "
resolution 10 \n";
  }
}
**/

void reportHistogram(ostream& output, const double binSize, const int maxBins,
                     vector<ray>& rays) {
  ...
  // Output Histogram
  output << "Ray Histogram - Bin Size = " << binSize
         << " Number of Bins: " << maxBins
         << "From: 0 To: " << binSize * 1.0 * maxBins << endl;
  for (int i = 0; i < maxBins; i++) {
    output << bins[i] << endl;
  }
}
