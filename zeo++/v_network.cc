...
voronoi_network::voronoi_network(c_class &c, double net_tol_)
  ...
  for (l = 0; l < nxyz; l++) {
    ...
    ptsc[l] = 0;
    ptsmem[l] = init_network_vertex_memory;
  }

  // Allocate memory for network edges and related statistics
  edc = 0;
  edmem = init_network_vertex_memory * nxyz;
  ed = new int *[edmem];
  ne = new int *[edmem];
  ...
  for (l = 0; l < edmem; l++)
    pered[l] = new unsigned int[init_network_edge_memory];
  for (l = 0; l < edmem; l++) {
    nu[l] = nec[l] = 0;
    numem[l] = init_network_edge_memory;
  }
...
void voronoi_network::add_network_memory(int l) {
  ...
  if (ptsmem[l] > max_network_vertex_memory)
    voro_fatal_error("Container vertex maximum memory allocation exceeded",
                     VOROPP_MEMORY_ERROR);
...
void voronoi_network::add_edge_network_memory() {
  ...
  while (i < edmem) {
    ...
    nnu[i] = nnec[i] = 0;
    nnumem[i] = init_network_edge_memory;
...
void voronoi_network::add_particular_vertex_memory(int l) {
  ...
  if (numem[l] > max_vertex_order)
    voro_fatal_error("Particular vertex maximum memory allocation exceeded",
                     VOROPP_MEMORY_ERROR);
...
void voronoi_network::store_network(vector<VOR_NODE> &nodes,
  ...
  for (l = 0; l < edc; l++) {
    ...
    nodes.push_back(
        VOR_NODE(ptsp[j], ptsp[j + 1], ptsp[j + 2], ptsp[j + 3], atomIDs));
  }
  ...
  for (l = 0; l < edc; l++) {
    ...
    for (q = 0; q < nu[l]; q++) {
      ...
      z2 = ptsp[j + 2] + ak * bz - z;
      edges.push_back(VOR_EDGE(l, ed[l][q], raded[l][q].e, ai, aj, ak,
...
void voronoi_network::print_network(FILE *fp, bool reverse_remove) {
  ...
  for (l = 0; l < edc; l++) {
    ptsp = pts[reg[l]];
    j = 4 * regp[l];
    fprintf(fp, "%d %g %g %g %g", l, ptsp[j], ptsp[j + 1], ptsp[j + 2],
            ptsp[j + 3]);
...
inline unsigned int voronoi_network::pack_periodicity(int i, int j, int k) {
  ...
  pa <<= 8;
  pa += ((unsigned int)(127 + j));
  pa <<= 8;
  pa += ((unsigned int)(127 + k));
  return pa;
}
...
inline void voronoi_network::unpack_periodicity(unsigned int pa, int &i, int &j,
                                                int &k) {
...
void voronoi_network::add_to_network_internal(v_cell &c, int idn, double x,
                                              double y, double z, double rad,
                                              int *cmap) {
  ...
  for (l = 0; l < c.p; l++, vmp += 4) {
    ...
    crad = 0.5 * sqrt(cp[4 * l] * cp[4 * l] + cp[4 * l + 1] * cp[4 * l + 1] +
                      cp[4 * l + 2] * cp[4 * l + 2]) -
           rad;
...
inline void voronoi_network::add_neighbor(int k, int idn) {
  for (int i = 0; i < nec[k]; i++)
    if (ne[k][i] == idn) return;
...
template <class v_cell>
void voronoi_network::add_edges_to_network(v_cell &c, double x, double y,
                                           double z, double rad, int *cmap) {
  int i, j, ai, aj, ak, bi, bj, bk, k, l, q, *vmp;
  unsigned int cper;
  double vx, vy, vz, wx, wy, wz, dx, dy, dz, dis;
  double *pp;
  for (l = 0; l < c.p; l++) {
    vmp = cmap + 4 * l;
    k = *(vmp++);
...
template <class v_cell>
void voronoi_network::add_to_network_rectangular_internal(v_cell &c, int idn,
                                                          double x, double y,
                                                          double z, double rad,
                                                          int *cmap) {
  int i, j, k, ijk, l, q, ai, aj, ak, *vmp(cmap);
...
int voronoi_network::not_already_there(int k, int j, unsigned int cper) {
  for (int i = 0; i < nu[k]; i++)
    if (ed[k][i] == j && pered[k][i] == cper) return i;
  return nu[k];
}

bool voronoi_network::search_previous(double gx, double gy, double x, double y,
                                      double z, int &ijk, int &q, int &pi,
                                      int &pj, int &pk) {
    ...
    for (j = aj; j <= bj; j++) {
      ...
      for (i = ai; i <= bi; i++) {
        ...
        for (q = 0; q < ptsc[ijk]; q++, pp += 4)
          if (fabs(*pp + px3 - x) < net_tol &&
              fabs(pp[1] + py2 - y) < net_tol && fabs(pp[2] + pz - z) < net_tol)
            return true;
      }
    }
  }
...
bool voronoi_network::safe_search_previous_rect(double x, double y, double z,
                                                int &ijk, int &q, int &ci,
                                                int &cj, int &ck) {
  const double tol(0.5 * net_tol);
  if (search_previous_rect(x + tol, y + tol, z + tol, ijk, q, ci, cj, ck))
    return true;
  if (search_previous_rect(x - tol, y + tol, z + tol, ijk, q, ci, cj, ck))
    return true;
  if (search_previous_rect(x + tol, y - tol, z + tol, ijk, q, ci, cj, ck))
...
bool voronoi_network::search_previous_rect(double x, double y, double z,
                                           int &ijk, int &q, int &ci, int &cj,
                                           int &ck) {
  ...
  double *pp(pts[ijk]);
  for (q = 0; q < ptsc[ijk]; q++, pp += 4)
    if (fabs(*pp - x) < net_tol && fabs(pp[1] - y) < net_tol &&
        fabs(pp[2] - z) < net_tol)
      return true;
  return false;
}
...
template void voronoi_network::add_to_network<voronoicell>(voronoicell &, int,
                                                           double, double,
                                                           double, double);
template void voronoi_network::add_to_network<voronoicell_neighbor>(
    voronoicell_neighbor &, int, double, double, double, double);
template void voronoi_network::add_to_network_rectangular<voronoicell>(
    voronoicell &, int, double, double, double, double);
template void voronoi_network::add_to_network_rectangular<voronoicell_neighbor>(
    voronoicell_neighbor &, int, double, double, double, double);
