...
/* Represents a node in a graph data structure created from a Voronoi network.
 * Stores information about the connections that stem from it as well as its
 * position.
 */

/** Create a node using the provided parameters. */
DIJKSTRA_NODE::DIJKSTRA_NODE(int myID, double myX, double myY, double myZ,
                             double maxR, bool active_flag) {
  id = myID;
  x = myX;
  y = myY;
  z = myZ;
 ...
 *  corresponding Voronoi network. Represents a single unit cell of the
 * material.
 */

DIJKSTRA_NETWORK::DIJKSTRA_NETWORK() { nodes = vector<DIJKSTRA_NODE>(); }

/* Output information about the network to the provided output stream. */
void DIJKSTRA_NETWORK::print(ostream &out) {
  for (unsigned int i = 0; i < nodes.size(); i++) nodes.at(i).print(out);
}

/* Build a DIJKSTRA_NETWORK from the provided VORONOI_NETWORK
 *  and store it using the provided pointer.
 */
void DIJKSTRA_NETWORK::buildDijkstraNetwork(const VORONOI_NETWORK *vornet,
  ...
  while (niter != vornet->nodes.end()) {
    DIJKSTRA_NODE node = DIJKSTRA_NODE(i, niter->x, niter->y, niter->z,
                                       niter->rad_stat_sphere, niter->active);
    i++;
    niter++;
    dnet->nodes.push_back(node);
  }

  // For each edge, store it in the DIJKSTRA node's list of connections that the
  // connection stems from
  vector<VOR_EDGE>::const_iterator eiter = vornet->edges.begin();
  while (eiter != vornet->edges.end()) {
    DELTA_POS pos =
        DELTA_POS(eiter->delta_uc_x, eiter->delta_uc_y, eiter->delta_uc_z);
    CONN conn = CONN(eiter->from, eiter->to, eiter->length,
                     eiter->rad_moving_sphere, pos);
...
/* Creates a new DIJKSTRA_NETWORK containing only the edges in which both
 terminuses are contained within the provided vector of ids.
 */
