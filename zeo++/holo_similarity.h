...
typedef struct {
  ...
  int y;
  int z;
} coords_int;  // standard triplet of ints
typedef struct {
  int num_entries;
...
void build_array(float ***array, hologram holo, const int NUM_BINS,
                 const int DIFFUSION_RANGE);
float get_similarity(float ***array_i, float ***array_j, const int NUM_BINS,
                     const int DIFFUSION_RANGE, const int SIMILARITY_MEASURE);

const int BIN_TAN = 1, MTW = 2, MTU = 3, MT_80_20 = 4, BARONI_URBANI = 5,
          CONT_TAN = 6, MTW_COMBO = 7, MTU_COMBO = 8, MISMATCH = 9,
          BRAY_CURTIS = 10, MAX_ALLOWED = 10;
  ...
  for (i = 0; i < holo.num_entries; i++) {
    ...
    for (i_diff = -1 * DIFFUSION_RANGE; i_diff <= DIFFUSION_RANGE; i_diff++) {
      ...
      if (!(new_hi < 0 || new_hi >= NUM_BINS)) {
        for (j_diff = -1 * DIFFUSION_RANGE; j_diff <= DIFFUSION_RANGE;
             j_diff++) {
          new_hj = hj + j_diff;
          if (!(new_hj < 0 || new_hj >= NUM_BINS)) {
            for (k_diff = -1 * DIFFUSION_RANGE; k_diff <= DIFFUSION_RANGE;
                 k_diff++) {
              new_hk = hk + k_diff;
              if (!(new_hk < 0 || new_hk >= NUM_BINS)) {
                if (!(new_hk >
                      new_hj)) {  // i.e. don't write to the side of the grid
                                  // which wasn't written to before!
                  float distance =
                      sqrt((i_diff) * (i_diff) + (j_diff) * (j_diff) +
                           (k_diff) * (k_diff));
                  distance += 1;  // avoid dividing by zero
                  array[new_hi][new_hj][new_hk] +=
                      value / distance;  // increment this neighbouring point by
                                         // the inverse distance times the value
                                         // of this hologram point
                }
              }
            }
          }
...
float get_similarity(float ***array_i, float ***array_j, const int NUM_BINS,
    ...
    for (l = 0; l < NUM_BINS; l++) {
      for (m = 0; m < NUM_BINS; m++) {
        if (!(m > l)) {  // i.e. don't read from the empty side of the grid
          ...
          sum_diff += (array_i[k][l][m] - array_j[k][l][m]) *
                      (array_i[k][l][m] - array_j[k][l][m]);
  ...
  if (SIMILARITY_MEASURE == CONT_TAN) {
    ...
    if (denominator <= 0.0)
      ...
      similarity = sum_ab / denominator;
  } else if (SIMILARITY_MEASURE == BIN_TAN) {
    float denominator = (float)(ni + nj - n11);
    if (denominator <= 0.0)
      similarity = -1;
    else
      similarity = ((float)(n11)) / denominator;
  } else if (SIMILARITY_MEASURE == MISMATCH) {
    float denominator = sum_aa + sum_bb;
    if (denominator <= 0.0)
      similarity = -1;
    else
      similarity = 1 - (sum_diff / denominator);
  } else if (SIMILARITY_MEASURE == MTU || SIMILARITY_MEASURE == MTW) {
    if (SIMILARITY_MEASURE == MTW)
      p_hat = ((float)(n11 + (n - n00))) / ((float)(2 * n));  // weighted
                                                              // version
    else
      p_hat = 0.5;  // unweighted version
    if (n - n00 == 0)
      presence_tan = 1;  // as instructed in paper
    else
      presence_tan = ((float)(n11)) / ((float)(n - n00));
    if (n - n11 == 0)
      absence_tan = 1;  // as instructed in paper
    else
      absence_tan = ((float)(n00)) / ((float)(n - n11));
    similarity = (((2.0 - p_hat) / 3.0) * presence_tan) +
                 (((1.0 + p_hat) / 3.0) * absence_tan);
  } else if (SIMILARITY_MEASURE == MTU_COMBO ||
             SIMILARITY_MEASURE == MTW_COMBO) {
    if (SIMILARITY_MEASURE == MTW_COMBO)
      p_hat = ((float)(n11 + (n - n00))) / ((float)(2 * n));  // weighted
                                                              // version
    else
      p_hat = 0.5;  // unweighted version
    ...
    if (denominator <= 0)
      ...
      presence_tan = sum_ab / denominator;
    if (n - n11 == 0)
      absence_tan = 1;  // as instructed in paper
    else
      absence_tan = ((float)(n00)) / ((float)(n - n11));
    ...
    if (sum_a_plus_b <= 0.0)
      ...
      similarity = 1 - (sum_diff_a_b /
                        sum_a_plus_b);  // because this is a distance function
  } else if (SIMILARITY_MEASURE == BARONI_URBANI) {
    float denominator = sqrt(float(sum_a * sum_d)) + sum_a + sum_b + sum_c;
    if (denominator <= 0.0)
      similarity = -1;
    else
      similarity = sqrt(float((sum_a * sum_d) + sum_a)) / denominator;
  }
  return similarity;
}
