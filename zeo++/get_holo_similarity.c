...
main(int argc, char *argv[]) {
  ...
  if (argc != needed_args) {
    printf(
        ...
        needed_args);
    printf(
        "./a.out 1_holo.txt 2_holo.txt DIFFUSION_RANGE SIMILARITY_MEASURE\n");
  ...
  status = 0;
  while (status != -1) {
    status = fscanf(in1, "%d %d %d %d", &grid_point.x, &grid_point.y,
  ...
  status = 0;
  while (status != -1) {
    status = fscanf(in2, "%d %d %d %d", &grid_point.x, &grid_point.y,
                    &grid_point.z, &quantity);
    if (status != -1) {
  ...
  if (is_valid[0] && is_valid[1]) {
    ...
    printf("At least one structure is invalid\n");
}
