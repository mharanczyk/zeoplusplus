...
class zeoJob {
  ...
  double nMCsamplesScaleFactor;    // scaling factor for the default number of
                                   // samples
 ...
 public:
  ...
  /* Deconstructor */
  void deinit() { Material.deinit(); };

  bool setup(std::vector<std::vector<std::string> > commands_string,
             const char *name_string, const char *extension_string) {
    ...
    for (int i = 0; i < numCommands; i++) {
      ...
      if (command[0].compare("-r") == 0) {
      ...
      } else if (commands[i][0].compare("-ext") == 0 ||
                 commands[i][0].compare("-eo") == 0 ||
                 commands[i][0].compare("-extendedOutput") == 0) {
        extendedOutputFlag = true;
      } else if (commands[i][0].compare("-vo") == 0 ||
                 commands[i][0].compare("-visual") == 0 ||
                 commands[i][0].compare("-visout") == 0) {
        ...
        Material.atmnet.allowAdjustCoordsAndCellFlag =
            Material.allowAdjustCoordsAndCell;
        Material.orgAtomnet.allowAdjustCoordsAndCellFlag =
            Material.allowAdjustCoordsAndCell;
        ...
        nMCASAsampleDensity =
            (int)(nMCASAsampleDensity * nMCsamplesScaleFactor);
        if (nMCAVsampleDensity < 1 || nMCASAsampleDensity < 1) {
          error = true;
          cerr << "Density of MC samples below 1. Please use larger value with "
                  "-sqf argument. Current density for AV = "
    ...
    if (probeRadiusFlag == false) {
      for (int i = 0; i < numCommands; i++) {
        ...
        if (command[0].compare("-zvor") == 0 ||
            ...
            command[0].compare("-vol") == 0 || command[0].compare("-sa") == 0 ||
          ...
          error = true;
          cerr << command[0]
               << " option requires probe radius. Please use \"-probe R\" "
                  "(where R is radius)\n";
        }
      }
    }

    /* Pre-process the arugment list to make sure all run dependiences are
     * fulfilled */
    if (volumeRunFlag == false) {
      for (int i = 0; i < numCommands; i++) {
        vector<string> command = commands[i];
        if (command[0].compare("-block") == 0 ||
            command[0].compare("-psd") == 0) {
          error = true;
          cerr << command[0]
               << " option depends on accessible volume calculation. Please "
                  "add \"-vol nS\" (where nS is the number of MC samples per "
                  "cubic Ang).\n";
        };
      };
    }
    ...
    if (error == false) {
      ...
      if (strcmp(extension, "cuc") == 0) {
        if (!readCUCFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "arc") == 0) {
        if (!readARCFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "cssr") == 0) {
        if (!readCSSRFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "cif") == 0) {
        if (!readCIFFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "car") == 0) {
        if (!readCARFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "v1") == 0) {
        if (!readV1File(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "pdb") == 0) {
        if (!readPDBFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      } else if (strcmp(extension, "dlp") == 0) {
        if (!readDLPFile(filename, &(Material.atmnet), Material.radial))
          error = true;
      }
    ...
    if (!error) {
      ...
      if (Material.radial) loadRadii(&(Material.atmnet));

      loadMass(Material.useMass,
               &(Material.atmnet));  // assigns masses to atoms (assigns 0s in
                                     // useMass flag = false)

      // High accuracy options - if the system contains particles of different
      // radii, the employed radical Voronoi decomposition may not be accurate.
      // it can be, however, improved if large particles are replaced by
      // clusters of smaller one (idea taken from Carolyn Philiphs/Chris
      // Rycroft).
      Material.atmnet.copy(
          &(Material.orgAtomnet));  // keep a copy of original atomnet
                                    //(if high accuracy is not set, this is the
                                    // same as analyzed network)
  ...
  void execute() {
     ...
     *             becomes modified (sphere approx). Please use
     * Material.orgAtomnet to access original atom network (as read from the
     * input file)
     *
    ...
    for (int i = 0; i < numCommands && !error; i++) {
      ...
      if (command[0].compare("-cssr") == 0) {
        if (strcmp(extension, ".v1") == 0) {
          cerr
              << "Can not output a .cssr file when a .v1 file is used as input."
              << "\n"
              << "Exiting ..."
              << "\n";
          //        exit(1);
        ...
        if (!writeToCSSR((char *)filename.data(), &(Material.orgAtomnet))) {
          error = true;
          break;
        }
        ...
        // if(!writeToCIF((char *)filename.data(), name, &atmnet)) {error=true;
        // break;}
        if (!writeToCIF((char *)filename.data(), &(Material.orgAtomnet))) {
          error = true;
          break;
        }
        ...
        if (!writeToXYZ((char *)filename.data(), &(Material.orgAtomnet),
                        is_supercell, is_duplicate_perimeter_atoms)) {
          error = true;
          break;
        }
        ...
        if (!writeToXYZ((char *)filename1.data(), &(Material.orgAtomnet),
                        is_supercell, is_duplicate_perimeter_atoms)) {
          error = true;
          break;
        }
        ...
        is_supercell = true;
        if (!writeToXYZ((char *)filename2.data(), &(Material.orgAtomnet),
                        is_supercell, is_duplicate_perimeter_atoms)) {
          error = true;
          break;
        }
        ...
        if (!writeToVTK((char *)filename3.data(), &(Material.orgAtomnet))) {
          error = true;
          break;
        }
        ...
        if (!writeToVTK((char *)filename.data(), &(Material.orgAtomnet))) {
          error = true;
          break;
        }
        ...
        if (!writeToV1((char *)filename.data(), &(Material.orgAtomnet))) {
          error = true;
          break;
        }
        ...
        if (!writeToNt2((char *)filename.data(), &(Material.vornet))) {
          error = true;
          break;
        }
        ...
        bool is_supercell = false;
        if (!writeToMOPAC((char *)filename.data(), &(Material.orgAtomnet),
                          is_supercell)) {
          error = true;
          break;
        }
        ...
        if (command[0].compare("-strinfo") == 0)
          getStructureInformation(
              (char *)filename.data(), (char *)filenameExtOutput.data(),
              &(Material.orgAtomnet), false);  // last argument 4 ext. output
        else
          getStructureInformation((char *)filename.data(),
                                  (char *)filenameExtOutput.data(),
                                  &(Material.orgAtomnet), true);
        ...
        Material.runVoroFlat();
        writeZeoVisFile((char *)filename.data(), &(Material.cells),
                        &(Material.atmnet), &(Material.vornet));
      } else if (command[0].compare("-zvor") ==
                 0) {  // THIS NEEDS CLEARINING
        ...
        Material.runVoroFlat();
        CHANNEL::findChannels(&(Material.vornet), probeRadius, &accessInfo,
                              &channels);
        ...
        if (command.size() == 4) {
          ...
          skel_a = strtod(command[1].data(), NULL),
          skel_b = strtod(command[2].data(), NULL),
          skel_c = strtod(command[3].data(), NULL);
        };
        Material.runVoroFlat();
        //        visVoro(name, probeRad, skel_a, skel_b, skel_c, &vornet,
        //        &atmnet);
      ...
      // Rich: holograms as a separate flag, -holo
        ...
        if (command.size() == 2) {  // i.e. a directory was specified
          ...
          analyze_accessible_voronoi_pre_segment(
              &(Material.vornet), probeRadius, &accessInfo, name,
              bin_directory);  // this line generates and outputs Voronoi
                               // holograms with bins from a specified directory
        }
        analyze_accessible_voronoi_pre_segment(
            &(Material.vornet), probeRadius, &accessInfo,
            name);  // this line generates and outputs Voronoi holograms with
                    // default bins
        ...
        if (visualize == false) {
          ...
          output << filename << " summary(Max_of_columns_above)   " << di << " "
                 << df << "  " << dif << "  probe_rad: " << probeRadius
                 << "  probe_diam: " << 2 * probeRadius << "\n";
        } else {
          for (unsigned int i = 0; i < channels.size(); i++) {
        ...
        vector<PORE> pores;
        Material.runVoroFlat();
        PORE::findChannelsAndPockets(&(Material.vornet), probeRadius,
                                     &accessInfo, &pores);
        ...
        double volume = calcDeterminant(
            Material.atmnet.ucVectors);  // Unit cell volume/Units of A^3
        int numSamples = (int)(volume * numSamplesAV);
        calcAV(&(Material.atmnet), &(Material.orgAtomnet),
               Material.highAccuracy, probeRadius, probeRadius, numSamples,
               true, output, (char *)filename.c_str(), 0, 0, 0, 0, -1, -1, 0);
        calcASA(&(Material.atmnet), &(Material.orgAtomnet),
                Material.highAccuracy, probeRadius, probeRadius,
                calcDensity(&(Material.atmnet)), numSamplesASA, true, output,
                (char *)filename.c_str(), 0, 0, 0, 0);

        for (unsigned int i = 0; i < pores.size(); i++)
          if (pores[i].dimensionality > 0)
            pores[i].printPoreSummary(output, &(Material.atmnet));  // Channels

        for (unsigned int i = 0; i < pores.size(); i++)
          if (pores[i].dimensionality == 0)
            pores[i].printPoreSummary(output, &(Material.atmnet));  // Pockets
        ...
        for(unsigned int i=0; i < channels.size(); i++){
          pair <double, pair<double,double> > maxdidfdif =
        channels[i].findFreeIncludedSphereDiameter(); output << "Channel  " << i
        << "  "  << maxdidfdif.first << "  " << maxdidfdif.second.first << "  "
        << maxdidfdif.second.second << "\n";
          };
        */
        output.close();
        ...
        analyzePoreInfoFiles(&(Material.atmnet), filename_InputData,
                             filename_output);
      }

      else if (command[0].compare("-test") == 0) {  // test new functionality
        // string filename_output =  processFilename(command, name,
        // ".poreinfoSummary", 1, 2); if(filename_output.empty()) {error=true;
        // break;} string filename_InputData = string(command[1]);
        ...
        if (Material.VisFlag == true) {
          string filename_vis;
          if (Material.VisSetting == "ZEOVIS")
            filename_vis = basefilename + ".zvol";
          if (Material.VisSetting == "VISIT")
            filename_vis = basefilename + ".vvol";
          if (Material.VisSetting == "LIVERPOOL")
            filename_vis = basefilename + ".lvol";
          output.open(filename_vis.data(), fstream::out);
          Material.AVreportPoints(output);
          output.close();
        };
        // additional calculations for accessible/inaccessible points from AV
        // calc
        ...
        if (Material.AVrequestPSD == true) {
          ...
          if (Material.VisFlag == true) {
            string filename_vis;
            if (Material.VisSetting == "VISIT")
              filename_vis = basefilename + ".vpsd";
            if (Material.VisSetting == "LIVERPOOL")
              filename_vis = basefilename + ".lpsd";
            output.open(filename_vis.data(), fstream::out);
        ...
        if (Material.VisFlag == true) {
          string filename_vis;
          if (Material.VisSetting == "ZEOVIS")
            filename_vis = basefilename + ".zsa";
          if (Material.VisSetting == "VISIT")
            filename_vis = basefilename + ".vsa";
          if (Material.VisSetting == "LIVERPOOL")
            filename_vis = basefilename + ".lsa";
          output.open(filename_vis.data(), fstream::out);
      ...
      /* Functionality related with PLD calculations */
      else if (command[0].compare("-pld") == 0 ||
               command[0].compare("-pldf") == 0 ||
               command[0].compare("-pldmol") == 0) {
        string basefilename = processFilename(command, name, "", 1, 2);
        string filename_output = basefilename + ".pld";
        double segmentingRadius =
            -1;  // radius used for inital segmentation of strucutre
        ...
        fstream output;
        output.open(filename_output.data(), fstream::out);
        Material.PLDcalc(probeRadius, segmentingRadius, segmentingFile, output,
                         name);
        output.close();
        if (Material.VisFlag == true) {
      ...
      /* Silica substitution functions */
        ...
        /* This Si substitution function changes every second Si into Al
           then convert them back to Si according to the request fraction (frac
           argument) This function fails for system in which topology does not
           allow such initial substitution and therefore the next block of
           function presents an alternative */
        ...
        //        newNetwork.allowAdjustCoordsAndCellFlag =
        ...
        bool subSeed = false;      // Substitute first Si when creating initial
                                   // 50/50 configuration
        if (frac < 0 || frac > 0.5) {
          cerr << "Error: Frac argument needs to be a number from the range of "
                  "(0,0.5>\n"
               << "/n";
        } else
          success =
              fracSubstituteAtoms(&(Material.orgAtomnet), &newNetwork, subSeed,
                                  frac, randSeed, &numSubs, &subFrac, true);
        if (!success) {
          ...
          cout << "Substituted " << numSubs << " Si atoms for a fraction of "
               << subFrac << " when analyzing " << name << "\n";
        }
      }  // ends -fsub
        ...
        /* This function has two modifications w.r.t -fsub. It allows chaging
           random seed to generate different configurations and removes topology
           check which ensured that every other atom is Si (in some cases
           achieving the requested Al fraction will not be possible. */
        ...
        bool subSeed = false;  // Substitute first Si when creating initial
                               // 50/50 configuration
        bool success = fracSubstituteAtoms_Maciek(
            (Material.orgAtomnet), newNetwork, subSeed, frac, randSeed, numSubs,
            subFrac, true);
        if (!success) {
          cerr << "Error: Atom substitution unsucessful."
               << "\n"
      ...
      // flag to print to terminal the tetrahedrality of, e.g. Zn4 groups inside
      // a material
      ...
      // flag to write to terminal the size of supercell required in order to
      // satisfy 'minimum image convention' (correct terminology?)
      // - how many cells in each axis required so that no sphere of a given
      // radius overlaps with itself periodically
        ...
        double diam = 2.0 * rad;
        TRIPLET smallest_supercell =
            Material.orgAtomnet.getSmallestSupercell(diam);
        printf(
            "%s supercell: %d %d %d = %d cells\n", name, smallest_supercell.x,
            smallest_supercell.y, smallest_supercell.z,
            smallest_supercell.x * smallest_supercell.y * smallest_supercell.z);
      }
