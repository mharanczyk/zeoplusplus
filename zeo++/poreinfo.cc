...
/* Function that performs analysis for a set of .poreinfo files listed in
 listfilename.
 * Output is written to outputfilename. Atom_network has to be provided to
 enable distance calculations */

void analyzePoreInfoFiles(ATOM_NETWORK *atmnet, std::string listfilename,
  ...
  if (flist.is_open() == false) {
    cerr << "Error: A file with .poreinfo frames (" << listfilename
         << ") failed to open. \n";
  } else {
 ...
 */
void loadPoreInfoFile(vector<vector<POREINFO> > *poreinfoframes,
                      std::string poreinfofilename) {
  ...
  if (input.is_open() == false) {
    ...
    input >> temp >> temp0 >> nPores >> temp1 >> nChannels >> temp2 >> nPockets;
    getline(input, temp);
    ...
    input >> temp >> n;
    if (n != nChannels) {
      cerr << "Number of AV channels if different than -chan channels, "
              "consider running with -ha; "
           << n << "  " << nChannels << "\n";
    };
    ...
    input >> temp >> n;
    if (n != nPockets) {
      cerr << "Number of AV pockets if different than -chan pockets, consider "
              "running with -ha; "
           << n << "  " << nPockets << "\n";
    };
    ...
    input >> temp >> n;
    if (n != nChannels) {
      cerr << "Number of ASA channels if different than -chan channels, "
              "consider running with -ha;"
           << n << "  " << nChannels << "\n";
    };
    ...
    input >> temp >> n;
    if (n != nPockets) {
      cerr << "Number of ASA pockets if different than -chan pockets, consider "
              "running with -ha; "
           << n << "  " << nPockets << "\n";
    };
    ...
    for (int i = 0; i < nChannels; i++) {
      int nNodes;
      NODESPHERE ns;
      input >> nNodes >> PoreInfoFrame.at(i).di >> PoreInfoFrame.at(i).pos[0] >>
          PoreInfoFrame.at(i).pos[1] >> PoreInfoFrame.at(i).pos[2] >>
          PoreInfoFrame.at(i).enc_radius;
    ...
    for (int i = 0; i < nPockets; i++) {
      int nNodes;
      NODESPHERE ns;
      input >> nNodes >> PoreInfoFrame.at(i + nChannels).di >>
          PoreInfoFrame.at(i + nChannels).pos[0] >>
          PoreInfoFrame.at(i + nChannels).pos[1] >>
          PoreInfoFrame.at(i + nChannels).pos[2] >>
          PoreInfoFrame.at(i + nChannels).enc_radius;
      for (int j = 0; j < nNodes; j++) {
        input >> ns.a >> ns.b >> ns.c >> ns.r;
      };
