...
/* This class is handling replacment of atoms (large spheres) with clusters of
 * small spheres it is done to achive better approximation of S-Voronoi (S-cell)
 * using radial Voronoi decomposition implemented in Voro++ */
/* The class was written by Marielle Pinheiro (March, 2013), modified and merge
 * with Zeo++ by Maciej Haranczyk */
...
class AtomCluster {
  ...
  */
  ...
  void copyReplacementAtoms(ATOM_NETWORK *atmnet, int atomID,
                            std::vector<ATOM> *newatoms);

 private:
  // translation functions
  ATOM translate_sphere(ATOM sphere, double x_step, double y_step,
                        double z_step, int x_sign, int y_sign, int z_sign);
  ...
  ATOM calc_center(std::vector<ATOM> old_coords, int i_0, int i_1, int i_2,
                   int i_3, int i_4, double shift);
