 ...
 * are also classified in a similar manner as source connections (-1 change in
 * uc in d.o.i.), sink connections (+1 in d.o.i.) or a regular (0 in d.o.i.).
 */
class TRAVERSAL_NETWORK {
 public:
  ...
  /** Construct a traversal network in the direction (dx,dy,dz) using the
   * provided source ids, sink ids, and classified connections. */
  TRAVERSAL_NETWORK(int dx, int dy, int dz, std::vector<int> srcIDs,
                    std::vector<std::vector<CONN> > regConns,
                    std::vector<std::vector<CONN> > srcConns,
                    std::vector<std::vector<CONN> > sinkConns,
   ...
   * based on their component change in unit cell
  ...
  std::pair<bool, PATH> findMaxFreeSphere(std::map<int, int> *idAliases,
...
bool betterPath(std::pair<int, std::pair<DELTA_POS, double> > p1,
                std::pair<int, std::pair<DELTA_POS, double> > p2);
 ...
 */
std::vector<std::vector<double> > calculateNodeOverlap(ATOM_NETWORK *atmnet,
 ...
 */
std::vector<std::set<int> > clusterElements(
    std::set<int> cageNodes, std::vector<std::vector<double> > overlapFractions,
    double MIN_FRACTION);
...
class CAGE {
  ...
  /* Compute the center and radius of the cage formed by the provided set of
   * node ids in the Voronoi network. Store the resulting center and radius as
   * well as the node ids and displacements.
   */
  void reconstructCage(std::set<int> ids, ATOM_NETWORK *atmnet,
                       VORONOI_NETWORK *vornet);
...
void identifyCages(ATOM_NETWORK *atmnet, VORONOI_NETWORK *vornet,
                   ...
                   std::fstream &output, std::vector<CAGE> &cages);
