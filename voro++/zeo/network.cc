...
int main(int argc, char **argv) {
  ...
  if (fscanf(fp, "%s %lg %lg %lg", buffer, &bx, &x, &x) != 4)
    file_import_error();
  if (strcmp(buffer, "va=") != 0)
    voro_fatal_error("Invalid first vector", VOROPP_FILE_ERROR);
  if (fscanf(fp, "%s %lg %lg %lg", buffer, &bxy, &by, &x) != 4)
    file_import_error();
  if (strcmp(buffer, "vb=") != 0)
    voro_fatal_error("Invalid second vector", VOROPP_FILE_ERROR);
  if (fscanf(fp, "%s %lg %lg %lg", buffer, &bxz, &byz, &bz) != 4)
    file_import_error();
  if (strcmp(buffer, "vc=") != 0)
    voro_fatal_error("Invalid third vector", VOROPP_FILE_ERROR);
...
void compute(c_class &con, char *buffer, int bp, double vol) {
  ...
  extension("nd2", bu);
  vn.draw_network(buffer);
  extension("nt2", bu);
  vn.print_network(buffer);

  // Print rectangular cell network
  extension("ntd", bu);
  vn2.draw_network(buffer);
  extension("net", bu);
  vn2.print_network(buffer);
  ...
  extension("dom", bu);
  con.draw_domain_gnuplot(buffer);
}
