...
voronoicell_base::voronoicell_base(double max_len_sq)
    : current_vertices(init_vertices),
      ...
      pts(new double[current_vertices << 2]),
      tol(tolerance * max_len_sq),
      tol_cu(tol * sqrt(tol)),
      big_tol(big_tolerance_fac * tol),
    ...
    while (j < s * mec[i]) {
      ...
      if (k >= 0) {
        ...
        if (dsp == stackp2) {
          ...
          if (dsp == stackp3)
            voro_fatal_error("Couldn't relocate dangling pointer",
...
void voronoicell_base::add_memory_ds() {
  ...
  if (current_delete_size > max_delete_size)
    voro_fatal_error(
        "Delete stack 1 memory allocation exceeded absolute maximum",
        VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::add_memory_ds2() {
  ...
  if (current_delete2_size > max_delete2_size)
    voro_fatal_error(
        "Delete stack 2 memory allocation exceeded absolute maximum",
        VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::add_memory_xse() {
  ...
  if (current_xsearch_size > max_xsearch_size)
    voro_fatal_error(
        "Extra search stack memory allocation exceeded absolute maximum",
        VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::init_base(double xmin, double xmax, double ymin,
...
void voronoicell_base::init_tetrahedron_base(double x0, double y0, double z0,
  ...
  for (i = 0; i < p; i++)
    for (j = 0; j < nu[i]; j++) {
      ...
      while (ed[k][l] != i) {
        l++;
        if (l == nu[k])
          voro_fatal_error("Relation table construction failed",
                           VOROPP_INTERNAL_ERROR);
      }
      ed[i][nu[i] + j] = l;
    }
...
/** Starting from a point within the current cutting plane, this routine
 * attempts to find an edge to a point outside the cutting plane. This prevents
 * the plane routine from . \param[in,out] up */
...
inline bool voronoicell_base::search_upward(unsigned int &uw, int &lp, int &ls,
                                            int &us, double &l, double &u) {
  int vs;
  lp = up;
  l = u;
...
bool voronoicell_base::definite_max(int &lp, int &ls, double &l, double &u,
                                    unsigned int &uw) {
  int tp = lp, ts, qp = 0;
  unsigned int qw;
  double q;
...
inline bool voronoicell_base::search_downward(unsigned int &lw, int &lp,
                                              int &ls, int &us, double &l,
                                              double &u) {
  int vs;
  ...
  if (us == nu[up])
    if (definite_min(lp, us, l, u, lw)) return false;
...
bool voronoicell_base::definite_min(int &lp, int &us, double &l, double &u,
                                    unsigned int &lw) {
  int tp = up, ts, qp = 0;
  unsigned int qw;
  double q;
...
template <class vc_class>
bool voronoicell_base::nplane(vc_class &vc, double x, double y, double z,
                              double rsq, int p_id) {
...
template <class vc_class>
bool voronoicell_base::create_facet(vc_class &vc, int lp, int ls, double l,
                                    int us, double u, int p_id) {
...
template <class vc_class>
bool voronoicell_base::delete_connection(vc_class &vc, int j, int k,
                                         bool hand) {
...
bool voronoicell_base::failsafe_find(int &lp, int &ls, int &us, double &l,
                                     double &u) {
...
/** Calculates the contributions to the Minkowski functionals for this Voronoi
 * cell. \param[in] r the radius to consider. \param[out] ar the area
 * functional. \param[out] vo the volume functional. */
...
inline void voronoicell_base::minkowski_contrib(int i, int k, int m, double r,
                                                double &ar, double &vo) {
  double ix = pts[4 * i], iy = pts[4 * i + 1], iz = pts[4 * i + 2],
         kx = pts[4 * k], ky = pts[4 * k + 1], kz = pts[4 * k + 2],
         mx = pts[4 * m], my = pts[4 * m + 1], mz = pts[4 * m + 2],
         ux = kx - ix, uy = ky - iy, uz = kz - iz, vx = mx - kx, vy = my - ky,
         vz = mz - kz, e1x = uz * vy - uy * vz, e1y = ux * vz - uz * vx,
         e1z = uy * vx - ux * vy, e2x, e2y, e2z,
  ...
  double e3x = e1z * e2y - e1y * e2z, e3y = e1x * e2z - e1z * e2x,
         e3z = e1y * e2x - e1x * e2y, x0 = e1x * ix + e1y * iy + e1z * iz;
  if (x0 < tol) return;

  double ir = e2x * ix + e2y * iy + e2z * iz,
         is = e3x * ix + e3y * iy + e3z * iz,
         kr = e2x * kx + e2y * ky + e2z * kz,
         ks = e3x * kx + e3y * ky + e3z * kz,
         mr = e2x * mx + e2y * my + e2z * mz,
         ms = e3x * mx + e3y * my + e3z * mz;
...
void voronoicell_base::minkowski_edge(double x0, double r1, double s1,
                                      double r2, double s2, double r,
                                      double &ar, double &vo) {
...
void voronoicell_base::minkowski_formula(double x0, double y0, double z0,
                                         double r, double &ar, double &vo) {
  ...
  double xs = x0 * x0, ys = y0 * y0, zs = z0 * z0, res = xs + ys,
         rvs = res + zs, theta = atan(z0 / y0), rs = r * r, rc = rs * r, temp,
         voc, arc;
  if (r < x0) {
    temp =
        2 * theta - 0.5 * pi - asin((zs * xs - ys * rvs) / (res * (ys + zs)));
    voc = rc / 6. * temp;
    arc = rs * 0.5 * temp;
    ...
    double temp2 = (rs * x0 - xs * x0 / 3.), x2s = rs * xs / res,
           y2s = rs * ys / res, temp3 = asin((x2s - y2s - xs) / (rs - xs)),
           temp4 = asin((zs * xs - ys * rvs) / (res * (ys + zs))),
           temp5 = sqrt(rs - res);
    voc = 0.5 * temp * temp2 + x0 * y0 / 6. * temp5 +
          r * rs / 6 * (temp3 - temp4);
    arc = x0 * r * temp - 0.5 * temp2 * y0 * r / ((rs - xs) * temp5) +
          x0 * y0 / 6. * r / temp5 + rs * 0.5 * temp3 +
          rs * rs / 3. * 2 * xs * ys /
              (res * (rs - xs) *
               sqrt((rs - xs) * (rs - xs) -
                    (x2s - y2s - xs) * (x2s - y2s - xs))) -
          rs * 0.5 * temp4;
  } else {
    voc = x0 * y0 * z0 / 6.;
    arc = 0;
...
void voronoicell_base::draw_pov(double x, double y, double z, FILE *fp) {
  ...
  for (i = 0; i < p; i++, ptsp += 4) {
    sprintf(posbuf1, "%g,%g,%g", x + *ptsp * 0.5, y + ptsp[1] * 0.5,
            z + ptsp[2] * 0.5);
    ...
    for (j = 0; j < nu[i]; j++) {
      ...
      if (k < i) {
        pt2 = pts + (k << 2);
        sprintf(posbuf2, "%g,%g,%g", x + *pt2 * 0.5, y + 0.5 * pt2[1],
                z + 0.5 * pt2[2]);
...
void voronoicell_base::draw_gnuplot(double x, double y, double z, FILE *fp) {
  ...
  for (i = 1; i < p; i++)
    for (j = 0; j < nu[i]; j++) {
      ...
      if (k >= 0) {
        fprintf(fp, "%g %g %g\n", x + 0.5 * pts[i << 2],
                y + 0.5 * pts[(i << 2) + 1], z + 0.5 * pts[(i << 2) + 2]);
        l = i;
        m = j;
        do {
          ...
          l = k;
          fprintf(fp, "%g %g %g\n", x + 0.5 * pts[k << 2],
                  y + 0.5 * pts[(k << 2) + 1], z + 0.5 * pts[(k << 2) + 2]);
...
inline void voronoicell_base::normals_search(std::vector<double> &v, int i,
                                             int j, int k) {
  ed[i][j] = -1 - k;
 ...
 *                    coordinate system. */
void voronoicell_base::vertices(double x, double y, double z,
                                std::vector<double> &v) {
  v.resize(3 * p);
  double *ptsp = pts;
  for (int i = 0; i < 3 * p; i += 3) {
...
void voronoicell_base::output_vertices(double x, double y, double z, FILE *fp) {
  if (p > 0) {
    fprintf(fp, "(%g,%g,%g)", x + *pts * 0.5, y + pts[1] * 0.5,
            z + pts[2] * 0.5);
    for (double *ptsp = pts + 4; ptsp < pts + (p << 2); ptsp += 4)
      fprintf(fp, " (%g,%g,%g)", x + *ptsp * 0.5, y + ptsp[1] * 0.5,
              z + ptsp[2] * 0.5);
  }
}
...
bool voronoicell_base::plane_intersects(double x, double y, double z,
                                        double rsq) {
 ...
 * \param[in] rsq the distance along this vector of the plane.
 * \return False if the plane does not intersect the plane, true if it does. */
bool voronoicell_base::plane_intersects_guess(double x, double y, double z,
                                              double rsq) {
  up = 0;
  double g = x * pts[up << 2] + y * pts[(up << 2) + 1] + z * pts[(up << 2) + 2];
  if (g < rsq) {
 ...
 * \return False if the plane does not intersect the plane, true if it does. */
inline bool voronoicell_base::plane_intersects_track(double x, double y,
                                                     double z, double rsq,
                                                     double g) {
  for (int tp = 0; tp < p; tp++)
    if (x * pts[tp << 2] + y * pts[(tp << 2) + 1] + z * pts[(tp << 2) + 2] >
        rsq)
      return true;
  return false;
  /*
  ...
  for(us=0;us<nu[up];us++) {
          ...
          if(t>g) {
                  ...
                  while (t<rsq) {
                          if(++count>=p) {
#if VOROPP_VERBOSE >=1
                                  fputs("Bailed out of convex
calculation",stderr); #endif for(tp=0;tp<p;tp++)
if(x*pts[tp<<2]+y*pts[(tp<<2)+1]+z*pts[(tp<<2)+2]>rsq) return true; return
false;
                          }
...
void voronoicell_base::output_custom(const char *format, int i, double x,
                                     double y, double z, double r, FILE *fp) {
 ...
 * \param[in] (ymin,ymax) the minimum and maximum y coordinates.
 * \param[in] (zmin,zmax) the minimum and maximum z coordinates. */
void voronoicell_neighbor::init(double xmin, double xmax, double ymin,
                                double ymax, double zmin, double zmax) {
 ...
 * \param (x2,y2,z2) a position vector for the third vertex.
 * \param (x3,y3,z3) a position vector for the fourth vertex. */
void voronoicell_neighbor::init_tetrahedron(double x0, double y0, double z0,
                                            double x1, double y1, double z1,
                                            double x2, double y2, double z2,
                                            double x3, double y3, double z3) {
...
template bool voronoicell_base::nplane(voronoicell &, double, double, double,
                                       double, int);
template bool voronoicell_base::nplane(voronoicell_neighbor &, double, double,
                                       double, double, int);
template void voronoicell_base::check_memory_for_copy(voronoicell &,
                                                      voronoicell_base *);
template void voronoicell_base::check_memory_for_copy(voronoicell_neighbor &,
                                                      voronoicell_base *);
