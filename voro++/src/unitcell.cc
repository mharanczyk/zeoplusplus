 ...
 *                            vector. */
unitcell::unitcell(double bx_, double bxy_, double by_, double bxz_,
    ...
    : bx(bx_),
      ...
      byz(byz_),
      bz(bz_),
      unit_voro(max_unit_voro_shells * max_unit_voro_shells * 4 *
                (bx * bx + by * by + bz * bz)) {
  int i, j, l = 1;
...
bool unitcell::intersects_image(double dx, double dy, double dz, double &vol) {
  const double bxinv = 1 / bx, byinv = 1 / by, bzinv = 1 / bz,
               ivol = bxinv * byinv * bzinv;
  voronoicell c;
  c = unit_voro;
  dx *= 2;
  dy *= 2;
  ...
  if (!c.plane(0, -byinv, byz * byinv * bzinv, -dy + 1)) return false;
  if (!c.plane(bxinv, -bxy * bxinv * byinv, (bxy * byz - by * bxz) * ivol,
               dx + 1))
    return false;
  if (!c.plane(-bxinv, bxy * bxinv * byinv, (-bxy * byz + by * bxz) * ivol,
               -dx + 1))
...
/** Computes a list of periodic domain images that intersect the unit Voronoi
 * cell. \param[out] vi a vector containing triplets (i,j,k) corresponding to
 * domain images that intersect the unit Voronoi cell, when it is centered in
 * the middle of the primary domain. \param[out] vd a vector containing the
 * fraction of the Voronoi cell volume
...
void unitcell::images(std::vector<int> &vi, std::vector<double> &vd) {
  ...
  bool *a = new bool[mss],
       *ac = a + max_unit_voro_shells *(1 + ms2 * (1 + ms2)), *ap = a;
  int i, j, k;
  double vol;
...
bool unitcell::unit_voro_intersect(int l) {
  ...
  for (i = -l; i <= l; i++)
    if (unit_voro_test(i, l, 0)) return true;
  for (i = 1; i < l; i++)
    for (j = -l + 1; j <= l; j++) {
  ...
  for (i = -l; i <= l; i++)
    for (j = -l; j <= l; j++)
      if (unit_voro_test(i, j, l)) return true;
  return false;
}
...
void unitcell::draw_domain_gnuplot(FILE *fp) {
  fprintf(fp, "0 0 0\n%g 0 0\n%g %g 0\n%g %g 0\n", bx, bx + bxy, by, bxy, by);
  fprintf(fp, "%g %g %g\n%g %g %g\n%g %g %g\n%g %g %g\n", bxy + bxz, by + byz,
          bz, bx + bxy + bxz, by + byz, bz, bx + bxz, byz, bz, bxz, byz, bz);
  fprintf(fp, "0 0 0\n%g %g 0\n\n%g %g %g\n%g %g %g\n\n", bxy, by, bxz, byz, bz,
          bxy + bxz, by + byz, bz);
  fprintf(fp, "%g 0 0\n%g %g %g\n\n%g %g 0\n%g %g %g\n\n", bx, bx + bxz, byz,
          bz, bx + bxy, by, bx + bxy + bxz, by + byz, bz);
}
...
void unitcell::draw_domain_pov(FILE *fp) {
  ...
  fprintf(fp,
          ...
          "cylinder{<%g,%g,%g>,<%g,%g,%g>,rr}\n",
          bxz, byz, bz, bx + bxz, byz, bz, bxy + bxz, by + byz, bz,
          bx + bxy + bxz, by + byz, bz);
  fprintf(fp,
  ...
  fprintf(fp,
          "cylinder{<%g,%g,%g>,<%g,%g,%g>,rr}\n"
          "cylinder{<%g,%g,%g>,<%g,%g,%g>,rr}\n",
          bxz, byz, bz, bxy + bxz, by + byz, bz, bx + bxz, byz, bz,
          bx + bxy + bxz, by + byz, bz);
  fprintf(fp,
  ...
  fprintf(fp,
          ...
          "cylinder{<%g,%g,0>,<%g,%g,%g>,rr}\n",
          bxy, by, bxy + bxz, by + byz, bz, bx + bxy, by, bx + bxy + bxz,
          by + byz, bz);
  fprintf(fp,
          "sphere{<0,0,0>,rr}\nsphere{<%g,0,0>,rr}\n"
          "sphere{<%g,%g,0>,rr}\nsphere{<%g,%g,0>,rr}\n",
