...
class container_periodic_base : public unitcell, public voro_base {
 public:
   ...
   * the particle radii. */
  const int ps;
  container_periodic_base(double bx_, double bxy_, double by_, double bxz_,
                          double byz_, double bz_, int nx_, int ny_, int nz_,
                          int init_mem_, int ps);
  ...
  inline void print_all_particles() {
    ...
    for (ijk = 0; ijk < oxyz; ijk++)
      for (q = 0; q < co[ijk]; q++)
        printf("%d %g %g %g\n", id[ijk][q], p[ijk][ps * q], p[ijk][ps * q + 1],
               p[ijk][ps * q + 2]);
  }
  void region_count();
 ...
 protected:
  ...
  void put_locate_block(int &ijk, double &x, double &y, double &z, int &ai,
                        int &aj, int &ak);
  ...
  inline void remap(int &ai, int &aj, int &ak, int &ci, int &cj, int &ck,
...
class container_periodic : public container_periodic_base, public radius_mono {
 public:
  container_periodic(double bx_, double bxy_, double by_, double bxz_,
                     double byz_, double bz_, int nx_, int ny_, int nz_,
                     int init_mem_);
  void clear();
  ...
  inline bool compute_cell(v_cell &c, int ijk, int q) {
    int k(ijk / (nx * oy)), ijkt(ijk - (nx * oy) * k), j(ijkt / nx),
        i(ijkt - j * nx);
...
class container_periodic_poly : public container_periodic_base,
                                public radius_poly {
 public:
  container_periodic_poly(double bx_, double bxy_, double by_, double bxz_,
                          double byz_, double bz_, int nx_, int ny_, int nz_,
                          int init_mem_);
  void clear();
  void put(int n, double x, double y, double z, double r);
  void put(int n, double x, double y, double z, double r, int &ai, int &aj,
           int &ak);
