...
void help_message() {
  puts(
      "Voro++ version 0.4.6, by Chris H. Rycroft (UC Berkeley/LBL)\n\n"
      "Syntax: voro++ [options] <x_min> <x_max> <y_min>\n"
      "               <y_max> <z_min> <z_max> <filename>\n\n"
      "By default, the utility reads in the input file of particle IDs and "
      "positions,\n"
      "computes the Voronoi cell for each, and then creates <filename.vol> "
      "with an\n"
      ...
      " -l <len>   : Manually specify a length scale to configure the "
      "internal\n"
      "              computational grid\n"
      " -m <mem>   : Manually choose the memory allocation per grid block\n"
      "              (default 8)\n"
      " -n [3]     : Manually specify the internal grid size\n"
      " -o         : Ensure that the output file has the same order as the "
      "input\n"
      "              file\n"
      ...
      " -wb [6]    : Add six plane wall objects to make rectangular box "
      "containing\n"
      ...
      "              and displacement x4\n"
      " -y         : Save POV-Ray particles to <filename_p.pov> and POV-Ray "
      "Voronoi\n"
...
void custom_output_message() {
  puts(
      "The \"-c\" option allows a string to be specified that will customize "
      "the output\n"
      "file to contain a variety of statistics about each computed Voronoi "
      "cell. The\n"
      "string is similar to the standard C printf() function, made up of text "
      "with\n"
      "additional control sequences that begin with percentage signs that are "
      "expanded\n"
      "to different statistics. See http://math.lbl.gov/voro++/doc/custom.html "
      "for more\n"
      "information.\n"
      "\nParticle-related:\n"
      ...
      "  %n A list of neighboring particle or wall IDs corresponding to each "
      "face\n"
      "\nVolume-related:\n"
...
void version_message() { puts("Voro++ version 0.4.6 (October 17th 2013)"); }

// Prints an error message. This is called when the program is unable to make
// sense of the command-line options.
void error_message() {
  fputs(
      "voro++: Unrecognized command-line options; type \"voro++ -h\" for "
      "more\ninformation.\n",
      stderr);
}
...
void cmd_line_output(c_loop &vl, c_class &con, const char *format,
                     FILE *outfile, FILE *gnu_file, FILE *povp_file,
                     FILE *povv_file, bool verbose, double &vol, int &vcc,
                     int &tp) {
  int pid, ps = con.ps;
  double x, y, z, r;
...
int main(int argc, char **argv) {
  ...
  double ls = 0;
  blocks_mode bm = none;
  bool gnuplot_output = false, povp_output = false, povv_output = false,
       polydisperse = false;
  bool xperiodic = false, yperiodic = false, zperiodic = false, ordered = false,
       verbose = false;
  pre_container *pcon = NULL;
  pre_container_poly *pconp = NULL;
  wall_list wl;
  ...
  while (i < argc - 7) {
    if (strcmp(argv[i], "-c") == 0) {
      ...
      if (nx <= 0 || ny <= 0 || nz <= 0) {
        fputs(
            "voro++: Computational grid specified with -n must be greater than "
            "one\n"
            "in each direction\n",
            stderr);
        wl.deallocate();
  ...
  if (bm == none) {
    if (polydisperse) {
      pconp = new pre_container_poly(ax, bx, ay, by, az, bz, xperiodic,
                                     yperiodic, zperiodic);
      pconp->import(argv[i + 6]);
      pconp->guess_optimal(nx, ny, nz);
    } else {
      pcon = new pre_container(ax, bx, ay, by, az, bz, xperiodic, yperiodic,
                               zperiodic);
    ...
    if (bm == length_scale) {
      ...
      if (ls < tolerance) {
        ...
        if (ls < 0) {
          ...
          fprintf(stderr,
                  "The length scale is smaller than the safe limit of %g. "
                  "Either\nincrease the particle length scale, or recompile "
  ...
  const char *c_str =
      (custom_output == 0 ? (polydisperse ? "%i %q %v %r" : "%i %q %v")
                          : argv[custom_output]);
  ...
  if (polydisperse) {
    if (ordered) {
      particle_order vo;
      container_poly con(ax, bx, ay, by, az, bz, nx, ny, nz, xperiodic,
                         yperiodic, zperiodic, init_mem);
      con.add_wall(wl);
      if (bm == none) {
        pconp->setup(vo, con);
        delete pconp;
  ...
  if (verbose) {
    printf(
        "Container geometry        : [%g:%g] [%g:%g] [%g:%g]\n"
        "Computational grid size   : %d by %d by %d (%s)\n"
        "Filename                  : %s\n"
        "Output string             : %s%s\n",
        ax, bx, ay, by, az, bz, nx, ny, nz,
        bm == none ? "estimated from file"
                   : (bm == length_scale ? "estimated using length scale"
                                         : "directly specified"),
