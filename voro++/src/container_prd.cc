 ...
 *                particle. */
container_periodic_base::container_periodic_base(double bx_, double bxy_,
                                                 double by_, double bxz_,
                                                 double byz_, double bz_,
                                                 int nx_, int ny_, int nz_,
                                                 int init_mem_, int ps_)
  ...
  int *pp = co;
  while (pp < co + oxyz) *(pp++) = 0;
  pp = mem;
  while (pp < mem + oxyz) *(pp++) = 0;
  char *cp = img;
  while (cp < img + oxyz) *(cp++) = 0;
 ...
 *			    coordinate directions.
 * \param[in] init_mem_ the initial memory allocation for each block. */
container_periodic::container_periodic(double bx_, double bxy_, double by_,
                                       double bxz_, double byz_, double bz_,
                                       int nx_, int ny_, int nz_, int init_mem_)
    : container_periodic_base(bx_, bxy_, by_, bxz_, byz_, bz_, nx_, ny_, nz_,
                              init_mem_, 3),
 ...
 *			    coordinate directions.
 * \param[in] init_mem_ the initial memory allocation for each block. */
container_periodic_poly::container_periodic_poly(double bx_, double bxy_,
                                                 double by_, double bxz_,
                                                 double byz_, double bz_,
                                                 int nx_, int ny_, int nz_,
                                                 int init_mem_)
...
void container_periodic::put(int n, double x, double y, double z) {
  ...
  for (int l = 0; l < co[ijk]; l++)
    check_duplicate(n, x, y, z, id[ijk][l], p[ijk] + 3 * l);
  id[ijk][co[ijk]] = n;
...
void container_periodic_poly::put(int n, double x, double y, double z,
  ...
  put_locate_block(ijk, x, y, z);
  for (int l = 0; l < co[ijk]; l++)
    check_duplicate(n, x, y, z, id[ijk][l], p[ijk] + 4 * l);
  id[ijk][co[ijk]] = n;
 ...
 */
void container_periodic::put(int n, double x, double y, double z, int &ai,
                             int &aj, int &ak) {
  int ijk;
 ...
 * \param[in] n the numerical ID of the inserted particle.
 * \param[in] (x,y,z) the position vector of the inserted particle. */
void container_periodic::put(particle_order &vo, int n, double x, double y,
                             double z) {
  int ijk;
  put_locate_block(ijk, x, y, z);
  id[ijk][co[ijk]] = n;
 ...
 * \param[in] (x,y,z) the position vector of the inserted particle.
 * \param[in] r the radius of the particle. */
void container_periodic_poly::put(particle_order &vo, int n, double x, double y,
                                  double z, double r) {
  int ijk;
  put_locate_block(ijk, x, y, z);
  id[ijk][co[ijk]] = n;
 ...
 * false otherwise. */
void container_periodic_base::put_locate_block(int &ijk, double &x, double &y,
                                               double &z) {
...
inline void container_periodic_base::remap(int &ai, int &aj, int &ak, int &ci,
                                           int &cj, int &ck, double &x,
                                           double &y, double &z, int &ijk) {
  // Remap particle in the z direction if necessary
  ck = step_int(z * zsp);
  if (ck < 0 || ck >= nz) {
    ak = step_div(ck, nz);
...
bool container_periodic::find_voronoi_cell(double x, double y, double z,
  ...
  if (w.ijk != -1) {
    ...
    ci += w.di;
    if (ci < 0 || ci >= nx) ai += step_div(ci, nx);
 ...
 * \return True if a particle was found. If the container has no particles,
 * then the search will not find a Voronoi cell and false is returned. */
bool container_periodic_poly::find_voronoi_cell(double x, double y, double z,
                                                double &rx, double &ry,
                                                double &rz, int &pid) {
  int ai, aj, ak, ci, cj, ck, ijk;
  particle_record w;
...
void container_periodic_base::add_particle_memory(int i) {
  ...
  if (nmem > max_particle_memory)
    voro_fatal_error("Absolute maximum memory allocation exceeded",
                     VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 3
...
void container_periodic::print_custom(const char *format,
                                      const char *filename) {
 ...
 * \param[in] format the custom output string to use.
 * \param[in] filename the name of the file to write to. */
void container_periodic_poly::print_custom(const char *format,
                                           const char *filename) {
  FILE *fp = safe_fopen(filename, "w");
  print_custom(format, fp);
  fclose(fp);
...
void container_periodic_base::create_all_images() {
  ...
  for (k = 0; k < oz; k++)
    for (j = 0; j < oy; j++)
      for (i = 0; i < nx; i++) create_periodic_image(i, j, k);
}
      ...
      for (i = 0; i < nx; i++, l++)
        if (mem[l] > 0) {
          ...
          for (pp = p[l], c = 0; c < co[l]; c++, pp += ps)
            if (*pp < mix || *pp > max || pp[1] < miy || pp[1] > may ||
                pp[2] < miz || pp[2] > maz)
              printf("%d %d %d %d %f %f %f %f %f %f %f %f %f\n", id[l][c], i, j,
                     k, *pp, pp[1], pp[2], mix, max, miy, may, miz, maz);
        }
}
...
void container_periodic_base::create_side_image(int di, int dj, int dk) {
  ...
  double dis = ima * bxy + quadiv * bx,
         switchx = di * boxx - ima * bxy - quadiv * bx, adis;
...
void container_periodic_base::create_vertical_image(int di, int dj, int dk) {
  int l, dijk = di + nx * (dj + oy * dk), dijkl, dijkr,
  ...
  int fi = qi - qidiv * nx, fj = qj - qjdiv * ny,
      fijk = fi + nx * (fj + oy * (dk - ima * nz)), fijk2;
...
void container_periodic_base::put_image(int reg, int fijk, int l, double dx,
                                        double dy, double dz) {
