...
int main() {
  ...
  container con(x_min, x_max, y_min, y_max, z_min, z_max, 10, 10, 10, false,
                false, false, 8);
  ...
  if (cl.start()) do
      if (con.compute_cell(c, cl)) {
        ...
        y1 = 2 * rnd() - 1;
        z1 = 2 * rnd() - 1;
        r = 1 / sqrt(x1 * x1 + y1 * y1 + z1 * z1);
        x1 *= r;
        y1 *= r;
        z1 *= r;

        // Construct a second perpendicular vector
        if (abs(x1) > 0.5 || abs(y1) > 0.5) {
          r = 1 / sqrt(x1 * x1 + y1 * y1);
          x2 = -y1 * r;
          y2 = x1 * r;
          z2 = 0;
        } else {
          r = 1 / sqrt(x1 * x1 + z1 * z1);
          x2 = -z1 * r;
          y2 = 0;
          z2 = x1 * r;
        }
        ...
        for (z = vx; z <= v; z += h)
          for (y = vy; y <= v; y += h)
            for (x = vz; x <= v; x += h) {
              ...
              if (xt < x_min || xt > x_max || yt < y_min || yt > y_max ||
                  zt < z_min || zt > z_max)
                continue;
