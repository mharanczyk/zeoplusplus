...
struct wall_shell : public wall {
 public:
  wall_shell(double xc_, double yc_, double zc_, double rc, double sc,
             int w_id_ = -99)
  ...
  bool cut_cell_base(v_cell &c, double x, double y, double z) {
    double xd = x - xc, yd = y - yc, zd = z - zc,
           dq = xd * xd + yd * yd + zd * zd, dq2;
    if (dq > 1e-5) {
      dq2 = 2 * (sqrt(dq) * lc - dq);
      dq = 2 * (sqrt(dq) * uc - dq);
      return c.nplane(xd, yd, zd, dq, w_id) &&
             c.nplane(-xd, -yd, -zd, -dq2, w_id);
    }
    return true;
  }
...
int main() {
  ...
  if (vl.start()) do
      if (con.compute_cell(c, vl)) {
        ...
        c.neighbors(vi);
        if (vi.size() > nface + 2)
          voro_fatal_error("Too many faces; boost nface", 5);

        for (l = 0; l < (signed int)vi.size(); l++)
          if (vi[l] >= 0) qp[qn[i]++] = vi[l];
      }
    while (vl.inc());
  ...
  for (l = 0; l < particles; l++) {
    ...
    z = p[j + 2] - p[k + 2];
    dz = p[j + 2] - p[o + 2];
    if (p[j] * (y * dz - z * dy) + p[j + 1] * (z * dx - x * dz) +
            p[j + 2] * (x * dy - y * dx) <
        0) {
      for (i = 0; i < qn[l] / 2; i++) {
        o = q[l * nface + i];
