 ...
 public:
  ...
  bool cut_cell_base(v_cell &c, double x, double y, double z) {
    ...
    double xx = x * x, yy = y * y, zz = z * z, s[4],
           b_cal = 1.0 - (xx + yy) * rr_inv + (4.0 - zz * rr_inv) * aa_inv +
                   aa_inv * aa_inv,
           c_cal = 2.0 * (1.0 - (xx + yy + zz) * rr_inv) * aa_inv +
                   2.0 * aa_inv * aa_inv,
           d_cal = (1.0 - (xx + yy + zz * aa) * rr_inv) * aa_inv * aa_inv,
           xell[4], yell[4], zell[4], d2[4];
    int i, j = 0,
           q = quarticSolve(a_cal, b_cal, c_cal, d_cal, s[0], s[1], s[2], s[3]);

    for (i = 0; i < q; i++) {
      xell[i] = x / (1 + s[i]);
      yell[i] = y / (1 + s[i]);
      zell[i] = z / (1 + s[i] * aa);
      d2[i] = (xell[i] - x) * (xell[i] - x) + (yell[i] - y) * (yell[i] - y) +
              (zell[i] - z) * (zell[i] - z);
    }

    for (i = 1; i < 4; i++)
      if ((d2[i] <= d2[j]) && (abs(xell[i] * xell[i] + yell[i] * yell[i] +
                                   aa * zell[i] * zell[i] - rr) < 1e-4))
        j = i;

    double xn = xell[j] - x, yn = yell[j] - y, zn = zell[j] - z,
           dn2 = xn * xn + yn * yn + zn * zn;
